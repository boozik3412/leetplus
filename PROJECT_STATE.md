...
- Platform administration: `/administration` is a separate platform-admin-only control plane for tenants, Langame sources, diagnostics, tenant lifecycle actions, support notes, and audit trail across all tenants; legacy `/admin` redirects there.
...
- Expanded platform administration: tenant lifecycle statuses, lifecycle actions with slug confirmation and audit trail, support notes, production diagnostics, and inactive-tenant auth blocking for ordinary users while platform admins keep recovery access.
