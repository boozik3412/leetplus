...
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `EXACT-SHA LEDGER CI ACCEPTED + LOCAL PROVIDER 8/8 / DORMANT / NO GRANTS`          | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Exact replay после lost register/consume/revoke/expiry response возвращает только тот же request; изменённый replay fail closed. Registration сверяет DB/role OID, negative role attributes, memberships/ownership/default ACL, DB/schema ACL, полный PG16 table/sequence privilege set и ровно 4 callable RPC. Disposable PG matrix принята на SHA `0578e5e4…`, CI `31688314546` `3/3 SUCCESS`, artifact `sha256:9e0a8a9b…fa62c`. Local synthetic provider принимает только branded CURRENT193 `SYNTHETIC_CI`, frozen lost-response spec, exact `claim/execute/reconcile`, ambiguity close и zero-inflight drain; coordinator не принимает arbitrary query. Production entry всегда deny; миграция не создаёт role и не выдаёт grants/route authority | Реальный owner/runtime DB client с pinned TLS/role identity и process lifecycle; затем canonical restored-copy rehearsal |
