...
CURRENT187-J5-R1 локально реализует capability-bearing runner foundation:
branded J1–J4 chain, 4 positive bindings, 20 actual negative PostgreSQL
connection attempts и 12 non-mutating control-policy evaluations. Runner `9/9`,
J5 `19/19`, aggregate CURRENT187 `98/98`, typecheck green. Actual disposable
topology, production-like execution, signer/root, persistence и F/deploy binding
ещё обязательны; внешний доступ остаётся `NO-GO`.
...
| `CURRENT187-J5-R1`                   |        P0 | `ENGINEERING ACCEPTED LOCALLY / SYNTHETIC CAPABILITIES / DENY-ONLY`               | Branded J1–J4 chain; 4 actual positive receipt bindings; 20 classified negative PostgreSQL connection attempts; 12 non-mutating stale-HBA/pool-mode/identity-collapse policy evaluations; exact scenario transport modes; allowed/unclassified/throw/disconnect fail closed; receipt excludes URLs, credentials, CA/host/database/error text; runner `9/9`, combined J5 `19/19`, aggregate CURRENT187 `98/98`, typecheck green; all access/effect flags false                                                                                                                                                                                                                                                                                                                                                                                                                            | Actual disposable TLS/HBA/PgBouncer matrix; production-like four-service run; protected signer/root; persisted consumption/revocation; F/deploy binding; independent review                                                                                |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   CI `31594459396`. Capability-bearing runner foundation локально принят:
   runner `9/9`, J5 `19/19`, aggregate CURRENT187 `98/98`. Следующим шагом
   выполнить actual disposable 36-outcome integration, затем добавить protected
   signer/root и закрыть CURRENT187
