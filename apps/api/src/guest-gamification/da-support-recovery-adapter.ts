...
export function parseDaSupportMode(value: string | undefined): DaSupportMode {
  if (value === 'preview' || value === 'apply' || value === 'reconcile')
    return value;
  throw new ConflictException(
    'DA support mode must be preview, apply, or reconcile.',
  );
}

export function assertDaApplyConfirmation(
  value: string | undefined,
  digest: string,
) {
  if (value !== `${DA_SUPPORT_RECOVERY.confirmationPrefix}${digest}`)
    throw new ConflictException(
      'DA apply confirmation must bind the fresh preview digest.',
    );
}
...
export interface DaRecoveryService {
  previewSupportRewardRecovery(
    user: unknown,
    request: ReturnType<typeof daSupportRequest>,
  ): Promise<unknown>;
  applySupportRewardRecovery(
    user: unknown,
    request: ReturnType<typeof daSupportRequest> & {
...
export async function runDaSupportRecovery(input: {
  ...
  if (!input.preview)
    throw new ConflictException(
      'DA apply requires the immediately preceding preview.',
    );
