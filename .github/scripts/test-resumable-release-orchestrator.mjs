...
function smokeRolledBackSupersessionArgs(
  root,
  planSha256,
  replacementReleaseSha = SUCCESSOR_SHA,
  slotBindReceiptSha256 = "0".repeat(64),
  ...
  args[0] = "supersede-after-smoke-bind-rollback";
  return args;
}
...
async function publishAcceptedBindFixtureSlotRollback(
  ...
  const indexPath = path.join(receiptRoot, "blue.latest");
  const index = new Map(parseKv(await readFile(indexPath, "utf8")));
  const bindReceiptPath = index.get("RECEIPT_PATH");
  assert.ok(bindReceiptPath);
  const bindReceipt = await readFile(bindReceiptPath, "utf8");
  const bindEntries = parseKv(bindReceipt);
  const bind = new Map(bindEntries);
  const operationId = bind.get("OPERATION_ID");
  const priorReleaseSha = bind.get("PRIOR_RELEASE_SHA");
  const priorTarget = bind.get("PRIOR_TARGET");
  const bindAcceptedAt = bind.get("ACCEPTED_AT");
  assert.ok(operationId);
  assert.ok(priorReleaseSha);
  assert.ok(priorTarget);
  assert.ok(bindAcceptedAt);
  const unmaskIntent = JSON.parse(
    await readFile(
      path.join(
        root,
        "var/lib/leetplus/deploy-receipts/release-orchestrator",
        OPERATION_ID,
        "03-smoke-unmask.intent.json",
      ),
      "utf8",
    ),
  );
  ...
  const state = await fixtureState(root);
  state.bound = false;
  state.slotActive = leaveTargetActive;
  state.slotFailed = false;
  state.slotMasked = false;
  state.sourceReleaseSha = priorReleaseSha;
  await writeJson(path.join(root, "fixture-state.json"), state);
...
test("supersedes an approved HYDRATE-intent operation before runtime effects", async (t) => {
  ...
  assert.equal(state.cutoverEffects, 0);
  assert.equal(
    await main(supersessionArgs(fixture.root, fixture.planSha256, RELEASE_SHA)),
    1,
  );
  await assert.rejects(
...
test("rejects supersession after any phase has been accepted", async (t) => {
  ...
  assert.equal(
    await main(
      smokeRolledBackSupersessionArgs(fixture.root, fixture.planSha256),
    ),
    1,
  );
  await assert.rejects(
...
test("terminalizes an exact failed cross-slot bridge only after receipt-bound bind rollback", async (t) => {
  ...
  state.releaseSha = SUCCESSOR_SHA;
  await writeJson(path.join(root, "fixture-state.json"), state);
  assert.equal(await main(rolledBackSupersessionArgs(root, planSha256)), 1);
  ...
  const supersessionPath = path.join(path.dirname(planPath), "superseded.json");
  const supersession = JSON.parse(await readFile(supersessionPath, "utf8"));
  ...
  nextArgs[nextArgs.indexOf("--release-sha") + 1] = SUCCESSOR_SHA;
  nextArgs[nextArgs.indexOf("--previous-migration") + 1] = CURRENT191_MIGRATION;
  nextArgs[nextArgs.indexOf("--previous-migration-count") + 1] = "191";
  assert.equal(await main(nextArgs), 0);
});

test("terminalizes an accepted BIND after failed SMOKE only after exact rollback and target restoration", async (t) => {
  const bridgeOptions = {
    bridgeMode: "ALLOW_CURRENT_190",
    bugReportingMode: "OFF",
    migration: CURRENT191_MIGRATION,
  ...
  const root = await setupFixture(
    "post-smoke-bind-rollback-supersession-",
    bridgeOptions,
  );
  t.after(() => rm(root, { recursive: true, force: true }));
  const args = current191PrepareArgs(root, "current191-bridge");
  ...
  const planPath = path.join(operationDirectory, "plan.json");
  const plan = JSON.parse(await readFile(planPath, "utf8"));
  const planSha256 = canonicalRecordSha256(plan);
  let state = await fixtureState(root);
  ...
  assert.equal(state.hydrationEffects, 1);
  assert.equal(state.bindEffects, 1);
  assert.equal(state.runtimeStartCalls, 2);
  assert.equal(state.slotActive, true);
  assert.equal(state.cutoverEffects, 0);
  await lstat(path.join(operationDirectory, "02-bind.receipt.json"));
  await lstat(path.join(operationDirectory, "03-smoke.intent.json"));
  await lstat(path.join(operationDirectory, "03-smoke-unmask.intent.json"));
  await assert.rejects(
    lstat(path.join(operationDirectory, "03-smoke.evidence.json")),
    { code: "ENOENT" },
  );
  await assert.rejects(
    lstat(path.join(operationDirectory, "03-smoke.receipt.json")),
    { code: "ENOENT" },
  );
  state.releaseSha = SUCCESSOR_SHA;
  await writeJson(path.join(root, "fixture-state.json"), state);
  assert.equal(
    await main(smokeRolledBackSupersessionArgs(root, planSha256)),
    1,
  );

  const rollback = await publishAcceptedBindFixtureSlotRollback(root, {
    leaveTargetActive: true,
  });
  const targetEnvironmentPath = path.join(root, "etc/leetplus/slots/blue.env");
  await chmod(targetEnvironmentPath, 0o600);
  await copyFile(
    path.join(operationDirectory, "02-bind-slot-environment.previous.env"),
    targetEnvironmentPath,
  );
  await chmod(targetEnvironmentPath, 0o440);
  const exactSupersessionArgs = smokeRolledBackSupersessionArgs(
    root,
    planSha256,
    SUCCESSOR_SHA,
    rollback.bindReceiptSha256,
  ...
  assert.equal(await main(exactSupersessionArgs), 1);
  ...
  assert.equal(
    await main(
      smokeRolledBackSupersessionArgs(
        root,
        planSha256,
        SUCCESSOR_SHA,
        "0".repeat(64),
  ...
  const targetSlotPath = path.join(root, "srv/leetplus/slots/blue");
  await rm(targetSlotPath, { force: true });
  await symlink(
    path.join(root, "srv/leetplus/releases", RELEASE_SHA),
    targetSlotPath,
  );
  assert.equal(await main(exactSupersessionArgs), 1);
  await rm(targetSlotPath, { force: true });
  await symlink(
    path.join(root, "srv/leetplus/releases", PREVIOUS_SHA),
    targetSlotPath,
  );
  assert.equal(await main(exactSupersessionArgs), 0);
  assert.equal(await main(exactSupersessionArgs), 0);

  const supersessionPath = path.join(operationDirectory, "superseded.json");
  const supersession = JSON.parse(await readFile(supersessionPath, "utf8"));
  assert.equal(supersession.schemaVersion, 3);
  assert.equal(supersession.decision, SMOKE_ROLLED_BACK_SUPERSEDED_DECISION);
  assert.equal(supersession.completedPhases, 2);
  assert.equal(supersession.pendingPhase, "SMOKE");
  assert.equal(supersession.pendingRecord, "INTENT");
  assert.equal(supersession.targetPriorReleaseSha, PREVIOUS_SHA);
  assert.equal(supersession.replacementReleaseSha, SUCCESSOR_SHA);
...
test("rejects post-SMOKE supersession when binder BIND predates quiesce", async (t) => {
  ...
  const root = await setupFixture(
    "post-smoke-stale-bind-supersession-",
    bridgeOptions,
  );
  t.after(() => rm(root, { recursive: true, force: true }));
  const args = current191PrepareArgs(root, "current191-bridge");
