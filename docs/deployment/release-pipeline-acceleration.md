...
| Fast CI `33870481213` подтвердил exact cgroup/InvocationID, но existence-check объявил masked manager transport доступным | `InaccessiblePaths=` сохраняет в namespace недоступный placeholder inode, поэтому `-e`/успешный `stat` не доказывает возможность Unix-socket соединения | canary и реальный Node entrypoint fail-closed проверяют тип endpoint через `-S`/`stats.isSocket()`: любой видимый socket отклоняется, а inaccessible non-socket placeholder корректно считается изолированным |
