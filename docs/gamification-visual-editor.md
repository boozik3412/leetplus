...
Редактор работает по схеме `черновик -> публикация` только для композиции гостевой страницы и визуальных параметров. Условия, статус и клубная область лутбоксов, заданий, Battle Pass и промо-баннеров задаются расширенным редактором. Подтвержденная активация такого элемента сразу включает его в актуальную конфигурацию выбранных клубов; повторная публикация визуального draft для этого не нужна.
...
- `GET /guests/gamification/visual-editor/draft?id=&storeId=` - получить существующий draft или собрать его из актуальной конфигурации выбранного клуба.
- `PATCH /guests/gamification/visual-editor/draft` - сохранить черновик.
- `POST /guests/gamification/visual-editor/draft/publish` - валидировать и применить визуальный draft в гостевую конфигурацию.
...
## Правила активации и публикации

- Расширенный редактор подтверждает активацию, деактивацию и удаление элементов с перечислением затрагиваемых клубов. После подтверждения активные размещения синхронизируются с игровым контуром сразу.
- Публикация визуального редактора не должна сбрасывать правила других клубов и не является обязательным шагом для уже активированного правила.
- Активация чек-ина блокируется, если не выбрана награда.
- Чек-ин применяется как управляемое правило `CHECK_IN`.
- Гостевая кнопка чек-ина показывается только при активном check-in rule для выбранного клуба.
- Battle Pass, лутбоксы, задания и promo cards применяются tenant-scoped и store-scoped, без live-запросов в Langame из браузера.
