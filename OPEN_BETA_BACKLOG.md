...
| `CURRENT187-J`                       |        P0 | `FOUNDATION + J1 EXACT-HEAD CI ACCEPTED / DENY-ONLY`                              | Foundation: exact `APPLICATION/COORDINATOR/MIGRATION/WORKER` synthetic matrix, separate network/host-control brands and five deployment digests, SHA `04ffff27…`, CI `31420665364` — `3/3 SUCCESS`. J1: capability-bearing actual Prisma/PostgreSQL backend-session collector checks exact DB/role OID, application, backend/network, read-only, TLS and role-policy facts; unit `11/11`, aggregate CURRENT187 `53/53`; exact SHA `a9513c69…`, CI `31577001152` — `3/3 SUCCESS`, actual loopback PG smoke `SUCCESS`, artifact `sha256:8e0c26f5…d334`. J1 keeps endpoint/HBA/PgBouncer/negative-probe and every access/effect flag false                                                                                                                                                                                                                                                  | Four production service-identity runs; independent endpoint/TLS-peer, HBA/reload and PgBouncer collectors; negative probes; host signer/root, freshness and persisted consumption/revocation; hostile real topology; binding into F/deploy authority; independent review |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   session collector также принят exact-SHA CI. Следом реализовать independently
   signed endpoint/TLS-peer/HBA/PgBouncer collectors и закрыть CURRENT187
