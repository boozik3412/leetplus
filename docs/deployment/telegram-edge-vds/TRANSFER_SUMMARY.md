...
- `telegram-edge` пересылает updates в основной LeetPlus API и отправляет replies в Telegram.
...
- Edge 1337 не должен хардкодить старую Mini App клавиатуру: он использует `replyMarkup` из основного API как opaque JSON. Если позже включить API-side sender, основной API вернет `replyDispatch=SENT` без `reply` payload, поэтому edge не продублирует отправку.
...
Основная VDS не отправляет Telegram replies напрямую в базовой live-схеме: `GUEST_GAME_TELEGRAM_WEBHOOK_REPLY_ENABLED=false`. Sender считается готовым через 1337 polling edge.
