...
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `PRISMA EXACT-SHA CI ACCEPTED + BOOTSTRAP LOCAL 5/5 / DORMANT`                      | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events; provider `8/8` сохраняет frozen replay, exact `claim/execute/reconcile`, ambiguity close и zero-inflight drain. Separate Prisma adapter `8/8` создаёт owner/runtime clients только на loopback CI, сверяет live DB/role/OID и не предоставляет arbitrary SQL; actual disposable-clone step принят exact SHA `2a5811e0…`, CI `31692243774` `3/3 SUCCESS`, artifact `sha256:8a336bb0…10f2`. Новый bootstrap `5/5` до создания clients проверяет signed CURRENT193 receipt, затем атомарно собирает Prisma→persisted register/consume→provider session и закрывает обе сессии при rejected/ambiguous startup; actual composed CI ещё pending. Production entry всегда deny; production credentials/roots/roles/grants не создаются | Принять composed bootstrap CI; затем pinned production TLS, revoke/credential rotation и canonical restored-copy rehearsal |
