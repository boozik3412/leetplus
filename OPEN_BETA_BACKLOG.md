...
| `CURRENT193-LANGAME-RUNTIME`         |        P0 | `FOUNDATION + DISPOSABLE ROLE MATRIX LOCALLY / NONAUTHORIZING / ROOTS EMPTY`       | Exact CI-only planner связывает release SHA, database identity, отдельные owner/executor OID и CURRENT192 SQL SHA; catalog matcher требует fixed LOGIN role с NOINHERIT/NOSUPERUSER/NOCREATEDB/NOCREATEROLE/NOREPLICATION/NOBYPASSRLS, zero membership/ownership/table/sequence/default privileges, exact DB/schema ACL и ровно три callable SECURITY DEFINER RPC при двух non-callable guards. Branded plan/receipt, proxy/accessor/clone/drift fail closed. Rollback-only PG16 matrix создаёт роль, снимает inherited PUBLIC, выдаёт ровно 3 RPC и отрицает table/sequence/guard/DDL/TEMP/role escalation; static/pure `9/9`, `authorization=false`, production roots frozen empty                                                                                                              | Принять actual PG16 matrix в exact-SHA CI; signed authority и persisted consume/revoke/lost-response ledger; dedicated runtime client вместо broad Prisma; lifecycle/drain/rotation; production-like rehearsal                                               |
