...
| `CURRENT188-LANGAME`                 |        P0 | `IMPLEMENTED NONCANONICAL / ACTIVATION + WEB BFF DORMANT`                         | foundation `6/6`; API `180/180`; disposable PG stage/replay/cross-tenant/stale/expiry/lost-response smoke `PASS`; exact HMAC-bound activation adapter default-off и без автозапуска sync; exhaustive-unimported Web BFF candidate `12/12` принимает только исходный same-origin Request, exact preview/activate POST без query/hash, cookie-only JWT и bounded 8 KiB stream; требует exact Nest `201 application/json`, связывает ACTIVATE response с request `receiptId` и возвращает safe projection. Legacy external sync теперь service-level fail-closed на fresh `PILOT/BETA/LIVE` до credential/provider/job/mutation (`12/12` local), поэтому не может автоматически создать все provider clubs; status/reconcile/initial read-only sync остаются blockers                                                                                                                       | Принять exact-SHA CI legacy-deny evidence; canonical consolidation, execute-only runtime role/grants, expirer, exact status/reconcile API и отдельно одобренный initial read-only sync; atomic Route/UI cutover и browser A/B                              |
