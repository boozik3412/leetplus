...
- SHA-bound artifact runtime layout расширен manifests, web public assets,
  canonical migrations и founder operational scripts. CI до upload обязан
  распаковать архив, проверить внутренний `SHA256SUMS`, выполнить frozen offline
  production install, Prisma generate и CLI/runtime resolution. Локальный
  layout/tar/checksum/extract smoke зелёный; exact-SHA CI нового artifact ещё
  pending
...
| `FOUNDER-ACTIVATION-ROLE-NETWORK`       |        P0 | `ENGINEERING IMPLEMENTED / UNIT 7/7 + SYNTHETIC TLS/SCRAM PASS / PROD NO-GO`      | Exact pre/post role receipt+catalog attestation, HBA catalog precedence, exact hostssl target+role+127/32 SCRAM allow, hostssl other-DB reject, hostnossl reject, CA SHA/standard IP peer verification, TLS session identity, correct login, wrong-secret `28P01`, other-DB/plaintext `28000`, direct table denial, stage-safe failure codes и secret/path/error-free receipt. Одноразовый PG16.13 на exact Git archive `032bacbf…`: TLS1.3/AES-256-GCM, evidence `5674b09f…dd7b`; после controlled role rollback process/port/temp residue 0. Implementation ещё ожидает own exact SHA/CI artifact; production не менялся                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Принять own exact SHA/CI; повторить на downloaded artifact + immutable production backup; затем PgBouncer/dedicated pool session identity/drain и live API process acceptance                                                                              |
