...
- Основной API отправляет safe reply в Telegram через Bot API: сначала кнопку `request_contact`, затем после contact-share inline-выбор `Вернуться на сайт LeetPlus`, `Открыть Mini App`, `Продолжить в боте`. При успешной API-side отправке ответ содержит `replyDispatch=SENT` без `reply` payload, поэтому edge не может продублировать `sendMessage`; если API-side sender выключен, edge может отправить safe reply как fallback.
...
API-side Telegram sender включен на основной VDS. Edge adapter на 1337 продолжает polling, прокидывает updates в API и отправляет reply только если основной API вернул safe `reply` payload.
