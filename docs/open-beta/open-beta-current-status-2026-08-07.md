...
> `39/39`, aggregate CURRENT187 `118/118`; actual hostile PG fixture включён в
> CI, но exact-SHA результат ещё не принят. Статус остаётся `NO-GO`.
...
| CURRENT187-J5-R3 ledger           | `NONCANONICAL PG CANDIDATE / STATIC ACCEPTED / CI PENDING` | exact transferable contract plus append-only/FORCE-RLS/owner-only candidate, exact-OID roles, execute-only consume/revoke RPC, total lock order and post-lock freshness; contract `6/6`, static `7/7`, combined J5 `39/39`, aggregate CURRENT187 `118/118`; actual hostile PG CI, independent review, canonical promotion and F binding pending                                                                              |
