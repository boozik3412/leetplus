...
artifact `sha256:5e8e07de…a3a`. CURRENT192 atomic selected-Store import step уже
прошёл PostgreSQL на SHA `6b386548…` в run `31686624414`; общий run не принят
из-за следующего CURRENT193 guard-search-path mismatch. Локальная коррекция
CURRENT193 даёт static `17/17`. Новый dormant CURRENT194 checksum-bound ledger
даёт static `7/7` и фиксирует exact register/consume/revoke/expiry replay без
каких-либо runtime grants. Production, route/UI, текущая сеть из четырёх
клубов и первый внешний tenant не изменяются.
...
| `CURRENT192-LANGAME-EXECUTION`       |        P0 | `PG STEP ACCEPTED AT EXACT SHA / FULL RUN FAILED DOWNSTREAM / DORMANT / NO RUNTIME GRANTS` | One-time approval-bound claim с digest raw token; DB независимо проверяет exact canonical CURRENT191 bytes/target/authorization/counts/order. Одна транзакция re-lock/re-prove tenant/GO/NETWORK/CURRENT188–191 binding, сохраняет manual product fields, не деактивирует missing SKU, пишет только выбранный Store UTC inventory snapshot и terminal `COMPLETED`; exact replay/reconcile/expired-unexecuted claim предусмотрены. Default-off/production-denied coordinator принимает только branded plan и после lost response продолжает только из proven `CLAIMED`; static `6/6`, planner `17/17`, coordinator `11/11`, Langame `247/247`. В run `31686624414` на SHA `6b386548…` Authority/Application и CURRENT192 PostgreSQL step `65` завершились успешно; общий run не принят из-за следующего CURRENT193 step                                                                                  | Получить единый `3/3 SUCCESS` на SHA с исправленным CURRENT193 и CURRENT194; затем runtime provider, canonical apply/rollback/zero-diff, atomic route/BFF/UI cutover и pilot                                                                                     |
| `CURRENT193-LANGAME-RUNTIME`         |        P0 | `LOCAL FIX + STATIC 17/17 / PG RE-RUN PENDING / NONAUTHORIZING / ROOTS EMPTY`     | Exact CI-only planner связывает release SHA, database identity, отдельные owner/executor OID и CURRENT192 SQL SHA; fixed LOGIN role имеет отрицательные privilege attributes, zero membership/ownership/direct/default privileges, exact DB/schema ACL и ровно 3 callable RPC при 2 guards. Rollback-only PG16 matrix снимает inherited PUBLIC и отрицает полный PG16 table privilege set, sequence/guard/DDL/TEMP/role escalation. Independent Ed25519 verifier связывает branded catalog receipt+release+DB/role OID+SQL SHA максимум на 5 минут. Run `31686624414` дал точную диагностику: guard-функции реально имеют `SET search_path = pg_catalog, public`, а remote fixture ожидал `NULL`; локальный matcher/fixture исправлен, static снова `17/17`. Narrow application DI port не принимает broad `PrismaService`; `authorization=false`, production roots empty                                                             | Принять исправленную actual PG16 matrix в одном exact-SHA CI; затем dedicated attested runtime provider, lifecycle/drain/rotation и canonical production-like rehearsal                                                                                      |
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `LOCAL FOUNDATION 7/7 / CI WIRED / DORMANT NONCANONICAL / NO GRANTS`              | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Exact replay после lost register/consume/revoke/expiry response возвращает только тот же request; изменённый replay fail closed. Registration заново сверяет DB/role OID, negative role attributes, memberships/ownership/default ACL, DB/schema ACL, полный PG16 table/sequence privilege set и ровно 4 callable RPC. Disposable smoke создаёт fixed role и grants только внутри rollback, запрещает runtime direct table read и TEMP, затем проверяет zero role residue. Миграция не создаёт role, не выдаёт PUBLIC/runtime grants, не содержит production/root/route authority; Application и PostgreSQL CI gates подключены                                                                                     | Принять actual PG16 register/consume/expiry/revoke/replay matrix на exact SHA; реализовать dedicated provider и process lifecycle; затем canonical restored-copy rehearsal                                                                                   |
