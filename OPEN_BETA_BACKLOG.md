...
| `FOUNDER-OPERATOR-BETA-GO`              |        P0 | `V2 + RUNTIME BOUNDARY IMPLEMENTED / PRODUCTION NO-GO`                                      | Ручное создание tester account запрещено. GO/revoke, atomic v2 activation и dedicated database pool/live least-privilege assertion не требуют USB/offline key; default mode `DISABLED`. V2 одной SERIALIZABLE transaction создаёт OWNER/NETWORK invite, запускает 30-day trial, consume GO и выполняет единственный HOLD→PENDING. Prisma/API/database checks, identity-mail regression, clean PG16 `184` migrations и restricted-role activation/replay/immutability/drift приняты; revoke/reissue/replay принят exact-SHA CI `f33e598a…`. Downloaded artifact child-process зелёный. Response secret-free. Production и текущие четыре клуба не изменены; PUBLIC/обычные roles не имеют RPC EXECUTE, production activation role/secret/grant не создавались                                                                                                                                                                                                                                                                                                                                                   | Создать role secret и применить grant/attestation на restored-copy; apply/replay/rollback; SMTP SENT/reissue/accept; Gate 1MT/2; затем PREPARE→one-tenant ACTIVE canary и mailbox-bound OWNER invite                                                       |
...
| `FOUNDER-OWNER-INVITE-LIFECYCLE`        |        P0 | `STATUS + REVOKE + REISSUE EXACT-SHA ACCEPTED / PROD NO-GO`                                | Status/revoke принят на `0bc178bc…`, PR CI `32084166159` — `4/4 SUCCESS`. Reissue создаёт только новый encrypted OWNER invite/outbox/token, фиксирует immutable predecessor-chain, выполняет exact replay до чтения successor aggregate и запрещает blind resend. Common tenant lock, fresh DB authority, invite CAS, causal outbox timestamps, pre-provider cancellation/evidence preservation и IdentityEmailClaim replacement выполняются атомарно. Local typecheck/lint, owner lifecycle `2 suites / 18`, static migration `4/4`, full API `157 suites / 3144 passed` и real PG `1/1` PASS. SHA `f33e598a…` принят push/PR CI `32098804217`/`32098806708`; artifact `9311012974`, digest `sha256:f0843edc…77e81`                                                                                                                                                                                                                                                                                                                                                   | Restored-copy SMTP `SENT`, reissue/accept canary, production-like rehearsal и Gate 1MT/2                                                                                                                                                                  |
...
1. Clean exact SHA и воспроизводимый CI artifact приняты. Исполнимый read-only
   ...
   independent checksum и isolated target, получить live `READY`, затем
   выполнить уже реализованный exact role controller и direct network acceptance.
   Synthetic ACL/lost-response/rollback и HBA/TLS/SCRAM matrices уже зелёные;
   следующий engineering шаг — dedicated pool/live API call и session drain.
