...
test("CURRENT187-I candidate is hash-pinned, noncanonical, and deny-only", async () => {
  ...
  assert.deepEqual(metadata.predecessor, {
    requiredContract: "CURRENT187_H_SIGNED_SEMANTIC_ALLOWLIST_EVALUATOR",
    resolved: false,
  });
  for (const flag of [
    "authorization",
    ...
    "canConsumeProductionApproval",
    "canRevokeProductionApproval",
    "canSend",
...
test("install is bounded to one confirmed disposable database and exact unprivileged roles", async () => {
  ...
  for (const role of ["consumer", "revoker", "runtime"]) {
    assert.match(sql, new RegExp(`current187i_${role}_role_name`, "u"));
    assert.match(sql, new RegExp(`current187i_${role}_role_oid`, "u"));
  }
...
test("append-only tables use forced RLS and owner-only policies", async () => {
  ...
  assert.match(sql, /"approvalDigest" CHAR\(64\).*NOT NULL UNIQUE/u);
  assert.match(
    sql,
    /CONSTRAINT "Current187SemanticApprovalRevocationLedger_scope_unique"\s+UNIQUE \("scope", "scopeDigest"\)/u,
  );
  ...
  assert.match(sql, /RAISE EXCEPTION 'CURRENT187-I ledger is append-only'/u);
});

test("consume and revoke share one deadlock-free root-to-evaluation lock order", async () => {
  const { sql } = await sources();
  ...
  const sharedOrder = [
    "current187i:root:",
    "current187i:approval:",
    "current187i:document:",
    "current187i:evaluation:",
  ];
  ...
  assert.ok(
    consume.indexOf("current187i:evaluation:") <
      consume.indexOf("current187i:operation:"),
  );
  assert.ok(
    consume.indexOf("current187i:operation:") <
      consume.indexOf("current187i:nonce:"),
  );
  assert.doesNotMatch(sql, /pg_advisory_lock\(/u);
});

test("fresh time and revocation checks occur after the complete shared lock wait", async () => {
  const { sql } = await sources();
  const consumeStart = sql.indexOf(
    'CREATE OR REPLACE FUNCTION public."current187_semantic_approval_consume_v1"',
  );
  ...
  const consume = sql.slice(consumeStart, revokeStart);
  const finalLock = consume.indexOf("current187i:nonce:");
  const exactReplay = consume.indexOf("FOR UPDATE", finalLock);
  const freshClock = consume.indexOf(
    "v_now := pg_catalog.clock_timestamp();",
    finalLock,
  );
  const expiry = consume.indexOf("IF v_authority_issued_at", freshClock);
  const revocationRead = consume.indexOf(
    'FROM public."Current187SemanticApprovalRevocationLedger"',
    expiry,
  );
  ...
  const revoke = sql.slice(revokeStart);
  const revokeFinalLock = revoke.indexOf("current187i:evaluation:");
...
test("RPCs pin exact command and receipt domains with bounded secret-free JSON", async () => {
  ...
  assert.match(sql, /approvalDigest.*documentDigest.*evaluationDigest/su);
