...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  Новый exact-SHA artifact, SMTP, Gate 1MT/2 и cutover ещё обязательны, поэтому
...
| `FOUNDER-OPERATOR-BETA-GO`              |        P0 | `CURRENT185 DELIVERY/ACCEPT EXACT-SHA CI ACCEPTED / PRODUCTION NO-GO`             | Ручное создание tester account запрещено. GO/revoke, atomic v2 activation и dedicated database pool/live least-privilege assertion не требуют USB/offline key; default mode `DISABLED`. На canonical CURRENT185 clean PG16 deployment, restricted-role activation/reissue, worker `SENT`, preview и production `AuthService.acceptInvite` приняты: один OWNER/NETWORK, tenant ONBOARDING, claim USER, ciphertext очищен. Exact SHA `14193e51…`, push/PR CI `32105326187`/`32105331954` — `4/4 SUCCESS`; artifact `9313186108`, digest `sha256:6d2747e7…2fe09`. Production и текущие четыре клуба не изменены; PUBLIC/обычные roles не имеют RPC EXECUTE, production activation/worker roles, secrets и grants не создавались                                                                                                                                                                                                                                                                                                                                                                                                        | Restored-copy apply/replay/rollback + worker/SENT/accept; production trusted-SMTP canary; Gate 1MT/2; затем PREPARE→one-tenant ACTIVE canary и mailbox-bound OWNER invite                                                                                  |
| `FOUNDER-RESTORED-COPY-PREFLIGHT`       |        P0 | `V2 LOCAL / PRODUCTION BACKUP READY / CLEAN REHEARSAL PASS / NEW CI PENDING`      | Настоящий production backup `sha256:31ad21d5…b9b` восстановлен в isolated PG16.14. V2 отдельно связывает `153 applied / 4 rolled back / 0 unfinished`; clean production-history lane применил ровно 32 migration, repeat дал zero pending, итог `185/4/0`. Все 134 исходные business-table row counts и control aggregates совпали. Activation role прошла apply/attest/TLS1.3 HBA+SCRAM/rollback; final role count 0. Полный PII-free отчёт: `founder-production-restored-copy-rehearsal-2026-08-18.md`. Production application/DB/services и Tenant A не изменялись                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Собрать controller/V2 bridge в новый exact SHA и CI artifact; повторить artifact-bound admission; затем SMTP canary, Gate 1MT/2 и controlled production cutover                                                                                            |
| `FOUNDER-PRODUCTION-HISTORY-LANE`       |        P0 | `LOCAL CLEAN RESTORED-COPY PASS / CI ARTIFACT PENDING`                            | Новый `plan/apply/check` controller exact-bound к source migration/rollback manifests, четырём stale empty digest runs и полному 185-file tree. CURRENT179 SHA `f4437ebb…91a`, CURRENT185 SHA `2979599d…674`, tree `873a3af7…062`, plan `c5dbdf31…37a`; final preterminal `7a0bb533…bcc`, worker `d2025dca…465`. Runtime принимает только canonical clean digest и этот reviewed production-history digest; произвольный третий digest fail-closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Новый clean SHA, CI, downloadable artifact и повторный artifact-bound check; не применять production до отдельного cutover GO                                                                                                                              |
...
| `FOUNDER-PILOT-MAIL-TENANT-ENROLLMENT`  |        P0 | `EXACT-SHA CI ACCEPTED / CLEAN+SEQUENTIAL PG PASS / PROD NO-GO`                   | Новый operator CLI выполняет только `plan/apply/check/disable` для одного exact `PILOT` tenant. CURRENT185/head/count, activation release/environment, OWNER/NETWORK invite, worker role/OID и пять RPC, provider digest и fixed policy проверяются под tenant advisory lock в `SERIALIZABLE`; active claim/drift/cross-tenant substitution блокируются, lost commit response reconciles только по полному state equality, disable монотонно повышает revision. Unit `7/7`, full disposable PG `1/1`; `55748217…` устранил TLS residue PgBouncer fixture и добавил post-cleanup `ssl=off`. Push/PR CI `32115334678`/`32115340009` — `4/4 SUCCESS`, focused PG `32115339918` — `SUCCESS`; artifact `9316782148`, GitHub digest `sha256:a2b0ea21…3f0706`, downloaded SHA-256 `d360e5b1…a7849167`, `14=8+6` scripts. Production и Tenant A не менялись                                                                                                                                                                                                                                                                                 | Exact downloaded artifact на immutable restored copy; production worker role/TLS/SMTP canary; Gate 1MT/2 и controlled Tenant B enrollment                                                                                                                  |
...
1. Принять exact SHA и воспроизводимый CI artifact нового canonical CURRENT185
   delivery/accept successor. Его clean deployment, worker `SENT` и owner
   accept уже зелёные локально. CURRENT198–202 и USB не входят в critical path
   и остаются deny-only post-beta hardening.
2. Завершить Gate 1MT store-scope/HTTP/BFF/browser adoption по всему
   согласованному scope.
3. На immutable production backup и isolated target выполнить уже
   реализованные preflight, role/network admission, CURRENT184→185
   apply/repeat, worker enrollment, `SENT`, accept и restore/rollback; принять
   readiness, session drain, monitoring и отдельный `PRODUCTION DEPLOY GO`.
