...
| `CURRENT188-LANGAME`                 |        P0 | `IMPLEMENTED NONCANONICAL / THROUGH INITIAL-SYNC PREFLIGHT + WEB BFF DORMANT`      | Preview/activate/status/reconcile default-off и production-denied; reconcile принят exact SHA `4e2c9b29…`, CI `31671722614` `3/3 SUCCESS`, artifact `sha256:7bea44f8…003b1`; API `201/201`, tenant `986/986`, full `3038 + 2 todo`, Web `17/17`. Новый local provider-read preflight повторно связывает exact receipt/activation/Store/source/credential/audit и NETWORK authority до и после сети, делает только 3 bounded `GET` (`clubs/products/goods` выбранного клуба), запрещён в production, не создаёт Product/Inventory/SyncJob и оставляет actual import unavailable; targeted API `219/219`, Web `19/19`. Legacy external sync deny принят SHA `9d66276a…`; остальные provider clubs не создаются автоматически | Принять exact-SHA CI preflight evidence; реализовать persisted approval и selected-Store initial import с идемпотентностью/reconcile; canonical consolidation, execute-only runtime role/grants, expirer и production authorization; atomic Route/UI cutover и browser A/B |
