...
- Edge 1337 не должен хардкодить старую Mini App клавиатуру: он отправляет `replyMarkup` из основного API как opaque JSON.
...
Основная VDS не отправляет Telegram replies напрямую: `GUEST_GAME_TELEGRAM_WEBHOOK_REPLY_ENABLED=false`.
