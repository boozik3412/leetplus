# Historical instruction snapshot: 15–17 September 2026

Archived on 24 September 2026 from root AGENTS.md in the selected worktree.
Read only to trace earlier source decisions, deployments or acceptance receipts.
The quotation below preserves the previous text, including its old browser status,
test counts, fixture ports and release identities. None of those observations proves
current state or authorizes an operation. Current root AGENTS.md and applicable
canonical contracts govern work. Do not replay old commands or assume old services exist.
