...
-- CURRENT187-J5-R3 is NONCANONICAL, DENY-ONLY and restricted to an
-- explicitly confirmed disposable CI database. It is not a Prisma migration
-- and conveys no apply, tenant, tester, provider or production authority.

DO $current187_j5l_prerequisite$
DECLARE
  ...
  runtime_oid OID;
BEGIN
  IF pg_catalog.current_database() !~ '^lp_c187j5l_[0-9a-f]{12}_ci$'
     OR pg_catalog.current_setting(
       'leetplus.current187j5l_confirmation',
       true
     ) IS DISTINCT FROM
       'rehearse-current187j5l-connection-probe-ledger-loopback-ci-only'
  THEN
    RAISE EXCEPTION
      'CURRENT187-J5-R3 is restricted to an explicitly confirmed disposable CI database'
      USING ERRCODE = '55000';
  END IF;

  consumer_name := pg_catalog.current_setting(
    'leetplus.current187j5l_consumer_role_name',
    true
  );
  revoker_name := pg_catalog.current_setting(
    'leetplus.current187j5l_revoker_role_name',
    true
  );
  runtime_name := pg_catalog.current_setting(
    'leetplus.current187j5l_runtime_role_name',
    true
  );
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 duty-role names are invalid'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF actor_oid IS DISTINCT FROM database_owner_oid
     ...
     OR consumer_oid::TEXT IS DISTINCT FROM pg_catalog.current_setting(
       'leetplus.current187j5l_consumer_role_oid',
       true
     )
     OR revoker_oid::TEXT IS DISTINCT FROM pg_catalog.current_setting(
       'leetplus.current187j5l_revoker_role_oid',
       true
     )
     OR runtime_oid::TEXT IS DISTINCT FROM pg_catalog.current_setting(
       'leetplus.current187j5l_runtime_role_oid',
       true
     )
  THEN
    RAISE EXCEPTION
      'CURRENT187-J5-R3 requires the database owner and exact unprivileged duty-role OIDs'
      USING ERRCODE = '55000';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 duty roles must have no memberships'
      USING ERRCODE = '55000';
  END IF;
END
$current187_j5l_prerequisite$;

CREATE TABLE public."Current187ConnectionProbeLedgerPolicy" (
  ...
  CONSTRAINT "Current187ConnectionProbeLedgerPolicy_singleton_check"
    CHECK ("singletonId" = 1),
  CONSTRAINT "Current187ConnectionProbeLedgerPolicy_contract_check"
    CHECK ("contract" = 'CURRENT187_CONNECTION_PROBE_LEDGER_SYNTHETIC_CI_V1'),
  CONSTRAINT "Current187ConnectionProbeLedgerPolicy_role_names_check"
    CHECK (
      "consumerRoleName" <> "revokerRoleName"
      AND "consumerRoleName" <> "runtimeRoleName"
      AND "revokerRoleName" <> "runtimeRoleName"
    ),
  CONSTRAINT "Current187ConnectionProbeLedgerPolicy_role_oids_check"
    CHECK (
...
INSERT INTO public."Current187ConnectionProbeLedgerPolicy" (
  "singletonId", "contract", "consumerRoleName", "consumerRoleOid",
  "revokerRoleName", "revokerRoleOid", "runtimeRoleName", "runtimeRoleOid"
)
SELECT
  1,
...
CREATE TABLE public."Current187ConnectionProbeConsumptionLedger" (
  "operationId" UUID PRIMARY KEY,
  "nonce" CHAR(64) COLLATE "C" NOT NULL UNIQUE,
  "envelopeDigest" CHAR(64) COLLATE "C" NOT NULL UNIQUE,
  "connectionProbeMatrixDigest" CHAR(64) COLLATE "C" NOT NULL,
  "verificationReceiptDigest" CHAR(64) COLLATE "C" NOT NULL,
  ...
  CONSTRAINT "Current187ConnectionProbeConsumptionLedger_digest_check"
    CHECK (
      "nonce" ~ '^[0-9a-f]{64}$' AND "nonce" <> repeat('0', 64)
      AND "envelopeDigest" ~ '^[0-9a-f]{64}$'
      AND "envelopeDigest" <> repeat('0', 64)
      AND "connectionProbeMatrixDigest" ~ '^[0-9a-f]{64}$'
      AND "connectionProbeMatrixDigest" <> repeat('0', 64)
      AND "verificationReceiptDigest" ~ '^[0-9a-f]{64}$'
      AND "verificationReceiptDigest" <> repeat('0', 64)
  ...
  CONSTRAINT "Current187ConnectionProbeConsumptionLedger_binding_check"
    CHECK (
  ...
  CONSTRAINT "Current187ConnectionProbeConsumptionLedger_secret_free_check"
    CHECK (
      octet_length("commandCanonicalJson") <= 32768
...
CREATE INDEX "Current187ConnectionProbeConsumptionLedger_matrix_idx"
  ON public."Current187ConnectionProbeConsumptionLedger" (
    "connectionProbeMatrixDigest"
  );
CREATE INDEX "Current187ConnectionProbeConsumptionLedger_root_idx"
  ON public."Current187ConnectionProbeConsumptionLedger" (
    "publicKeyFingerprint"
  );

CREATE TABLE public."Current187ConnectionProbeRevocationLedger" (
  "eventId" UUID PRIMARY KEY,
  "scope" VARCHAR(16) COLLATE "C" NOT NULL,
  "scopeDigest" CHAR(64) COLLATE "C" NOT NULL,
  "envelopeDigest" CHAR(64) COLLATE "C" NOT NULL,
  "connectionProbeMatrixDigest" CHAR(64) COLLATE "C" NOT NULL,
  "publicKeyFingerprint" CHAR(64) COLLATE "C" NOT NULL,
  ...
  "receiptCanonicalJson" TEXT COLLATE "C" NOT NULL,
  "receiptDigest" CHAR(64) COLLATE "C" NOT NULL UNIQUE,
  CONSTRAINT "Current187ConnectionProbeRevocationLedger_scope_unique"
    UNIQUE ("scope", "scopeDigest"),
  CONSTRAINT "Current187ConnectionProbeRevocationLedger_scope_check"
    CHECK (
      "scope" IN ('ENVELOPE', 'MATRIX', 'ROOT')
      AND (
        ("scope" = 'ENVELOPE' AND "scopeDigest" = "envelopeDigest")
        OR (
          "scope" = 'MATRIX'
          AND "scopeDigest" = "connectionProbeMatrixDigest"
        )
        OR ("scope" = 'ROOT' AND "scopeDigest" = "publicKeyFingerprint")
      )
    ),
  CONSTRAINT "Current187ConnectionProbeRevocationLedger_digest_check"
    CHECK (
  ...
  CONSTRAINT "Current187ConnectionProbeRevocationLedger_secret_free_check"
    CHECK (
      octet_length("commandCanonicalJson") <= 32768
      AND octet_length("receiptCanonicalJson") <= 16384
      AND "commandCanonicalJson" !~* '(@|BEGIN [A-Z ]+KEY|https?://|password|privateKey|secret|accessToken|refreshToken|providerMessageId)'
...
ALTER TABLE public."Current187ConnectionProbeLedgerPolicy"
  ENABLE ROW LEVEL SECURITY;
ALTER TABLE public."Current187ConnectionProbeLedgerPolicy"
  FORCE ROW LEVEL SECURITY;
ALTER TABLE public."Current187ConnectionProbeConsumptionLedger"
  ENABLE ROW LEVEL SECURITY;
ALTER TABLE public."Current187ConnectionProbeConsumptionLedger"
  FORCE ROW LEVEL SECURITY;
ALTER TABLE public."Current187ConnectionProbeRevocationLedger"
  ENABLE ROW LEVEL SECURITY;
ALTER TABLE public."Current187ConnectionProbeRevocationLedger"
  FORCE ROW LEVEL SECURITY;

DO $current187_j5l_owner_policies$
DECLARE
  owner_name TEXT := current_user;
BEGIN
  EXECUTE pg_catalog.format(
    'CREATE POLICY %I ON public.%I AS PERMISSIVE FOR ALL TO %I USING (true) WITH CHECK (true)',
    'Current187ConnectionProbeLedgerPolicy_owner_only',
    'Current187ConnectionProbeLedgerPolicy',
    owner_name
  );
  EXECUTE pg_catalog.format(
    'CREATE POLICY %I ON public.%I AS PERMISSIVE FOR ALL TO %I USING (true) WITH CHECK (true)',
    'Current187ConnectionProbeConsumptionLedger_owner_only',
    'Current187ConnectionProbeConsumptionLedger',
    owner_name
  );
  EXECUTE pg_catalog.format(
    'CREATE POLICY %I ON public.%I AS PERMISSIVE FOR ALL TO %I USING (true) WITH CHECK (true)',
    'Current187ConnectionProbeRevocationLedger_owner_only',
    'Current187ConnectionProbeRevocationLedger',
    owner_name
  );
END
$current187_j5l_owner_policies$;

CREATE OR REPLACE FUNCTION public."current187_connection_probe_ledger_reject_mutation_v1"()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = pg_catalog
AS $current187_j5l_reject_mutation$
BEGIN
  RAISE EXCEPTION 'CURRENT187-J5-R3 ledger is append-only'
    USING ERRCODE = '55000';
END
$current187_j5l_reject_mutation$;

CREATE TRIGGER "Current187ConnectionProbeLedgerPolicy_no_update_delete"
BEFORE UPDATE OR DELETE ON public."Current187ConnectionProbeLedgerPolicy"
FOR EACH ROW EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
CREATE TRIGGER "Current187ConnectionProbeLedgerPolicy_no_truncate"
BEFORE TRUNCATE ON public."Current187ConnectionProbeLedgerPolicy"
FOR EACH STATEMENT EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
CREATE TRIGGER "Current187ConnectionProbeConsumptionLedger_no_update_delete"
BEFORE UPDATE OR DELETE ON public."Current187ConnectionProbeConsumptionLedger"
FOR EACH ROW EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
CREATE TRIGGER "Current187ConnectionProbeConsumptionLedger_no_truncate"
BEFORE TRUNCATE ON public."Current187ConnectionProbeConsumptionLedger"
FOR EACH STATEMENT EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
CREATE TRIGGER "Current187ConnectionProbeRevocationLedger_no_update_delete"
BEFORE UPDATE OR DELETE ON public."Current187ConnectionProbeRevocationLedger"
FOR EACH ROW EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
CREATE TRIGGER "Current187ConnectionProbeRevocationLedger_no_truncate"
BEFORE TRUNCATE ON public."Current187ConnectionProbeRevocationLedger"
FOR EACH STATEMENT EXECUTE FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"();
...
DECLARE
  ...
  v_existing public."Current187ConnectionProbeConsumptionLedger"%ROWTYPE;
...
BEGIN
  ...
  FROM public."Current187ConnectionProbeLedgerPolicy"
  WHERE "singletonId" = 1;
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consume caller is not the exact duty role'
      USING ERRCODE = '42501';
  END IF;

  IF p_command_canonical_json IS NULL
     OR p_command_digest IS NULL
     OR octet_length(p_command_canonical_json) > 32768
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consumption command digest is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consumption command JSON is invalid'
      USING ERRCODE = '22023';
  END;
  ...
  IF pg_catalog.jsonb_typeof(v_document) IS DISTINCT FROM 'object'
     ...
     OR (v_document ->> 'operationId') !~
       '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consumption command contract is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  v_matrix_digest := v_document ->> 'connectionProbeMatrixDigest';
  v_verification_receipt_digest :=
    v_document ->> 'verificationReceiptDigest';
  ...
  IF EXISTS (
    ...
    WHERE digest_value !~ '^[0-9a-f]{64}$'
     ...
     OR (v_document ->> 'validUntil') !~
       '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$'
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consumption binding is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  v_expected_command_canonical_json :=
    ...
    || ',"connectionProbeMatrixDigest":'
    || pg_catalog.to_jsonb(v_matrix_digest)::TEXT
    ...
    || ',"environment":"ci"'
    || ',"issuedAt":' || pg_catalog.to_jsonb(v_document ->> 'issuedAt')::TEXT
    || ',"kind":"CURRENT187_CONNECTION_PROBE_CONSUMPTION_COMMAND"'
    || ',"nonce":' || pg_catalog.to_jsonb(v_nonce)::TEXT
    || ',"operationId":'
    || pg_catalog.to_jsonb(v_document ->> 'operationId')::TEXT
    || ',"payloadDigest":' || pg_catalog.to_jsonb(v_payload_digest)::TEXT
    || ',"profile":"CURRENT187_CONNECTION_PROBE_LEDGER_SYNTHETIC_CI_V1"'
    || ',"publicKeyFingerprint":'
    || pg_catalog.to_jsonb(v_public_key_fingerprint)::TEXT
    || ',"releaseSha":' || pg_catalog.to_jsonb(v_release_sha)::TEXT
    || ',"schemaVersion":1'
    || ',"signingKeyId":' || pg_catalog.to_jsonb(v_signing_key_id)::TEXT
    || ',"slice":"CURRENT187_J5_R3_PERSISTED_CONNECTION_PROBE_CONSUMPTION_REVOCATION_LEDGER"'
    || ',"trustDomain":"LEETPLUS_CURRENT187_INDEPENDENT_CONNECTION_PROBE_AUTHORITY_V1"'
    || ',"validUntil":'
    || pg_catalog.to_jsonb(v_document ->> 'validUntil')::TEXT
    || ',"verificationReceiptDigest":'
    || pg_catalog.to_jsonb(v_verification_receipt_digest)::TEXT
    || '}';

  IF p_command_canonical_json IS DISTINCT FROM
     v_expected_command_canonical_json
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 consumption command is not canonical'
      USING ERRCODE = '22023';
  END IF;

  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'current187j5l:root:' || v_public_key_fingerprint,
      0
    )
  );
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'current187j5l:envelope:' || v_envelope_digest,
      0
    )
  );
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'current187j5l:matrix:' || v_matrix_digest,
      0
    )
  );
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'current187j5l:operation:' || v_operation_id::TEXT,
      0
    )
  );
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('current187j5l:nonce:' || v_nonce, 0)
  );

  SELECT pg_catalog.count(*)::INTEGER
  INTO v_existing_count
  FROM public."Current187ConnectionProbeConsumptionLedger" AS consumption
  ...
  IF v_existing_count > 0 THEN
    IF v_existing_count IS DISTINCT FROM 1 THEN
      RAISE EXCEPTION 'CURRENT187-J5-R3 consumption identity conflict'
        USING ERRCODE = '23505';
    END IF;
  ...
  IF v_issued_at > v_now
     OR v_valid_until <= v_now
     OR v_valid_until <= v_issued_at
     OR v_valid_until - v_issued_at > INTERVAL '5 minutes'
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 connection probe is expired'
      USING ERRCODE = '55000';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM public."Current187ConnectionProbeRevocationLedger" AS revocation
    WHERE (revocation."scope" = 'ENVELOPE'
        AND revocation."scopeDigest" = v_envelope_digest)
       OR (revocation."scope" = 'MATRIX'
        AND revocation."scopeDigest" = v_matrix_digest)
       OR (revocation."scope" = 'ROOT'
        AND revocation."scopeDigest" = v_public_key_fingerprint)
  )
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 connection probe is revoked'
      USING ERRCODE = '55000';
  END IF;
  ...
  INSERT INTO public."Current187ConnectionProbeConsumptionLedger" (
    "operationId", "nonce", "envelopeDigest",
    "connectionProbeMatrixDigest", "verificationReceiptDigest",
    "payloadDigest", "publicKeyFingerprint", "signingKeyId", "releaseSha",
    "clusterIdentityDigest", "databaseUniverseDigest", "issuedAt",
    "validUntil", "commandCanonicalJson", "commandDigest", "consumedAt",
    "consumedTransactionId", "receiptCanonicalJson", "receiptDigest"
  ) VALUES (
    v_operation_id, v_nonce, v_envelope_digest, v_matrix_digest,
    v_verification_receipt_digest, v_payload_digest,
    v_public_key_fingerprint, v_signing_key_id, v_release_sha,
    v_cluster_identity_digest, v_database_universe_digest, v_issued_at,
    v_valid_until, p_command_canonical_json, p_command_digest, v_consumed_at,
    v_transaction_id, v_receipt_text, v_receipt_digest
  );

  RETURN v_receipt_text;
END
$current187_j5l_consume$;
...
DECLARE
  ...
  v_now TIMESTAMPTZ;
  v_existing_count INTEGER;
  v_existing public."Current187ConnectionProbeRevocationLedger"%ROWTYPE;
  v_transaction_id TEXT;
  v_receipt_base TEXT;
  v_receipt_digest TEXT;
  v_receipt_text TEXT;
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revoke caller is not the exact duty role'
      USING ERRCODE = '42501';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation command digest is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  BEGIN
    v_document := p_command_canonical_json::JSONB;
  EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation command JSON is invalid'
      USING ERRCODE = '22023';
  END;
  ...
  IF pg_catalog.jsonb_typeof(v_document) IS DISTINCT FROM 'object'
     ...
     OR (v_document ->> 'eventId') !~
       '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation command contract is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF v_scope NOT IN ('ENVELOPE', 'MATRIX', 'ROOT')
     OR EXISTS (
       SELECT 1
       FROM pg_catalog.unnest(ARRAY[
         v_scope_digest,
     ...
     OR (v_scope = 'ENVELOPE'
       AND v_scope_digest IS DISTINCT FROM v_envelope_digest)
     OR (v_scope = 'MATRIX'
       AND v_scope_digest IS DISTINCT FROM v_matrix_digest)
     OR (v_scope = 'ROOT'
       AND v_scope_digest IS DISTINCT FROM v_public_key_fingerprint)
     OR (v_document ->> 'revokedAt') !~
       '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$'
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation binding is invalid'
      USING ERRCODE = '22023';
  END IF;
  ...
  v_expected_command_canonical_json :=
    ...
    || ',"envelopeDigest":' || pg_catalog.to_jsonb(v_envelope_digest)::TEXT
    || ',"environment":"ci"'
    || ',"eventId":' || pg_catalog.to_jsonb(v_document ->> 'eventId')::TEXT
    || ',"kind":"CURRENT187_CONNECTION_PROBE_REVOCATION_COMMAND"'
    || ',"profile":"CURRENT187_CONNECTION_PROBE_LEDGER_SYNTHETIC_CI_V1"'
    || ',"publicKeyFingerprint":'
    || pg_catalog.to_jsonb(v_public_key_fingerprint)::TEXT
    || ',"purpose":"CURRENT187_CONNECTION_PROBE_REVOCATION_V1"'
    ...
    || ',"slice":"CURRENT187_J5_R3_PERSISTED_CONNECTION_PROBE_CONSUMPTION_REVOCATION_LEDGER"'
    || ',"trustDomain":"LEETPLUS_CURRENT187_CONNECTION_PROBE_REVOCATION_AUTHORITY_V1"}';

  IF p_command_canonical_json IS DISTINCT FROM
     v_expected_command_canonical_json
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation command is not canonical'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF v_existing_count > 0 THEN
    IF v_existing_count IS DISTINCT FROM 1 THEN
      RAISE EXCEPTION 'CURRENT187-J5-R3 revocation identity conflict'
        USING ERRCODE = '23505';
    END IF;
    SELECT * INTO STRICT v_existing
    FROM public."Current187ConnectionProbeRevocationLedger" AS revocation
  ...
  THEN
    RAISE EXCEPTION 'CURRENT187-J5-R3 revocation time expired while waiting'
      USING ERRCODE = '55000';
  END IF;
  ...
  INSERT INTO public."Current187ConnectionProbeRevocationLedger" (
    "eventId", "scope", "scopeDigest", "envelopeDigest",
    "connectionProbeMatrixDigest", "publicKeyFingerprint", "reasonDigest",
    "actorDigest", "revokedAt", "commandCanonicalJson", "commandDigest",
    "revokedTransactionId", "receiptCanonicalJson", "receiptDigest"
  ) VALUES (
    v_event_id, v_scope, v_scope_digest, v_envelope_digest, v_matrix_digest,
    v_public_key_fingerprint, v_reason_digest, v_actor_digest, v_revoked_at,
  ...
  RETURN v_receipt_text;
END
$current187_j5l_revoke$;

REVOKE ALL ON TABLE
  public."Current187ConnectionProbeLedgerPolicy" FROM PUBLIC;
REVOKE ALL ON TABLE
  public."Current187ConnectionProbeConsumptionLedger" FROM PUBLIC;
REVOKE ALL ON TABLE
  public."Current187ConnectionProbeRevocationLedger" FROM PUBLIC;
REVOKE ALL ON FUNCTION
  public."current187_connection_probe_ledger_reject_mutation_v1"()
  FROM PUBLIC;
REVOKE ALL ON FUNCTION
  public."current187_connection_probe_consume_v1"(TEXT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION
  public."current187_connection_probe_revoke_v1"(TEXT, TEXT) FROM PUBLIC;

DO $current187_j5l_grants$
DECLARE
...
BEGIN
  ...
  EXECUTE pg_catalog.format(
    ...
    'current187_connection_probe_consume_v1',
    consumer_name
  );
  EXECUTE pg_catalog.format(
    'GRANT EXECUTE ON FUNCTION public.%I(text,text) TO %I',
    'current187_connection_probe_revoke_v1',
    revoker_name
  );
END
$current187_j5l_grants$;
