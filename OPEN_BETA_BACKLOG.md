...
CI `31397844858` на `3804792e…` (`3/3 SUCCESS`). CURRENT187-H локально добавил
...
`163/163`. До exact-SHA CI H имеет статус `PRE-GREEN / DENY-ONLY` и не является
deployment GO. Production и
...
| `CURRENT187-H`                       |        P0 | `IMPLEMENTED LOCALLY / DENY-ONLY / PRE-GREEN / NONCANONICAL / NOT DEPLOYABLE`     | fourth independent Ed25519 semantic-approval purpose; exact secret-free allowlist document binds cluster/universe/review/facts and bounded timeline; branded evaluator returns only `MATCHED_DENY_ONLY`/`DENIED`; F now requires exact matched H receipt; authority `13/13`, acquisition/risk/allowlist/policy `24/24`, DDL fence `11/11`, full disposable rehearsal `163/163`; every launch/effect flag remains false                                                                                                                                                                                                                                                                                                                                                                                                                                 | Persisted one-time consumption/revocation/expiry/replay; independent review и exact-SHA CI; production roots/GO; host/TLS/HBA/pooler/service-account/runtime attestation; canonical restored-copy rehearsal                                                |
...
1. Принять CURRENT187-H exact-SHA CI и добавить persisted one-time
   consumption/revocation/expiry/replay semantic approval; затем закрыть
