...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  прошли `1/1 + 1/1` с zero residue. Production roles/SMTP canary, Gate 1MT/2
  и cutover ещё обязательны, поэтому release decision остаётся `NO-GO`
...
1. Завершить Gate 1MT store-scope/HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy trusted TLS SMTP и protected
   enrollment/SENT/accept/disable уже приняты. CURRENT198–202 и USB не входят
   в critical path и остаются deny-only post-beta hardening.
