...
- Актуальный status snapshot:
  [07.08.2026](./docs/open-beta/open-beta-current-status-2026-08-07.md)
- Канонический AccessScope package:
  [docs/security/access-scope](./docs/security/access-scope/README.md)

Сводка launch readiness на 07.08.2026: локальный CURRENT180–190 rehearsal
принят (`163/163`, independent `P0=0/P1=0`), но внешний доступ остаётся
`NO-GO`. В основной исполнимой таблице `101` P0: `8` готовы, `33` в работе,
`60` запланированы; Gate 0, Gate 1, Gate 1MT, Gate 2A, Gate 2 и Gate 3 не
закрыты. Production и `Tenant A/A1..A4` не изменялись.
...
| `CURRENT187-C`                       |        P0 | `IMPLEMENTED / DENY-ONLY`                                                         | 24-surface read-only multi-DB acquisition `12/12`, включая signed D attach-path; disposable PG16 `1/1`; residue `0/0/0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | HBA/TLS/pooler probes и hostile concurrent topology matrix                                                                                                                                                                                                 |
...
| `WEB-BUILD-REPRODUCIBLE`             |        P0 | `IMPLEMENTED LOCALLY`                                                             | build-time Google Fonts dependency удалена; системный local font stack закреплён `1/1`; focused lint/typecheck зелёные; Next.js production build с CI env компилирует и генерирует все `205` страниц без сетевой загрузки шрифтов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Повторить в clean CI artifact job и включить exact release SHA/provenance                                                                                                                                                                                  |
...
| `CURRENT180-190-REHEARSAL-ADMISSION` |        P0 | `LOCAL DISPOSABLE REHEARSAL ACCEPTED / PRODUCTION DENIED`                         | Planning `33/33`, SQL fingerprint `26/26`, coordinator `6/6`, materializer `24/24`, signed journal `24/24`, runtime `27/27`, runner/janitor `14/14`; единый последовательный gate — `163` test executions, `0` failures; final independent latest-byte audit `P0=0/P1=0`. Public-only restart verifier, descriptor-bounded reads, one-shot schema/tree/inode receipt, exact advisory-lock identity, cluster role/membership fingerprint, generic prior-run ownership-marker admission, signed source-zero-diff recovery gate и bounded lost-response reconciliation приняты. Два финальных PG16.13 run на CURRENT179 source завершились `ZERO_DIFF_ZERO_RESIDUE`: tokens `05c599…`/`c5a0bc…`, receipts `49f22f…`/`fd142b…`, общий source fingerprint `03e04e…`; source остался `179`, target/artifact/journal residue `0/0/0`. Production, `Tenant A/A1..A4` и tester не менялись | Зафиксировать exact commit SHA/artifact; canonical promotion и runtime roles/grants/attestation; signed restored-copy production-like apply/repeat/rollback/zero-diff. Это не production authority и не разрешение tester invite                           |
...
1. Зафиксировать уже принятый rehearsal на exact commit SHA и воспроизводимый
   CI artifact; planning/assembler receipt не считать DB authority.
2. Закрыть CURRENT187 infrastructure admission/provider recovery, exact runtime
   roles/grants/attestation и head-pinned application readiness.
3. Завершить Gate 1MT store-scope/HTTP/BFF/browser adoption по всему
   согласованному scope.
4. Выполнить canonical CURRENT180–190 promotion, signed production-like
   restored-copy apply/repeat/rollback/zero-diff и получить отдельный production
   root/deploy GO.
5. Controlled canary, затем Gate 2 на `Tenant A/A1..A4` и минимум семь суток
   stable internal alpha.
6. Отдельный `SHARED BETA GO`, затем штатное создание `Tenant B/Store B1` и
