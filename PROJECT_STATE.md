...
backfill `2026-08-27`–`2026-09-04` сохраняет `36/36 SUCCESS`. Stable timer
прошёл canonical child environment и stable-env digest, но финальный verifier
корректно откатил включение: systemd 255 сериализовал два exact `DropInPaths`
через один пробел, а избыточный pattern ожидал двойной разделитель. Timer снова
`inactive/disabled`, temporary permit удалён, оба 90-fence восстановлены,
public/corporate runtime и bonus-ledger timer остаются healthy. Текущий source
repair использует единственную строгую sorted exact-set проверку двух путей и
продолжает отклонять любой третий drop-in. До нового exact-main Fast/Full,
control install, повторного canary и timer check это не production fix.
...
- Current authoritative guide: `docs/deployment/guest-gamification-live-rewards.md`. It describes the live event pipeline, the 30-day claim wallet, UI refresh and production diagnostics.
...
- Границу участия профиля задаёт только первый доверенный `APP_OPEN` гостевого портала. Факты, события, rewards и entitlements до `GuestGameProfile.gameActivatedAt` не участвуют в прогрессе и не попадают в кошелёк. При первой активации знаковая XP-проекция `preActivationXpExcluded` один раз исключает автоматический XP до границы, сохраняет ручной и не связанный с событиями XP, а затем учитывает поздно восстановленные pre-activation postings без повторного списания.
- Выполнение обычного условия после активации только квалифицирует ценность в `rewardWallet`: mission, Battle Pass, check-in, event reward и event XP не начисляются, не ставятся на внешнюю доставку и не раскрывают код до явного claim гостя. Если одно событие даёт обычную награду и XP, кошелёк хранит два отдельных item с независимым идемпотентным lifecycle.
- Новый wallet item живёт до `qualifiedAt + 30 суток`. Непринятый `PENDING` на границе срока истекает и удаляется; старый reward переводится в `EXPIRED`, ожидающий claim effect — в `CANCELED`. Если claim принят строго до deadline, `PROCESSING` или `FAILED` сохраняется после 30 суток до подтверждённого результата или операционной сверки. Уже доставленные до запуска кошелька rewards и XP никогда не backfill-ятся как ожидающие получения.
- `POST /guest-portal/session/reward-wallet/items/:walletItemId/claim` действительно запускает получение обычной награды или атомарно применяет отложенный XP. `POST /guest-portal/session/reward-wallet/claim-all` делает то же только для обычных `PENDING/FAILED` item и никогда не открывает лутбокс, не расходует entitlement и не выбирает случайный приз. Закрытие completion-модалки — отдельный read-only ACK и не считается claim.
- Публичный контракт wallet item использует состояния `PENDING`, `PROCESSING`, `FAILED`, `OPENING` и действия `CLAIM_REWARD`, `OPEN_LOOT_BOX` либо `null`. История показывает канонические состояния `WAITING_CLAIM` и `DELIVERY_PROCESSING`, поэтому завершённое условие не выглядит уже выданной наградой.
- Код или claim payload обычной награды скрыт до принятого claim. Для бонусной награды scheduler допускает доставку только после `deliveryRequestedAt < claimExpiresAt`; неоднозначный результат Langame после отправленного запроса переводится в `RECONCILIATION_REQUIRED` без автоматического или гостевого retry, чтобы исключить двойное начисление.
- Неоткрытый entitlement открывается через exact-маршрут `POST /guest-portal/session/reward-wallet/items/:walletItemId/open`. Кошелёк общий для профиля внутри tenant; маршрут использует сохранённый исходный клуб entitlement, включая неактивный, и не переключает текущий клуб или session cookie гостя. Совместимый catalog-маршрут `/session/loot-boxes/:lootBoxId/open` использует тот же live entitlement и не может обойти 30-дневный срок старым unlock event; безопасный legacy-backfill допускается только для события внутри текущего 30-дневного окна и сначала материализует entitlement вместе с wallet item.
- При exact open wallet item и entitlement сначала атомарно резервируются состоянием `OPENING`; этот authoritative lock блокирует параллельное открытие и опасные административные изменения до финализации или контролируемого recovery. Существующий, но позже приостановленный/архивированный или перенесённый в другую область клубов шаблон не отменяет уже выданное точное право и не запускает повторную проверку расписания/лимитов. Удалённый шаблон по-прежнему обрабатывается fail-closed и требует операционной сверки; подмена правила запрещена.
- Exact-open событие помечается серверной квалификацией `PREQUALIFIED_LOOT_BOX_ENTITLEMENT`, поэтому сохранённый `HOURLY`/`regular_session` input не ошибочно попадает в generic-session remediation как новый session fact. Обычные LIVE/Ledger session-события этой квалификации не получают и сохраняют fail-closed проверку источника.
...
- The API-side `GuestGamificationPipelineSchedulerService` checks fresh snapshot facts every 15 seconds by default and records eligible Battle Pass/task results in the wallet. Ordinary rewards and XP remain behind the guest claim boundary; case rewards materialize their entitlement inline, and the guest's only manual action is opening the case. The scheduler is enabled in production when `SYNC_SERVICE_TOKEN` is present unless `GUEST_GAME_PIPELINE_SCHEDULER_ENABLED` explicitly overrides it.
...
- Langame exposes only one current/final `packet` value plus `expand` for a physical session; it does not expose tariff segments, the direction/time of a tariff change, or per-segment duration. Therefore an expanded session whose final marker looks hourly is kept `unknown` for typed rules and may satisfy only `ANY`. A previously proven hourly start remains valid only as the start fact; the combined duration of an expanded session remains generic unless segment history proves the hourly minutes. Continuing a package/subscription session without ending it does not create a second hourly start. The mission wizard, Battle Pass editor, standalone case editor, and guest requirement copy state this limitation explicitly. Ledger sync uses the `hourly-session-expand-v1` replay marker to refetch the historical active-rule window once per source while retaining stable v4 fact IDs and origin receipts.
...
- Product workspace first increment: `/products` is now a lightweight SKU hub with four summary metrics and direct operational routes; it no longer opens the full catalogue, product-movement report, or replenishment report. `GET /products/summary` returns only the required counters, while `GET /products/catalog` serves a server-filtered, sorted, paginated catalogue (default 50, maximum 100 SKU) and reads the latest `InventorySnapshot` per club/product instead of serializing the full stock history to the browser. The legacy `GET /products` contract remains temporarily for existing consumers that still require historical stock cost basis.
- Added multi-prize loot boxes in Guest Game Hub: `/guests/gamification` now edits loot box prize tables with reward type, amount, label, and drop chance per prize, plus an automatic probability normalization button that keeps existing proportions and brings the total to 100%. Backend stores the new contract in `probabilityRules.prizes`, keeps legacy `probabilityRules.items` as fallback, picks the selected prize by weight during dry-run/process-event, and creates rewards with the selected prize type/amount/label.
- Added the Guest Game visual editor in `/guests/gamification`: `Расширенные настройки` remains the default mode, while `Визуальный редактор` opens a safe club preview with clickable Battle Pass, lootbox, quest, promo, and check-in zones plus a right-side inspector. Changes are saved as `GuestGameVisualDraft` and applied only through publish; publish updates `GuestGameSeason.storeIds`, `GuestGameLootBox`, `GuestGameMission`, `GuestGamePromoCard`, and the managed `CHECK_IN` rule. Preview is demo/club-scoped, does not require a guest token, and does not expose raw phone, tokens, or Langame payload. Guest web home and Telegram Mini App now read published promo cards and check-in state from the common game-summary. Architecture notes are in `docs/gamification-visual-editor.md`.
- Adjusted the Telegram auth UX after production testing: `/game/auth` and `/play` now use one `Войти через Telegram` CTA with an informational confirmation and `Другой способ входа`, then open the Telegram deep link directly. After contact-share the main API returns a safe Telegram reply choice (`Вернуться на сайт LeetPlus`, `Открыть Mini App`, `Продолжить в боте`) for the 1337 edge to send as opaque reply markup, so Mini App is no longer the only forced continuation.
- Added editable store coordinates plus single and bulk address geocoding for Guest Game geosearch: `/stores` now saves `Store.latitude/longitude` through the regular tenant-scoped store API, validates coordinate ranges, accepts comma decimals, and can fill city/timezone/coordinates from protected Dadata-backed `GET /stores/address-geocode` and `POST /stores/address-geocode/missing` actions without exposing `DADATA_API_KEY` to the browser. `/stores` also accepts a persistent `Store.yandexMapsUrl`; the API parses full Yandex Maps links with `ll`, `pt`, `sll`, `whatshere[point]`, `rtext`, or coordinate `text` locally, and short/share Yandex Maps links are resolved server-side with a timeout only when the final URL stays inside supported Yandex Maps domains. The bulk action updates only active tenant stores with missing coordinates, up to 25 per run, and keeps existing manual coordinates intact. Production still needs the bulk action run for the 1337 clubs before nearby filtering is useful.
- Added a `/stores` operator readiness strip for Guest Game geosearch: it counts only active gamification-enabled clubs, shows how many have coordinates, highlights missing game-club coordinates, and links directly to `/game/clubs` and `/play` for post-bulk QA.
- Exposed Guest Game coordinate readiness in the public club directory: `GET /guest-portal/gamification/clubs` now returns `search.coordinates` with total/ready/missing/readyPercent for connected game clubs, and `/game/clubs` shows this as `Карта и поиск рядом` readiness so missing coordinates are visible before radius-search QA.
- Added a Guest Game pilot geosearch gate: pilot readiness now checks the 1337 target store coordinates and blocks the first-bonus runbook before dry-run/live when latitude or longitude is missing, pointing operators to `/stores` to fill coordinates before `/game/clubs` and `/play` nearby QA.
- Expanded the Telegram Mini App profile history for Guest Game: `/game/app` now renders the latest safe bonus ledger operations directly in the profile block, including status, amount, source, club, date, and balance after confirmation without raw phone, tokens, or Langame payload.
- Improved the public Guest Game club catalog for city search: `GET /guest-portal/gamification/clubs` now safely derives a display city from addresses such as `г. Ижевск, ...` when `Store.city` is still empty, so `/game/clubs` and `/play` city chips/search do not hide connected clubs before coordinates and normalized city fields are filled.
- Added the first live-mode SMS anti-abuse gate for Guest Game auth: before SMS.ru `/sms/send` or a generic SMS provider call, API counts saved SMS OTP challenges and blocks over-limit attempts with a safe 429. Defaults are 3 SMS per phone per 60 minutes, 30 SMS per club per 10 minutes, and 300 SMS per tenant per 24 hours, configurable through `GUEST_PORTAL_OTP_SMS_RATE_LIMIT_*`, without exposing raw phone, provider secrets, or counters to the browser. Guest Game Hub `OTP_SMS` readiness shows these limits as safe details and downgrades SMS to `PARTIAL` if any rate-limit/budget guard is disabled.
- Added an explicit SMS.ru live canary gate for Guest Game OTP: `/sms/send` remains ready in staged `GUEST_PORTAL_OTP_SMS_RU_TEST_MODE=true`, but live mode without `test=1` now requires `GUEST_PORTAL_OTP_SMS_RU_LIVE_CANARY_ENABLED=true`. Without that flag API returns safe `DELIVERY_BLOCKED` before calling SMS.ru, and Guest Game Hub/public verification readiness downgrades SMS to `PARTIAL` with required env and no `api_id` leak.
- Recorded the production SMS reserve state for Guest Game auth: `GET /guest-portal/gamification/clubs` shows `SMS_CODE` as `READY` only for staged/test-mode or explicit canary-ready env, staged OTP start against the demo club returns a safe provider-accepted SMS delivery status, and the remaining SMS work is live-mode/canary/budget QA rather than initial provider enablement.
- Aligned the public Guest Game verification plan with the live Telegram polling edge: `/guest-portal/gamification/clubs` now treats Telegram as ready when the main API has bot username, webhook/link secret, Mini App URL, and edge shared secret, without requiring API-side sender or a bot token on the main VDS.
- Added an explicit Guest Game Hub pilot readiness gate for the public QA path: `/guests/gamification` now checks `/game/auth -> Telegram contact-share/free call/SMS -> /game/clubs -> /play/game` through a dedicated `PUBLIC_GAME_QA` item, accepts Telegram or user-call readiness as alternatives to SMS/OTP, and keeps the legacy OTP item visible as a reserve channel instead of the only public-entry prerequisite.
- Updated the Guest Game Hub `guests/logs` production gate after a fresh audit on 2026-06-18: recent successful `GUEST_FOUNDATION` daily sync runs already include `includeGuestLogs=true` across the 3 Langame sources, but production still has `GuestLog=0` and `sourceCounts.guestLogs=0`. The pilot readiness now distinguishes a not-yet-run sync from a checked-empty endpoint, shows `проверено: 0` / `0 после sync`, and points operators to diagnostics or rule adjustment instead of blindly repeating the same sync.
...
- Added a safe VDS bot-consumer canary preview in Guest Game Hub: the card now shows the first `READY_FOR_BOT` candidates in the same `preparedAt`/`createdAt` order as `pullBotDeliveries`, bounded by `min(3, GUEST_GAME_BOT_CONSUMER_LIMIT)`, with only masked recipient/bot identity and reward/store metadata.
- Added a safe generic MAX provider boundary for Guest Game outbox delivery: the internal dispatcher can send MAX only when an explicit endpoint, bearer token, and `GUEST_GAME_MAX_DELIVERY_LIVE_CANARY_ENABLED=true` are configured; without the canary flag it blocks before the provider call and records a safe audit event. VDS `guest-game:bot-consumer` keeps its own scoped endpoint/token and `LIMIT=1` canary path. Neither path exposes raw phone, Langame payload, bot identity, provider payload, or tokens.
...
- Added Mini App operational readiness: Telegram contact-share replies now offer a `web_app.url` button after phone confirmation, Guest Game Hub exposes `TELEGRAM_MINI_APP` readiness without secrets, `.env.example` documents the Mini App URL/token/TTL knobs, and `docs/deployment/telegram-auth.md` covers safe launch, QA, and rollback.
...
- Added SMS.ru Callcheck as the current production provider for public gamification user-call auth: `/play` keeps the same `USER_CALL` flow, receives a temporary call number from SMS.ru, polls status by provider `check_id`, returns `freeCall=true`, shows a provider-neutral guest message that the call will be dropped immediately after verification, and Guest Game Hub readiness now supports either SMS.ru `api_id` or the previous manual callback provider without exposing secrets, raw phone, or Langame payload.
- Added SMS.ru `/sms/send` as the reserve OTP/SMS provider for public gamification auth: `GUEST_PORTAL_OTP_SMS_RU_API_ID` or the existing Callcheck `GUEST_PORTAL_USER_CALL_SMS_RU_API_ID` can back `SMS_CODE`, requests use `json=1` and optional `test=1`, and responses stay provider-neutral without exposing `api_id`, raw phone, or raw provider payload to the browser.
- Aligned Guest Game Hub `OTP_SMS` readiness with the SMS.ru `/sms/send` provider: the operator card now treats `GUEST_PORTAL_OTP_SMS_RU_API_ID` or reused `GUEST_PORTAL_USER_CALL_SMS_RU_API_ID` as configured, links to `docs/deployment/guest-auth-fallbacks.md`, shows only safe provider/test-mode/canary/fallback markers, and has regression coverage proving `api_id` values are not exposed.
...
- Added club-scoped one-time Langame auto-match for public gamification auth: `session/langame-match` limits live `guests/search` to the selected store source (`Store.integrationSourceId`/`externalDomain`), stores safe `GAME_PROFILE_LANGAME_AUTO_MATCH` events keyed by `profileId+storeId`, and returns cached results on later logins without another Langame call. `/game/auth` and `/play` show this as an automatic login step; Telegram contact-share runs it inside the webhook while phone-code/user-call/incoming-call flows trigger it after phone confirmation. Public registration still does not create a common `Guest` from live search and does not persist raw phone or Langame payload.
...
- Added one-time registration invite links in `/users`: owners/system admins can preconfigure a system/custom role plus whole-network or selected-club scope, copy the link, and the employee completes registration through the canonical `/register#invite=...` fragment link; preview and acceptance use fixed same-origin POST routes so the secret is not placed in a request URL.
