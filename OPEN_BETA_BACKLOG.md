...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  дополнительно расширена с `3/3` до `5/5`: real `CategoriesService` и
  `SuppliersService` доказали NETWORK-only list/create/update/merge/archive,
  cross-tenant deny и stale-scope deny; два последовательных current-head
  PostgreSQL run дали `5/5 + 5/5`, fixture residue `0|0|0|0`. Эти две новые
  проверки ещё требуется повторить на restored-copy clone. Production
  roles/SMTP canary, imports/reports/exports, остальные file parents,
