...
| `CURRENT187-J5-R3`                   |        P0 | `ENGINEERING ACCEPTED LOCALLY / LEDGER CONTRACT ONLY / NO DB EFFECT`              | Transferable exact consumption/revocation bundles bind verified J5 envelope+receipt to release, cluster/universe, matrix, operation/nonce, public root and timeline; revocation scopes `ENVELOPE/MATRIX/ROOT`; byte-exact persisted receipt attachment and replay/lost-response boundary; no DB/network/filesystem-write/process/env/deploy/tenant/provider capability; R3 `6/6`, combined J5 `32/32`, aggregate CURRENT187 `111/111`; all access/effect flags false                                                                                                                                                                                                                                                                                                                                                                                                                     | Noncanonical PostgreSQL tables/FORCE-RLS/execute-only RPC; hostile consume/revoke/expiry/race/lost-response fixture; independent review; canonical promotion; production root and F/deploy binding                                                         |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   принят `6/6`, combined J5 `32/32`, aggregate CURRENT187 `111/111`. Следующим
   шагом реализовать noncanonical PostgreSQL FORCE-RLS/execute-only ledger и
   hostile replay/race/lost-response acceptance; параллельный обязательный путь
