...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  Production roles/SMTP canary, file browser/unbind/race, job/Telegram
...
- `CHECKLIST_RUN`, `KNOWLEDGE_ARTICLE`, `SHIFT_REGULATION`,
  ...
  Restored-copy evidence: reader `4/4 + 4/4`, writer `5/5 + 5/5`, все `156`
  table counts zero-diff, database residue `0`;
- apply-backfill/reconciliation command, production-like inventory текущего
  tenant, manual quarantine resolution, reference removal/unbind,
  retention/delete/revoke,
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `32/32`: assortment `15/15`, team chat `4/4` с real HTTP SSE pre-header deny, CRM/users и staff attachments `5/5` со всеми семью reader parent kinds и native writer flow пяти content parents; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Exact `94db1fdd…` добавил atomic RecommendationState concurrency и bounded SSR fan-out; два restored-copy run дали `15/15 + 15/15`, report BFF `9/9`, production-build report/download/mutation browser journey, whole-schema `156`-table postflight и zero database residue. Exact `ccf81a28…`/`dfe5e0f8…` закрыл team-chat SSE BFF/API boundary. Exact `abb8a667…` закрыл fresh NETWORK/capability/exact-parent attachment reads; exact `fc07e959…` закрыл atomic native writers, same-parent replay и foreign-tenant rollback: unit `26/26 + 24/24`, PG `4/4 + 4/4` и `5/5 + 5/5`, zero table diff/residue. До готовности остаются file browser/unbind/races, job/Telegram/public-guest negative matrix пяти product modules и supporting `INTEGRATIONS`, controlled outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
18. Реализовать `BETA-MT-007..009`: полную shared PostgreSQL/API/BFF/browser/
    ...
    kinds, native writers пяти content parents и real HTTP SSE, и browser
    read/admission slice на `771bbd5f…` уже приняты; следующий срез —
    attachment browser/unbind/races и STORES policies, затем background
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   ...
   Остались attachment browser/unbind/races, jobs/Telegram/public guest
