...
- Added the first employee-facing sandbox preview for Staff Operations builders: `/staff/shift-regulations` and `/staff/checklist-templates` can now show a "view as administrator/employee" test flow with fake answers/evidence, required-field readiness, evidence readiness, score summary, and no real operational facts.
