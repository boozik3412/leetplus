...
PG/Redis/CURRENT191 data baseline. Bridge PR #243 also merged source; both
passed exact-main Full. Neither bridge nor B is accepted as installed or is an
external beta GO. A repair's installed acceptance, separate A→bridge and
bridge→B plans/GOs, and a full V1 active-app base release remain dependencies;
production application rollout and B pilot remain HOLD.
...
- Режим ускорения `CONTROLLED BETA-1` от 20.08.2026: первый внешний запуск
  ...
  isolated restore прошли последовательность `preflight → plan → apply → deploy → zero-diff deploy → check`
  как `PRODUCTION_HISTORY_REHEARSAL_VERIFIED`. Gate restored-copy
  history закрыт. Отдельная fresh-backup isolated lifecycle rehearsal затем
  приняла exact lifecycle `153 baseline → history 187 → role plan/apply/check/rollback/reconcile`:
  initial role plan корректно запретил отсутствующую на source
...
TENANT_OR_STORE_SYSTEM_IDENTITY`, а также закрепляет
  ...
  `3b1531d2a103304368d46781ebf76627eee1b2a6` принят GitHub Actions run
  `32267735326` как `4/4 SUCCESS`; evidence:
...
- Delivery dispatch и bot pull reward-delivery paths подключены к runtime
  identity foundation без открытия outbound: scheduled dispatch выбирает
  active `backgroundExecutionEnabled` store внутри tenant, требует exact
  `TENANT_STORE_SYSTEM + storeId` до `dispatchDeliveries`, запускает
  synthetic actor как `STORES/[runtimeStoreId]`, а bot pull возвращает
  deterministic empty result при missing store identity до outbox query/payload.
  `GuestGameDelivery` queries для `STORES` actor дополнительно фильтруются по
  `storeId IN allowedStoreIds`. Legacy protocol gate и `EXTERNAL_DENY` status
  остаются на месте, поэтому Telegram/MAX real-send для внешнего tenant не
  включён. Локально зелёные: focused `guest-gamification.service.spec.ts`
  (`431/431`), `test:ci:background-execution` (`16/16 suites`,
  `799/799 tests`), `test:ci:tenant-execution` (`18/18 suites`,
  `1004/1004 tests`), `lint:ci:tenant-execution`, API typecheck, Prettier
  check изменённых файлов и `git diff --check`. Exact-SHA
...
- Activity ledger sync background path подключён к runtime identity foundation:
  ...
  переведены; внешний доступ остаётся `NO-GO`.
- Ledger fallback background path подключён к runtime identity foundation:
  `GUEST_GAME_LEDGER_FALLBACK` теперь выбирает runtime store identity из
  активных `backgroundExecutionEnabled` stores tenant, требует exact
  `TENANT_STORE_SYSTEM + storeId` до ledger/activity reads, origin receipt
  claim, dry-run, rule-decision записи и `processEvent()`, а fallback actor
  запускается как `STORES/[runtimeStoreId]` вместо tenant-wide `NETWORK`.
  Missing store identity даёт deterministic `SKIPPED` с
  `BACKGROUND_STORE_ID_REQUIRED` до side effects. Локально зелёные: focused
  ledger fallback spec (`1/1 suite`, `85/85 tests`),
  `test:ci:background-execution` (`16/16 suites`, `804/804 tests`),
  `test:ci:tenant-execution` (`18/18 suites`, `1004/1004 tests`),
  `lint:ci:tenant-execution`, API typecheck, Prettier check изменённых файлов
  и `git diff --check`. Exact-SHA
  `6f8008a2551d4a96962c65e58a385e86e7b2f471` принят GitHub Actions run
  `32338344266` как `4/4 SUCCESS`; evidence:
...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  `sha256:f2cca9b5…d1e41`. На 18.08 настоящий production backup получен read-only,
  восстановлен на isolated PostgreSQL и принят clean production-history
  migration/repeat/data-zero-diff плюс activation-role TLS/HBA/SCRAM rollback.
  Exact SHA `3f325acc2428b1e3c3797075b218efeb454fae91` принят четырьмя CI
  runs; artifact `9321380247`, archive SHA-256 `adb75120…b7f4`, outer/inner
  checksums, offline hydration и повторный production-history/role/TLS/rollback
  admission на clean copy прошли. Trusted TLS SMTP worker и protected
  enrollment/SENT/accept/disable на disposable restored-copy clones также
  прошли `1/1 + 1/1` с zero residue. Gate 1MT PostgreSQL A/B matrix на отдельных
  клонах той же clean copy прошла `35/35`, включая real HTTP team-chat SSE,
  все семь attachment reader parent kinds и
  расширенный assortment HTTP `15/15`;
  fixture/database residue `0`.
  Следующий production-build HTTP/BFF/browser-срез на синтетическом Tenant B
  принят на exact SHA `771bbd5f…`: OWNER открыл согласованные продуктовые
  ...
  fixture/database residue `0`. На `230d62b1…` ассортиментная PostgreSQL suite
  сначала расширена реальными `CategoriesService` и `SuppliersService`, а на
  `d3a2d8b6…` — реальными `ProductCsvImportService` и `FactCsvImportService`,
  а на `58410b37…` — реальными `ReportsService` и `ReportsExportService`.
  Итоговая матрица `15/15` доказывает NETWORK-only category/supplier mutations,
  tenant-bound product/inventory/sales/movement CSV imports, все local report
  variants, CSV/XLSX exports без foreign rows, OOS/recommendation state,
  cross-tenant и stale-scope deny. Два последовательных прогона exact
  bytes `f59c32fc…` дали `12/12 + 12/12` на clean PostgreSQL и
  `12/12 + 12/12` на disposable restored-copy clone; fixture residue
  `0/0/0/0/0/0/0`, target/source
  core counts совпали `3/4/30/1483/51257`, database residue `0`. Затем
  два последовательных прогона exact `3e0389b4…` дали `14/14 + 14/14` на
  restored-copy clone
  через реальный Nest HTTP/RolesGuard. На exact `94db1fdd…` добавлены atomic
  RecommendationState concurrency regression и bounded SSR fan-out; два
  restored-copy run прошли `15/15 + 15/15`, BFF acceptance `9/9`.
  Production-build report/download/mutation browser journey принят: CSV/XLSX,
  recommendation `PATCH`, OOS `POST→DELETE`, console `0/0`, whole-schema
  `156`-table postflight без неожиданных deltas, database residue `0`.
  На exact `ccf81a28…` Web SSE BFF стал cookie-only, selector-allowlisted и
  private/no-store (`10/10`); exact `dfe5e0f8…` переносит fresh persisted
  authority до фиксации SSE `200`. Real Nest HTTP/PostgreSQL A/A1/A2↔B/B1
  matrix прошла `4/4 + 4/4`, Gate 1MT стал `30/30`; `156` table counts
  zero-diff, disposable DB residue `0`.
  Exact `abb8a667…` добавил fresh NETWORK/capability/exact-parent reader для
  checklist, knowledge, regulation, training и onboarding attachments;
  unit `26/26`, restored-copy `4/4 + 4/4`; после writer extension на этом checkpoint
  Gate 1MT — `32/32`,
  `156` table counts zero-diff и database residue `0`.
  Exact `fc07e959…` добавил native atomic create/update binding для regulation,
  knowledge, training, onboarding и checklist answers. Same-parent replay
  идемпотентен, cross-parent BOUND reuse запрещён, foreign-tenant reference
  откатывает parent write; focused unit `24/24`, restored-copy
  `5/5 + 5/5`, `156` table counts zero-diff, database residue `0`.
  Exact `f2e9e6ca…` добавил full-set lifecycle: status-only update сохраняет
  ссылки, omission удаляет exact native binding, последний blob переводится в
  `QUARANTINED`, а shift-regulation delete сначала снимает binding под parent
  lock. Final attachment-focused unit `48/48`; два fresh restored-copy run
  `5/5 + 5/5`, все `156` table counts zero-diff, database residue `0`.
  Exact `7928b7f8…` добавил реальный PostgreSQL row-lock race для конкурирующих
  bind одного PENDING blob и remove/replacement одного parent. Два fresh-clone
  run прошли `6/6 + 6/6`: один initial binding, полный rollback проигравшего
  parent, согласованные final JSON/binding, quarantine и запрет rebind;
  Gate 1MT стал `33/33`, все `156` table counts zero-diff, residue `0`.
  Exact `c5b86aba…` сериализует direct persisted-user revoke с download через
  exact tenant-scoped `User + StaffAttachment FOR SHARE` до metadata. Два
  fresh-clone run `7/7 + 7/7` доказали held `isActive=false` commit → stale
  subject deny без blob bytes и без повреждения binding; Gate 1MT стал `34/34`,
  focused unit `49/49`, `156` table counts zero-diff, residue `0`.
  Exact `bc8fffd2…` ввёл общий transaction-scoped advisory key для reader и
  `updateAccessRole`/`updateSystemRole`. Custom/system-role revoke дважды прошёл
...
- Gate 1MT staff STORES successor: knowledge (`085f8bbd…`), shift regulations
  (`6ce36a41…`), training (`40a8e828…`), onboarding (`26b9f442…`) и
  checklists/templates (`70d8301d…`) приняты на disposable PostgreSQL и
  production-build B1↔B2 browser. Checklist slice включает focused API
  `59/59`, full API `3192`, Web BFF `16/16`, PostgreSQL `13/13`, hidden
...
- Gate 1MT staff attachment parent-delete guard: migration
  ...
  изменялись.
- Gate 1MT staff attachment reconciliation controller: добавлен защищённый
  digest-bound workflow `plan → approve → apply/check → rollback/check` с
  immutable runtime binding, repeat-safe lost-response reconciliation,
  aggregate-only audit и fail-closed production attestation. Unit suite прошла
  `9/9`; disposable PostgreSQL 16 rehearsal прошёл plan, apply, повторный apply,
  check, rollback, повторный rollback и final zero-residue check. Три
  operational scripts включены в exact immutable release-artifact allowlist и
  защищены SHA256SUMS/provenance adversarial gate. Production
  plan/apply не выполнялись. Ложный exact-history blocker локализован: старый
  Windows checkout содержал `150` migration files с CRLF, хотя Git/LF manifest
  `1..178` корректно равен `7f986797…`; применённые raw-байты поэтому дали
  локальный `ba9ca94c…`. Применённые migration SQL не переписывались.
  `db:deploy` теперь использует одноразовый symlink-free LF-normalized Prisma
  artifact. Unit `3/3`, clean PostgreSQL 16.15 deploy `187/187`, CURRENT179
  preterminal digest `7f986797…` и повторный no-op deploy прошли. Attachment
  PostgreSQL lifecycle повторно прошёл plan/apply/replay/check/rollback/replay/
  check с final zero residue. До production effect остаются restored-copy
  rehearsal свежего backup на новом exact artifact, exact temporary
  role/grant attestation, owner review и отдельное approval; статус `NO-GO`.
...
- Gate 1MT public guest club selector binding: `selectGameClub` и Telegram
  Mini App session exchange теперь fail-closed отклоняют конфликтующие
  `clubId = tenant:store` плюс явные `tenantSlug/storeId`. Обычный selector
  reject происходит до tenant/store lookup, profile mutation и JWT signing;
  Mini App reject происходит до candidate/profile lookup и JWT signing, поэтому
  один доступный кандидат больше не выбирается молча при конфликтующих
  selectors. Targeted guest portal service suite прошла `196/196`, targeted
  ESLint и Prettier по изменённым файлам зелёные. Это частичный public guest
  hardening; общий Telegram/public guest/outbound matrix и production canary
  остаются открыты.
- Gate 1MT public guest BFF query allowlist: active
  `/api/guest-portal/[...path]` GET больше не пересылает произвольный
  `url.search`. Разрешены только `lat/lng/radiusKm` для public club directory
  и `offset/limit` для authenticated missions paging; все остальные query,
  включая tenant/store-like selectors на session/public-config paths, получают
...
- Gate 1MT Telegram edge outbound base URL protocol admission: active
  ...
  public guest/Telegram/outbound matrix и production canary остаются открыты.
- Gate 1MT Telegram poller stale/duplicate update guard: active
  `runTelegramPollingTick()` теперь пропускает stale updates до LeetPlus
  webhook handler, если `update_id < currentOffset`; duplicate `update_id`
  внутри одного `getUpdates` batch после первого успешного продвижения offset
  также не доходит до handler. Targeted Telegram/API suite прошла `5/5`
  suites, `218/218` tests; API typecheck, targeted ESLint, Prettier и
  `git diff --check` зелёные. Exact SHA `b415c5a3…` принят GitHub Actions run
...
- Dedicated activation Prisma pool admission реализован локально: перед каждым
  ...
  scoped lint зелёные. Exact SHA `51995635…` принят push/PR CI
  `32068262701`/`32068266758` attempt `2` как `3/3 SUCCESS`; artifact
  `9301062934`, digest `sha256:ed1db27f…16e7`. In-process HTTP/PG pool gate
  закрыт
- SHA-bound artifact runtime layout расширен manifests, web public assets,
  canonical migrations и founder operational scripts. Exact SHA `90a94f1b…`
  принят push/PR CI `32075030815`/`32075035388` как `3/3 SUCCESS`; artifact
  `9303394475`, `28 419 842` bytes, digest `sha256:b73c932f…d5fd`. До upload CI
  распаковал архив, проверил внутренний `SHA256SUMS`, выполнил frozen offline
  production install, Prisma generate и CLI/runtime resolution
- Downloaded artifact child-process fixture реализован: новый зависимый CI job
  обязан скачать exact artifact, повторить checksum/hydration, поднять
  disposable PostgreSQL и настоящий `dist/main.js`, пройти через JWT guards и
  HTTP `provision→GO→ACTIVATED→REPLAYED`, затем доказать database/role zero
  residue. Exact SHA `0c721f4d…` принят push/PR CI
  `32078882449`/`32078886786` как `4/4 SUCCESS`; artifact `9304656653`,
  `28 421 509` bytes, digest `sha256:5dc17d35…5700`. Child process вернул
  `SHELL_PROVISIONED→ISSUED→ACTIVATED→REPLAYED`, tenant
  `ACTIVE/OWNER_INVITED`, database residue `0`, role residue `0`
- Protected initial OWNER invite status/revoke принят на exact SHA
  `0bc178bca9b1d0bfa8e0cdd5388d994e2d75ae4b`: PR CI `32084166159` —
  `4/4 SUCCESS`, включая PostgreSQL fixture; artifact `9306361304`, digest
  `sha256:391b1b8d5df78165df33aa96b19ab8ba1ce03d69fed690aefd8bf15b900faaaf`.
  Reissue engineering принят следующим P0: canonical migration 184, immutable
  predecessor-chain, новый encrypted invite/outbox/token, causal
  `createdAt→releasedAt`, exact replay до successor lookup и запрет blind resend.
  Локально API typecheck/lint, owner lifecycle `2 suites / 18`, static migration
  `4/4`, full API `157 suites / 3144 passed` и real PG `1/1` зелёные. Exact SHA
  `f33e598ad2955afaf378777165bd2c34e6471c7a` принят push/PR CI
  `32098804217`/`32098806708` как `4/4` и `3/3 SUCCESS`; push artifact
  `9311012974`, digest `sha256:f0843edc…77e81`
- Текущий инженерный successor owner onboarding реализован поверх нового
  канонического CURRENT185
  (`20260818020000_identity_mail_delivery_current_head_v1`): active worker
  fail-closed привязан к точным `185` migrations, least-privilege worker
  реально выполнил `PENDING→SENT`, а затем `AuthService.acceptInvite` создал
  ровно одного `OWNER/NETWORK`, перевёл tenant
  `OWNER_INVITED→ONBOARDING`, claim `INVITE→USER` и уничтожил ciphertext.
  Отдельный strict-TLS SMTP fixture и полный disposable PostgreSQL
  `activate→revoke→reissue→SENT→preview→accept` прошли `1/1`; full API —
  `157 suites / 3144 passed / 2 todo`. Implementation SHA
  `14193e5151cf5ba1118466facdaf4a8a4a4e0922` принят push/PR CI
  `32105326187`/`32105331954` как `4/4 SUCCESS`; artifact `9313186108`,
  `28 444 909` bytes, digest
...
- Следующий P0 реализован на commit `3eef2be1b8a04888908bfd28d5e9c77007bd0449`:
  ...
  owner lifecycle через реальный CLI — `1/1 PASS`. Full CI обнаружил TLS
  residue PgBouncer fixture; `55748217…` восстанавливает exact auto-conf и
  добавляет post-cleanup `ssl=off` gate. Push/PR CI
  `32115334678`/`32115340009` приняты `4/4 SUCCESS`, focused PG
  `32115339918` — `SUCCESS`; artifact `9316782148`, digest
  `sha256:a2b0ea21…3f0706`, downloaded SHA-256 `d360e5b1…a7849167`, scripts
  `14=8+6`. Production, Tenant A и внешний tester не изменялись
...
CURRENT191 persisted approval/expiry ledger принят exact SHA `56f24216…`, CI
`31680337637` — `3/3 SUCCESS`, PostgreSQL replay/drift/expiry smoke зелёный,
artifact `sha256:5e8e07de…a3a`. CURRENT192 atomic selected-Store import,
исправленный CURRENT193 execute-only boundary и CURRENT194 persisted
attestation ledger приняты одним exact SHA `0578e5e4…`: CI `31688314546` —
`3/3 SUCCESS`, artifact `sha256:9e0a8a9b…fa62c`. Separate Prisma runtime принят
на SHA `2a5811e0…`, CI `31692243774` — `3/3 SUCCESS`; composed signed bootstrap
принят на SHA `e829f303…`, CI `31694281914` — `3/3 SUCCESS`, artifact
`sha256:1db3c8e7…afc7`. CURRENT194 lifecycle принят на SHA `01cd9e45…`, CI
`31697571300` — `3/3 SUCCESS`, artifact `sha256:2062ba99…b271`: gate `32/32`,
zero-inflight drain, owner-only revoke, exact lost-response replay, terminal
close и disposable credential rotation прошли реальный PostgreSQL fixture.
Документационный SHA `ed2ea410…` принят CI `31698913278` как `3/3 SUCCESS`,
artifact `sha256:f73c8241…1782`. Следующий слой принят exact SHA `9baa29c3…`,
CI `31700945915` — `3/3 SUCCESS`, artifact `sha256:57fb23ac…ddba6`:
restart reconciliation использует отдельный owner-only adapter и принимает только
...
CURRENT202 V2 single-founder global bootstrap реализован как отдельный
...
cooling-off входят в подписанный payload глобального platform trust. V2 явно
фиксирует `platformScope=GLOBAL`, `customerKeyCeremonyRequired=false`,
`additionalTenantKeyCeremonyRequired=false`, отдельный `SHARED BETA GO` и
полностью удаляет tenant/store/trial-поля V1 из payload/receipt. CURRENT198
transition gate сейчас
...
CURRENT202 V2/CLI matrix — `12/12`; CURRENT198 и CURRENT201 регрессии — `20/20`
и `16/16`. Exact SHA `c2b7b370627d79e75f0afbd41dbfe0cf04a5cb6b`
принят CI `31790021275` как `3/3 SUCCESS`; artifact `9215344140`, digest
`sha256:8e7b70c5fd252841f409d9efb311018ea5c796e7adb61083872529cafdddb504`.
Фактические keys/evidence, registry apply, production-origin
registration отложен в post-beta hardening; restored-copy rehearsal позже
принят отдельным production-backup evidence. Production application, текущая
...
Исторический CURRENT202 V1 принят exact SHA
...
| BETA-MOD-STAFF-005 | P0        | В работе      | Закрыть регламенты и чек-листы                      | Shift regulations `6ce36a41…` и checklists/templates `70d8301d…` приняты с fresh `NETWORK \| STORES` parent/file policy. Checklist acceptance: focused API `59/59`, full API `3192`, Web BFF `16/16`, local PG `13/13`, production-build B1↔B2 browser. До завершения задачи остаётся attachment archive/delete/orphan matrix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | BETA-MOD-STAFF-001               |
| BETA-MOD-STAFF-006 | P0        | В работе      | Закрыть знания, обучение и аттестации               | Knowledge `085f8bbd…`, training `40a8e828…` и onboarding `26b9f442…` приняты с fresh `NETWORK \| STORES` parent/file policy. До завершения задачи остаётся полный tests/assessments/readiness slice                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | BETA-MOD-STAFF-001, BETA-IAM-002 |
...
| BETA-MOD-STAFF-009 | P0        | В работе      | Защитить attachments и evidence                     | Reader всех семи parent kinds, native atomic binding/unbind/quarantine, bind/unbind/replacement races и persisted-user/custom-role/system-role revoke приняты. Knowledge/regulation/training/onboarding/checklist parents используют совпадающую fresh `NETWORK \| STORES` authority. Production read-only trigger inventory `7/7` принят на admitted SHA; aggregate scan выявил `5 446 UNRESOLVED`, `309` multiple-parent, `741` unreferenced и `243` URL-review cases. Digest-bound reconciliation controller прошёл unit `9/9` и disposable PostgreSQL plan/apply/replay/check/rollback/replay/zero-residue. Windows CRLF root cause устранён canonical LF deploy artifact: clean PostgreSQL 16.15 применил `187/187`, CURRENT179 manifest `7f986797…`, replay no-op. Production plan/apply не выполнялись. До effect нужны fresh-backup restored-copy rehearsal на новом admitted exact artifact, временная least-privilege role/grant attestation, owner review; затем audited production reconciliation + zero-diff, archive/delete/orphan/retention production-build browser matrix и общий file journey                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | BETA-MOD-STAFF-001, BETA-SEC-006 |
...
Current-state уточнение `BETA-MOD-STAFF-006`: onboarding exact
`26b9f4425e4fc416fb1e741949be2a30a53576d7` принят локально с fresh
`NETWORK | STORES` parent/file policy: unit `55/55`, Web BFF `15/15`, pilot
HTTP `160/160`, local PostgreSQL `12/12` и production-build B1↔B2 browser.
Push CI `32220369599` принял exact SHA как `4/4 SUCCESS`.
Checklists/templates exact
`70d8301d204141c7d4d07c83c2752737f18aaa7d` также приняты локально с fresh
`NETWORK | STORES` template/run/report/file policy: focused API `59/59`, full
API `3192`, Web BFF `16/16`, local PostgreSQL `13/13`, production-build B1↔B2
browser и fail-closed foreign filter/mutation. Последняя прежняя network-only
staff parent family закрыта. Exact push CI `32223728916` принял implementation
SHA как `4/4 SUCCESS`. Tests/assessments/readiness и общий Gate 1MT всё ещё не
закрыты.
...
- `CHECKLIST_RUN`, `KNOWLEDGE_ARTICLE`, `SHIFT_REGULATION`,
  `TRAINING_COURSE`, `ONBOARDING_PLAN` authoritative reader принят на exact
  `abb8a667…`; native atomic create/update binding принят на `fc07e959…`.
  Full-set native unbind/quarantine и shift-regulation delete lifecycle приняты
  на `f2e9e6ca…`. Restored-copy evidence: reader `4/4 + 4/4`, writer/lifecycle
  `5/5 + 5/5`, все `156` table counts zero-diff, database residue `0`;
- apply-backfill/reconciliation command, production-like inventory текущего
  tenant, manual quarantine resolution, legacy/backfill reference cleanup,
  archive/move retention/revoke,
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `35/35`: assortment `15/15`, team chat `4/4` с real HTTP SSE pre-header deny, CRM/users и staff attachments `8/8` со всеми семью reader parent kinds и native writer/lifecycle/race flow пяти content parents; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Exact `94db1fdd…` добавил atomic RecommendationState concurrency и bounded SSR fan-out; два restored-copy run дали `15/15 + 15/15`, report BFF `9/9`, production-build report/download/mutation browser journey, whole-schema `156`-table postflight и zero database residue. Exact `ccf81a28…`/`dfe5e0f8…` закрыл team-chat SSE BFF/API boundary. Exact `abb8a667…` закрыл fresh NETWORK/capability/exact-parent attachment reads; exact `fc07e959…` закрыл atomic native writers, same-parent replay и foreign-tenant rollback; exact `f2e9e6ca…` закрыл status-only retention, unbind, last-reference quarantine и shift-regulation delete lifecycle; exact `7928b7f8…` закрыл row-lock bind/unbind/replacement races; exact `c5b86aba…` закрыл direct persisted-user revoke/download race; exact `bc8fffd2…` закрыл custom/system-role capability-revoke/download races: role/attachment unit `56/56`, expanded users/staff `85/85`, PG `4/4 + 4/4` и `8/8 + 8/8`, zero table diff/residue. До готовности остаются file production-build browser/STORES policy, job/Telegram/public-guest negative matrix пяти product modules и supporting `INTEGRATIONS`, controlled outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
18. Реализовать `BETA-MT-007..009`: полную shared PostgreSQL/API/BFF/browser/
    ...
    PostgreSQL foundation `35/35`, включая все семь attachment reader parent
    kinds, native writer/lifecycle пяти content parents и real HTTP SSE, и
    browser read/admission slice на `771bbd5f…` уже приняты; следующий срез —
    attachment production-build browser и STORES policies, затем background
    jobs/Telegram/public guest binding.
...
| BETA-LAUNCH-CP-001 |        P0 | Выполнено     | Зафиксировать принятый identity/database baseline             | CURRENT186 отдельно принят и refrozen; CURRENT187 достиг Engineering Green; API/activation `8cce1408…` принят CI `32038312056`; invite-only Web `15b9e3ac…` принят CI `32040816369`. Восемь новых commit'ов `origin/main` объединены, конфликт разрешён с сохранением beta outbound deny и durable purchase drain. Канонический merged SHA `eb46d587…` имеет `behind 0`, принят push CI `32043177732` как `3/3 SUCCESS`; artifact `9292418006`, digest `eb5b7a…acfce9`. Targeted merge suites `501/501 + 72/72`, API/Web typecheck, focused lint и Web boundary `9/9` зелёные. Каждый CURRENT187 receipt сохраняет `testAccessAuthorized=false/sharedBetaAccess=false`                                                                                                                        | BETA-LAUNCH-CP-000, BETA-IAM-004K, CURRENT186 acceptance, CURRENT187 Engineering Green         |
...
| BETA-LAUNCH-CP-003 |        P0 | В работе      | Включить защищённые email-приглашения владельца и сотрудников | Реализованы `FOUNDER_OPERATOR_BETA_GO_V1`, atomic `FOUNDER_OPERATOR_BETA_ACTIVATION_V2` и отдельный activation database pool/live role assertion. Default `DISABLED`; `PREPARE` создаёт только shell + GO/revoke; `ACTIVE` одной SERIALIZABLE transaction создаёт OWNER/NETWORK aggregate, запускает trial, consume GO и делает единственный HOLD→PENDING. Clean PG16 `184` migrations и полный revoke/reissue/replay приняты exact-SHA CI на `f33e598a…`; restricted-role activation/replay/immutability и `INHERIT`/PUBLIC drift deny зелёные. Production role/secret/grant отсутствуют. Остаток — restored-copy enrollment, provider `SENT`, suspend/accept и production-like rehearsal. После accept OWNER управляет users/roles/stores только своего tenant; пароль задаётся получателем | BETA-IAM-004C, BETA-IAM-004L, BETA-IAM-005, BETA-IAM-007, BETA-TEN-001..008, BETA-SEC-008..010 |
...
| BETA-LAUNCH-CP-005 |        P0 | В работе      | Закрыть Gate 1MT, tenant-aware workers и shared Telegram      | Локальный synthetic browser/store-scope срез принят: owner открыл согласованные модули; scoped actor видел только разрешённый Store, включая direct denied-store URL и users/roles; invite-only flow создал email-bound employee invite только в разрешённый Store. Это `PARTIAL PASS`, не полная A/B matrix. Для финала нужны API/BFF UUID/filter/aggregate/export/file/job/SSE, tenant-aware schedulers/workers, shared Telegram dedupe/routing и отсутствие hardcoded demo behavior                                                                                                                                                                                                                                                                                                        | BETA-MT-001..009, все BETA-MOD-\* P0, BETA-SEC-003..006, BETA-OPS-002..003, BETA-OPS-008       |
...
Browser checkpoint 17.08.2026: полный локальный Nest/Web/PostgreSQL запуск на
синтетическом tenant подтвердил owner module smoke, allowed/denied Store
direct-URL containment, users/roles visibility и реальное создание
email-bound приглашения только в разрешённый Store. UI больше не создаёт новый
`User` с администраторским паролем: новый account возможен только через
персональную email-ссылку, пароль задаёт получатель. Реальный preview/accept
создал сотрудника с `STORES` и ровно одним разрешённым Store. Web gates: pilot BFF
`4/4`, users/roles BFF `5/5`, typecheck `PASS`, full lint `0 errors`.
Подробности:
[Gate 1MT local browser evidence](./docs/open-beta/gate-1mt-local-browser-evidence-2026-08-17.md).
Это повышает Gate 1MT только до `PARTIAL PASS`; production-like A/B matrix,
workers/Telegram/files/SSE, Gate 2, production deployment и внешний invite
остаются `NO-GO`.
...
| `CURRENT188-LANGAME`                    |        P0 | `EXACT-SHA CI ACCEPTED THROUGH INITIAL-SYNC PREFLIGHT / NONCANONICAL / DORMANT`   | Provider-read preflight принят exact SHA `6794214a…`, CI `31675201041` `3/3 SUCCESS`, artifact `sha256:a5ca6834…f7ca3`: API `219/219`, tenant `989/989`, full `3056 + 2 todo`, Web `19/19`. Он повторно связывает exact receipt/activation/Store/source/credential/audit и NETWORK authority до и после сети, делает только 3 bounded `GET` (`clubs/products/goods` выбранного клуба), запрещён в production, не создаёт Product/Inventory/SyncJob и оставляет actual import unavailable. Reconcile ранее принят SHA `4e2c9b29…`; legacy external sync deny — SHA `9d66276a…`; остальные provider clubs не создаются автоматически                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | CURRENT191 plan+approval приняты; CURRENT192 import остаётся dormant/noncanonical до PG/CI, runtime-role, canonical/rehearsal и atomic Route/UI cutover                                                                                                                                                                                                                                       |
| `CURRENT191-LANGAME-INITIAL-PLAN`       |        P0 | `EXACT-SHA CI ACCEPTED / PURE / DORMANT / ZERO EFFECT`                            | Immutable selected-Store product/inventory plan связывает CURRENT188 approval/read-set, exact tenant/Store/source/domain/club, safe names/state/counts и полный sorted action set. Reject: count drift, duplicate/invalid IDs, inventory outside products, accessor/symbol/extra fields, invalid quantity/domain. Всегда `providerWritesStarted=false`, `platformWritesStarted=false`, `productionImportAllowed=false`; exact SHA `d433cd67…`, CI `31677119183` `3/3 SUCCESS`, Langame `235/235`, artifact `sha256:010b2f9f…88fa`; локальный successor добавляет process-local branded canonical serializer для независимой DB-проверки bytes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Принять CURRENT192 PostgreSQL/CI, затем application coordinator/runtime grants/attestation и canonical production-like rehearsal                                                                                                                                                                                                                                                              |
| `CURRENT191-LANGAME-APPROVAL`           |        P0 | `EXACT-SHA CI ACCEPTED / DORMANT SUCCESSOR / NONCANONICAL / NO BUSINESS EFFECT`   | Отдельный `successor-candidates` ledger: 15-минутный PII-free preflight, one-time confirm, exact replay, bounded expiry, append-only audit. Record/confirm заново блокируют active tenant, consumed/unrevoked/unexpired GO, active NETWORK actor и полный CURRENT188 receipt/claim/Store/source/credential/ACTIVATED binding; approval `validUntil` точно равен исходному `preflight.expiresAt`. Нет PUBLIC/application grants, Prisma schema, Product/Inventory/SyncJob/provider/route effects; static `6/6`; exact SHA `56f24216…`, CI `31680337637` `3/3 SUCCESS`, artifact `sha256:5e8e07de…a3a`, rollback-only PostgreSQL replay/drift/expiry smoke зелёный                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | CURRENT192 claim/import/complete/reconcile PG acceptance; execute-only runtime role/grants/attestation; затем canonical promotion, restored-copy rehearsal и cutover                                                                                                                                                                                                                          |
| `CURRENT192-LANGAME-EXECUTION`          |        P0 | `EXACT-SHA CI ACCEPTED / DORMANT / NO RUNTIME GRANTS`                             | One-time approval-bound claim с digest raw token; DB независимо проверяет exact canonical CURRENT191 bytes/target/authorization/counts/order. Одна транзакция re-lock/re-prove tenant/GO/NETWORK/CURRENT188–191 binding, сохраняет manual product fields, не деактивирует missing SKU, пишет только выбранный Store UTC inventory snapshot и terminal `COMPLETED`; exact replay/reconcile/expired-unexecuted claim предусмотрены. Default-off/production-denied coordinator после lost response продолжает только из proven `CLAIMED`; static `6/6`, planner `17/17`, coordinator `11/11`, Langame `247/247`. PostgreSQL step и полный run приняты на SHA `0578e5e4…`, CI `31688314546` `3/3 SUCCESS`, artifact `sha256:9e0a8a9b…fa62c`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Реальный attested runtime DB client, canonical apply/rollback/zero-diff, atomic route/BFF/UI cutover и pilot                                                                                                                                                                                                                                                                                  |
| `CURRENT193-LANGAME-RUNTIME`            |        P0 | `EXACT-SHA CI ACCEPTED / NONAUTHORIZING / ROOTS EMPTY`                            | Exact CI-only planner связывает release SHA, database identity, отдельные owner/executor OID и CURRENT192 SQL SHA; fixed LOGIN role имеет отрицательные privilege attributes, zero membership/ownership/direct/default privileges, exact DB/schema ACL и ровно 3 callable RPC при 2 guards. Rollback-only PG16 matrix снимает inherited PUBLIC и отрицает полный PG16 table privilege set, sequence/guard/DDL/TEMP/role escalation. Independent Ed25519 verifier связывает branded catalog receipt+release+DB/role OID+SQL SHA максимум на 5 минут. Guard `search_path` исправлен; static `17/17` и actual PG16 step приняты на SHA `0578e5e4…`, CI `31688314546` `3/3 SUCCESS`. Narrow DI port не принимает broad `PrismaService`, `Prisma.Sql` или arbitrary query; production roots empty                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Реальный dedicated attested runtime DB client, lifecycle/drain/rotation и canonical production-like rehearsal                                                                                                                                                                                                                                                                                 |
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |        P0 | `RESTART RECOVERY EXACT-SHA CI ACCEPTED / DORMANT`                                | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Separate Prisma adapter и composed signed bootstrap приняты exact SHA `e829f303…`. Lifecycle сначала запрещает новые RPC и ждёт zero in-flight, затем выполняет только owner-side exact revoke с lost-response replay и всегда закрывает оба Prisma client; runtime role revoke не получает. Общий accepted gate `32/32`; disposable PG fixture ротирует обе credentials, отрицает старые пароли, открывает новый signed bootstrap и проверяет два terminal `REVOKED`/шесть ledger events. Lifecycle SHA `01cd9e45…`, CI `31697571300` `3/3 SUCCESS`, artifact `sha256:2062ba99…b271`. Fresh owner-only restart recovery adapter без runtime credential, malformed-request cleanup и actual-PG persisted replay приняты exact SHA `9baa29c3…`: gate `36/36`, CI `31700945915` `3/3 SUCCESS`, artifact ID `9181448914`, digest `sha256:57fb23ac…ddba6`. Production recovery deny, signed durable revoke intent ещё не реализован. Production credentials/roots/roles/grants не создаются                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Coordinator-signed durable revoke intent; pinned production TLS; canonical restored-copy rehearsal                                                                                                                                                                                                                                                                                            |
...
| `GATE-1MT-HTTP-INVENTORY`               |        P0 | `IMPLEMENTED`                                                                     | exact `295` handlers: `241` pass static slice, `54` blocked; all `21` outbound remain OFF. `POST /stores` fresh-NETWORK-bound, но external PILOT generic create остаётся `409` до dedicated provisioning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Каждый blocked handler должен получить persisted tenant/store/capability/entitlement enforcement и A/B evidence                                                                                                                                                                                                                                                                               |
| `GATE-1MT-BFF-INVENTORY`                |        P0 | `IMPLEMENTED / DORMANT CURRENT188–190 CANDIDATES / PUBLIC GAME GAP VISIBLE`       | exact `130` protected route-файлов / `158` handlers B2B и supporting integrations; SHA-256 `ec4c892…`; focused inventory `4/4`; guest cookie transport `2/2`; unimported candidates: CURRENT188 Langame `17/17`, CURRENT189 employee invites `8/8`, CURRENT190 persisted logout/media `6/6`; все используют server-side cookie bearer, bounded/private safe projections и доказывают отсутствие active Route imports. Legacy Langame settings, users invites и public ID-only media остаются активными/явно blocked до atomic cutover                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | После canonical/runtime acceptance зарегистрировать exact API routes и атомарно импортировать соответствующие BFF/UI candidates; удалить legacy write/public-media boundaries; затем пройти A/B browser matrix по всем трём контурам                                                                                                                                                          |
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 35/35 + REPORT/SSE BROWSER PASS`                                | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На CURRENT185 production-backup restored-copy clones выполнены assortment HTTP `15/15`, team-chat `4/4` с real HTTP SSE, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`, attachments `8/8` со всеми семью reader parent kinds и native writer/lifecycle/race flow пяти content parents: суммарно `35/35 PASS`; fixture/core/database residue нулевой. Exact `94db1fdd…` дважды прошёл `15/15`, report BFF `9/9`; production-build browser принял report reads, CSV/XLSX downloads, recommendation `PATCH`, OOS `POST→DELETE`, console `0/0` и whole-schema `156`-table postflight без unexpected deltas. Exact `ccf81a28…`/`dfe5e0f8…` принял team-chat BFF/API SSE boundary. Exact `abb8a667…` принял five-parent attachment reader: unit `26/26`, PG `4/4 + 4/4`. Exact `fc07e959…` принял five-parent atomic writer, same-parent replay и foreign-tenant rollback: unit `24/24`, PG `5/5 + 5/5`. Exact `f2e9e6ca…` принял status-only retention, unbind, last-reference quarantine и shift-regulation delete lifecycle: final attachment-focused unit `48/48`, PG `5/5 + 5/5`. Exact `7928b7f8…` принял row-lock bind/unbind/replacement races: PG `6/6 + 6/6`. Exact `c5b86aba…` принял direct persisted-user revoke/download race: unit `49/49`, PG `7/7 + 7/7`. Exact `bc8fffd2…` принял custom/system-role capability revoke: role/attachment unit `56/56`, expanded `85/85`, PG `8/8 + 8/8`. `156` table counts zero-diff, database residue `0`. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`, `gate-1mt-report-browser-evidence-2026-08-18.md`, `gate-1mt-team-chat-sse-evidence-2026-08-18.md`, `gate-1mt-attachment-parent-evidence-2026-08-18.md` | Store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram, attachment production-build browser, controlled outbound email/digest                                                                                                                                                                                                                     |
...
| `GATE-1MT-TEAM-CHAT-FRESH-SCOPE`        |        P0 | `RESTORED-COPY API/BFF/SSE ACCEPTED / NOT DEPLOYED`                               | Все 7 tenant-user paths team chat повторно получают persisted tenant/store authority внутри service до Prisma access; report/SSE explicit store проходит fresh requested-store validation; selectors для channels/messages/stores/users используют DB-returned scope; deny выполняется до default-channel side effect. `dfe5e0f8…` дополнительно завершает authority check до фиксации `200 text/event-stream`, поэтому hidden channel даёт pre-stream `404`, stale scope — `401`. Unit `22/22`; real restored-copy PG A/A1/A2↔B/B1 report/SSE/create/update/read-receipt/stale matrix `4/4 + 4/4`; Web BFF exact `ccf81a28…` принимает только один UUID selector, cookie auth и private/no-store, `10/10`; API/Web lint/typecheck/build зелёные, `156` table counts zero-diff, residue нулевой                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Attachment reader покрывает все 7 parent kinds; native bind/lifecycle/races и subject/custom/system-role revoke приняты на `fc07e959…`/`f2e9e6ca…`/`7928b7f8…`/`c5b86aba…`/`bc8fffd2…`; остаются production-build browser и STORES policies. 8 CRM routes остаются отдельным NETWORK-only slice без STORES widening; не открывает 60 NETWORK-only staff routes, salary или internal producers |
...
| `FOUNDER-OPERATOR-BETA-GO`              |        P0 | `RESTORED-COPY FULL OWNER WORKFLOW PASS / PRODUCTION NO-GO`                       | Ручное создание tester account запрещено. GO/revoke, atomic v2 activation и dedicated database pool/live least-privilege assertion не требуют USB/offline key; default mode `DISABLED`. На production-backup template disposable clone приняты shell→GO→activation/replay→mail enrollment→SENT→preview/accept→disable: один OWNER/NETWORK, tenant ONBOARDING, claim USER, ciphertext очищен. Exact release `3f325acc…`, artifact `9321380247`; production и текущие четыре клуба не изменены; PUBLIC/обычные roles не имеют RPC EXECUTE, production activation/worker roles, secrets и grants не создавались                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Production roles/secrets/controlled SMTP canary; Gate 1MT/2; затем PREPARE→one-tenant ACTIVE canary и mailbox-bound OWNER invite                                                                                                                                                                                                                                                              |
| `FOUNDER-RESTORED-COPY-PREFLIGHT`       |        P0 | `EXACT-SHA ARTIFACT + MIGRATION/ROLE/MAIL FULL PASS / PROD NO-GO`                 | Настоящий production backup `sha256:31ad21d5…b9b` восстановлен в isolated PG16.14. V2 связывает `153 applied / 4 rolled back / 0 unfinished`; clean history lane применил 32 migration, repeat zero pending, итог `185/4/0`, data zero-diff. Downloaded artifact `9321380247`, SHA `adb75120…b7f4`, outer/inner checksum и offline hydration приняты. Activation role прошла apply/attest/TLS1.3 HBA+SCRAM/rollback; trusted TLS SMTP и full owner enrollment/SENT/accept/disable прошли на disposable clones. Final role/database residue `0`, preflight `READY`. Отчёты: `founder-production-restored-copy-rehearsal-2026-08-18.md`, `founder-restored-copy-mail-rehearsal-2026-08-18.md`. Production application/DB/services и Tenant A не изменялись                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Gate 1MT/2; production roles/secrets/SMTP canary, monitoring и controlled cutover                                                                                                                                                                                                                                                                                                             |
| `FOUNDER-PRODUCTION-HISTORY-LANE`       |        P0 | `EXACT-SHA ARTIFACT + CLEAN RESTORED-COPY PASS / PROD NO-GO`                      | Новый `plan/apply/check` controller exact-bound к source migration/rollback manifests, четырём stale empty digest runs и полному 185-file tree. CURRENT179 SHA `f4437ebb…91a`, CURRENT185 SHA `2979599d…674`, tree `873a3af7…062`, plan `c5dbdf31…37a`; final preterminal `7a0bb533…bcc`, worker `d2025dca…465`. SHA `3f325acc…fae91` принят четырьмя CI runs; downloaded artifact `9321380247`, SHA `adb75120…b7f4`, outer/inner checksum, offline hydration, final history check и artifact-bound role/TLS/rollback прошли. Runtime принимает только canonical clean digest и этот reviewed production-history digest; произвольный третий digest fail-closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Trusted SMTP/worker enrollment, Gate 1MT/2 и отдельный production cutover GO                                                                                                                                                                                                                                                                                                                  |
| `FOUNDER-ACTIVATION-ROLE-DEPLOYMENT`    |        P0 | `EXACT-SHA CI ACCEPTED / UNIT 6/6 + SYNTHETIC PG LIFECYCLE PASS / PROD NO-GO`     | Restored-copy-only controller: live-branded preflight перед первым effect, SERIALIZABLE + advisory lock, client-side salted SCRAM verifier вместо raw password SQL, exact LOGIN/NOINHERIT attributes, connection limit/expiry, zero membership/ownership/cross-DB dependencies и direct other-DB grants, ровно CONNECT+USAGE+один wrapper EXECUTE, persisted role marker и secret-free receipt. Synthetic PG lifecycle: PLAN→APPLIED→ATTESTED→APPLY_RECONCILED→ROLLED_BACK→ROLLBACK_RECONCILED; исходные PUBLIC ACL восстановлены, residue 0. SHA `032bacbf…`, push CI `32059938202` и PR CI `32059941436` — `3/3 SUCCESS`; artifact `9298073553`, digest `sha256:137acecc…a8b`; production не менялся                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | На immutable production-backup restored copy принять downloaded CI artifact, direct network receipt, dedicated pool/live API call, session drain и controlled emergency revoke; затем migration apply/replay/rollback/zero-diff                                                                                                                                                               |
| `FOUNDER-ACTIVATION-ROLE-NETWORK`       |        P0 | `EXACT-SHA CI ACCEPTED / UNIT 7/7 + SYNTHETIC TLS/SCRAM PASS / PROD NO-GO`        | Exact pre/post role receipt+catalog attestation, HBA catalog precedence, exact hostssl target+role+127/32 SCRAM allow, hostssl other-DB reject, hostnossl reject, CA SHA/standard IP peer verification, TLS session identity, correct login, wrong-secret `28P01`, other-DB/plaintext `28000`, direct table denial, stage-safe failure codes и secret/path/error-free receipt. PG16.13: TLS1.3/AES-256-GCM, evidence `5674b09f…dd7b`; после controlled role rollback process/port/temp residue 0. SHA `821b2fbd…` принят push/PR CI `32065667436`/`32065674292` как `3/3 SUCCESS`; artifact `9300127232`, digest `sha256:f2cca9b5…d1e41`; production не менялся                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Повторить на downloaded artifact + immutable production backup; затем production PgBouncer/session drain и live API process acceptance                                                                                                                                                                                                                                                        |
| `FOUNDER-ACTIVATION-POOL-API`           |        P0 | `EXACT-SHA CI ACCEPTED / UNIT 60/60 + POSTGRESQL HTTP PASS / PROD NO-GO`          | Dedicated activation Prisma pool fresh-attest'ит exact session role/database/TLS в каждой транзакции; production ACTIVE требует `sslmode=verify-full`. Production database service и настоящий HTTP `AdminController` дали `ACTIVATED→REPLAYED` и zero residue. SHA `51995635…` принят push/PR CI `32068262701`/`32068266758` attempt 2 как `3/3 SUCCESS`; artifact `9301062934`, digest `sha256:ed1db27f…16e7`; следующий downloaded artifact child-process gate также принят на `0c721f4d…`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Restored-copy production-like acceptance с production HBA/TLS/PgBouncer и controlled revoke/session drain                                                                                                                                                                                                                                                                                     |
...
| `FOUNDER-OWNER-INVITE-LIFECYCLE`        |        P0 | `RESTORED-COPY SMTP+SENT+ACCEPT PASS / PROD NO-GO`                                | Status/revoke принят на `0bc178bc…`; reissue — `f33e598a…`. Canonical CURRENT185 fail-closed перепривязывает active worker к `185` migrations без расширения delivery RPC. На clean production-backup template disposable clone trusted TLS SMTP worker прошёл `1/1` за `173.653 s`: `SENT`, pre-provider RETRY и post-provider ambiguity без blind resend; отдельный full founder clone прошёл activation/enrollment/SENT/accept/disable `1/1` за `55.262 s`. После обоих database/role residue `0`, исходный preflight `READY`. Accept создаёт одного OWNER/NETWORK, переводит tenant в ONBOARDING и claim в USER, очищает ciphertext. Release `3f325acc…`, artifact `9321380247`, SHA `adb75120…b7f4`; production SMTP/roles/migration не менялись                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Production worker-role enrollment/secrets/controlled SMTP canary, Gate 1MT/2 и controlled owner invite                                                                                                                                                                                                                                                                                        |
| `FOUNDER-PILOT-MAIL-TENANT-ENROLLMENT`  |        P0 | `EXACT-SHA + RESTORED-COPY FULL WORKFLOW PASS / PROD NO-GO`                       | Operator CLI выполняет только `plan/apply/check/disable` для одного exact `PILOT` tenant. CURRENT185/head/count, activation release/environment, OWNER/NETWORK invite, worker role/OID и пять RPC, provider digest и fixed policy проверяются под tenant advisory lock в `SERIALIZABLE`; active claim/drift/cross-tenant substitution блокируются, lost commit response reconciles только по полному state equality, disable монотонно повышает revision. Unit `7/7`, CI/focused PG зелёные. На production-backup template отдельный clone прошёл shell→GO→activation→enrollment plan/apply/check→SENT→accept→disable; соседний clone доказал trusted TLS SMTP. Fail-closed HBA и `LOOPBACK_PLAINTEXT_CONNECTION_REQUIRED` сработали до effect; final `ssl=on`, exact HBA rules `3`, residue `0`. Полный отчёт: `founder-restored-copy-mail-rehearsal-2026-08-18.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Production role/TLS/SMTP secrets и controlled canary; Gate 1MT/2 и controlled Tenant B enrollment                                                                                                                                                                                                                                                                                             |
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   ...
   Production attachment trigger/read-only inventory принят; перед
   archive/delete/orphan production-build browser/STORES требуется audited
   reconciliation/backfill и zero-diff. После него остаются browser/STORES,
...
2. Принять readiness, session drain, monitoring и отдельный
   `PRODUCTION DEPLOY GO`; artifact-bound backup/migration/role/network часть
   этого шага уже закрыта.
3. В `PREPARE` создать exact production roles, настроить independent
   encryption/SMTP secrets и выполнить controlled SMTP canary.
4. Controlled cutover, затем Gate 2 на `Tenant A/A1..A4` и минимум семь суток
