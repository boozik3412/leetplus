...
`3/3 SUCCESS`, artifact `sha256:9e0a8a9b…fa62c`. Новый local synthetic provider
даёт `8/8`, но реальный runtime DB client, grants, production roots и route
wiring отсутствуют. Production, route/UI, текущая сеть из четырёх клубов и
первый внешний tenant не изменяются.
...
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `LEDGER CI ACCEPTED + PROVIDER/PRISMA LOCAL / CI PRISMA PENDING / DORMANT`          | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Exact replay после lost register/consume/revoke/expiry response возвращает только тот же request; изменённый replay fail closed. Registration сверяет DB/role OID, negative role attributes, memberships/ownership/default ACL, DB/schema ACL, полный PG16 table/sequence privilege set и ровно 4 callable RPC. Disposable ledger matrix принята на SHA `0578e5e4…`, CI `31688314546` `3/3 SUCCESS`, artifact `sha256:9e0a8a9b…fa62c`. Synthetic provider `8/8` принимает только branded CURRENT193 `SYNTHETIC_CI`, frozen lost-response spec, exact `claim/execute/reconcile`, ambiguity close и zero-inflight drain. Новый Prisma adapter `8/8` создаёт отдельные owner/runtime clients только на loopback CI, повторно сверяет live DB/role/OID и не предоставляет arbitrary SQL; actual disposable-clone CI matrix подключена, но ещё не принята exact SHA. Production entry всегда deny; production credentials/roles/grants не создаются | Принять actual Prisma CI matrix; затем pinned production TLS, lifecycle/rotation и canonical restored-copy rehearsal |
