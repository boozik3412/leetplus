...
- Следующее: расширить источники `/staff/notifications` true shift incidents после стабилизации employee identity в Langame operation/cash logs.
...
- Готово: опубликованные регламенты смен поддерживают обязательное подтверждение сотрудниками по выбранной роли и scope клуба; конструктор показывает required/acknowledged/pending counts, сотрудники могут подтвердить текущую версию, а повторная публикация создает новую версию подтверждения.
- Готово: регламенты смен хранят versioned publication snapshots в `StaffShiftRegulationVersion`; существующие опубликованные регламенты backfilled во время migration, каждая публикация записывает title/scope/sections, а конструктор показывает compact version history.
- Готово: регламенты смен поддерживают attached link materials: document, file, image, video, external URL, note, required flag, редактирование текущего регламента и publication snapshots с attachment counts.
...
- Готово: помощник суммирует регламенты в короткие инструкции смены с шагами briefing и контрольными точками.
...
- Готово: помощник предлагает материал базы знаний, retest или follow-up задачу после повторяющихся ошибок, подбирая уже существующие материалы, курсы и аттестации по смысловым совпадениям.
- Готово: помощник готовит manager summary за период с выполненной работой, просрочками, нарушениями и рекомендуемыми действиями.

Критерии приемки:

- Готово: AI output опционален и никогда не меняет published regulations, tasks или training без явного действия.
- Готово: каждый AI-generated checklist, summary или recommendation требует подтверждения пользователя перед publication или assignment.
- Готово: sensitive staff и guest data минимизируется: первый слой работает детерминированно внутри tenant, не отправляет prompts во внешние AI-провайдеры и не включает гостевые персональные данные.
...
3. Готово: добавлена схема базы данных для задач, шаблонов задач, регулярных правил, комментариев, вложений, аудита и staff assignments. `StaffTask`, `StaffTaskTemplate`, `StaffTaskRecurringRule`, `StaffTaskRecurringRuleRun`, `StaffTaskComment`, `StaffTaskAuditEvent`, `StaffAttachment` и `StaffMember` уже есть; scheduler и журнал автоматических запусков добавлены.
4. Готово: реализовать backend CRUD и list API для задач с tenant/store/staff access control. List/create/status update плюс comment/evidence endpoint уже есть; `/staff/task-rules` добавляет CRUD регулярных правил, ручное создание задач из правила, due-run endpoint и автоматический scheduler.
...
6. Готово: добавить шаблоны чек-листов и запуски чек-листов. `StaffChecklistTemplate`, `/staff/checklist-templates`, `StaffChecklistRun`, `/staff/checklists` и execution report уже есть; runs используют snapshots опубликованных регламентов или active checklist templates.
7. Готово: добавить регламенты, версии, подтверждения и targeting по роли/клубу. Draft/published/archived shift-regulation entity, acknowledgement tracking, publication version snapshots, attached link materials и uploaded file attachments уже есть.
8. Готово: добавить учебные материалы, курсы, тесты и отчеты по аттестациям. База знаний, structured training-material layer, training courses, onboarding plans, tests/attestations с result history, employee training profiles и readiness/attestation manager report уже есть.
9. Готово: добавить аналитику, экспорты и связи с текущими staff-control сигналами. `/staff/operations-dashboard` покрывает операционную дисциплину, рейтинги клубов/сотрудников, повторяющиеся проблемы чек-листов, warning/fine discipline, administrator rating и current risk routing; CSV/XLSX exports, salary/payroll-конструктор и staff-control anomaly connections уже добавлены.
...
- Готово: очередь ручных наград `MarketingMissionReward` фиксирует гостя/домен Langame, клуб, сумму, статус `PENDING/APPROVED/PAID/CANCELED`, согласующего, evidence и source/external поля для будущего импорта.
