...
| `CURRENT188-LANGAME`                 |        P0 | `IMPLEMENTED NONCANONICAL / ACTIVATION + STATUS + RECONCILE + WEB BFF DORMANT`    | foundation `6/6`; API staged service `30/30`, policy/service/role `170/170`, targeted Langame `201/201`; disposable PG stage/replay/cross-tenant/stale/expiry/lost-response smoke `PASS`. Preview/activate/status/reconcile повторно получают persisted role/capability/NETWORK authority через `FreshStoreScopeService`; adapters default-off и production-denied. Store-bound status принят exact SHA `f5b94c2e…`, CI `31668745439` `3/3 SUCCESS`, artifact `sha256:a42d200b…71a5c2`; stale `PENDING` fail-closed становится `EXPIRED`. Local reconcile связывает exact activation request и допускает `ACTIVATED` только при согласованном receipt/claim/Store/source/credential/audit, иначе только доказанные `NOT_APPLIED/EXPIRED`; partial/ambiguous evidence rejected. Exhaustive-unimported Web BFF `17/17`, bounded 8 KiB, safe private projections. Legacy external sync deny принят SHA `9d66276a…`; все provider clubs автоматически создать больше нельзя. Остался initial read-only sync blocker | Принять exact-SHA CI reconcile evidence; canonical consolidation, execute-only runtime role/grants, expirer, production authorization, отдельно одобренный initial read-only sync; atomic Route/UI cutover и browser A/B |
