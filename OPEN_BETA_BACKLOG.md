...
| BETA-IAM-004K | P0        | В работе      | `PROTECTED_MAIL_WORKER_TENANT_ENROLLMENT_V1`              | Slice 1 отделяет `providerAuthorityDigest` от `runtimeConfigDigest`; Tenant B не ротирует authority Tenant A. Slice 2 добавляет неавторизующий read-only preflight на exact `CURRENT_179/179`. Slice 3 создаёт вне canonical chain candidate `20260801010000_identity_mail_tenant_enrollment_control_plane`, SQL SHA-256 `e84ba3c4e9e61d1d759b82a33fc22c853471fb0ef908546e755699d0d264f683`: `DORMANT_SCHEMA_ONLY / NOT_DEPLOYABLE`, static `81/81`, self-test `21` probes, `COMPLIANT`; два независимых PostgreSQL 16.13 smoke — `PASS`, source `CURRENT_179/179` zero-diff, disposable clone/temp cleanup подтверждён, review P0/P1/P2=`0/0/0`. Exact implementation `475e9be4726787db955d895d348af1fc5a7c2db3`, CI `30690147568` (`run #64`) — `3/3 PASS`. Slice 4 добавляет только pure fail-closed Ed25519 verifier enrollment-команды (`12/12`, включая fixture-substituted pinned exact 52-column mapping без production root injection API) и process runtime-attestation (`12/12`): production roots frozen empty; runtime result всегда `authorization=false/canMutate=false/canSend=false` и требует per-tenant DB readiness. Exact tenant lock/drain v2 зафиксирован как `REVIEWED_DESIGN_ONLY`. Canonical target остаётся `CURRENT_179/179`; historical 004J evidence не меняется. Promotion запрещён до реализации worker v2 RPC/API, общего producer/worker tenant advisory lock, retirement legacy `issue_hold_v1/activate_v1`, `DRAINING` zero-secret/zero-inflight barrier, DB-enforced relational rollback, exact routine ACL/role-OID matrix, pre-RPC deadline, independently signed apply/rollback/zero-diff, production-like rehearsal и `SHARED BETA GO`. Deploy, enrollment, SMTP, Tenant B/Store B1, account и invite не выполняются | BETA-IAM-004J, BETA-OPS-002, BETA-OPS-004, BETA-SEC-008..010                |
...
grant и не запускает SMTP. Следующим implementation slice остаётся
неканонический worker v2/producer-lock/drain candidate с PostgreSQL race matrix.
