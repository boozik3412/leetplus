...
function defaultDraft(storeId = ""): DraftRegulation {
  return {
    id: null,
...
export function StaffShiftRegulationBuilder({
  ...
  currentUserRole,
  defaultStoreId = "",
  allowNetworkStore = true,
  canOpenChecklistTemplates = true,
}: {
  ...
  allowNetworkStore?: boolean;
  canOpenChecklistTemplates?: boolean;
}) {
  const router = useRouter();
  const initialDraft = rows[0]
    ? fromRegulation(rows[0])
    : defaultDraft(defaultStoreId);
  const [draft, setDraft] = useState<DraftRegulation>(() => initialDraft);
  ...
  function loadRegulation(row: StaffShiftRegulation | null) {
    const nextDraft = row ? fromRegulation(row) : defaultDraft(defaultStoreId);
    setDraft(nextDraft);
  ...
  function applyTemplate(template: StaffShiftRegulationTemplate) {
    setDraft(draftFromTemplate(template, defaultStoreId));
    setIsConstructorOpen(true);
    setError(null);
  }
  ...
  async function save(statusOverride?: StaffShiftRegulationStatus) {
    ...
    if (draft.requiresAssessmentRetake && !draft.assessmentId) {
      setError(
        "Выберите активный тест или аттестацию для пересдачи после публикации.",
      );
      return false;
    }
  ...
  async function deleteRegulation(row: StaffShiftRegulation) {
    ...
    try {
      ...
      const nextDraft = nextRow
        ? fromRegulation(nextRow)
        : defaultDraft(defaultStoreId);
      setDraft(nextDraft);
      setSavedDraftSnapshot(draftSnapshot(nextDraft));
      router.refresh();
                ...
                <div className="mt-4 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900/40">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                        Версия v{selectedRow.version}. Требуется подтверждение
                        для сотрудников выбранной роли и клуба.
                      </p>
                    </div>
                  ...
                  {selectedRow.requiresAssessmentRetake ? (
                    <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
                      <p className="font-semibold">
                        После этой версии требуется пересдача
                      </p>
                ...
                <div className="mt-4 rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                        Опубликованные снимки регламента. Чеклисты и
                        ознакомления остаются привязаны к своей версии.
                      </p>
                    </div>
                  ...
                  {selectedRow.versions.length > 0 ? (
                    ...
                    <p className="mt-3 rounded-md border border-dashed border-zinc-300 p-3 text-sm text-zinc-500 dark:border-zinc-800">
                      История начнет фиксироваться при публикации следующей
                      версии.
                    </p>
                  )}
                </div>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-[1.5fr_1fr_1fr]">
                ...
                <label className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateDraft({
                        shiftKind: event.target.value as StaffShiftKind,
                      })
                    }
                    className="h-11 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-zinc-700 dark:bg-zinc-950"
                  >
              ...
              <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_1fr_1fr]">
                ...
                <div className="rounded-md border border-zinc-200 bg-zinc-50 p-3 text-sm dark:border-zinc-800 dark:bg-zinc-900/40">
                  ...
                  <p className="mt-1 font-semibold">
                    {totals.sections} разд. · {totals.items} пунктов ·{" "}
                    {totals.score} баллов
                  </p>
                  <p className="mt-1 text-xs text-zinc-500">
                ...
                <div className="grid gap-3 lg:grid-cols-[1fr_1.4fr]">
                  <label className="flex items-start gap-3 rounded-md border border-zinc-200 bg-white p-3 text-sm transition hover:border-emerald-500/60 dark:border-zinc-800 dark:bg-zinc-950">
                    <input
                      ...
                      onChange={(event) =>
                        updateDraft({
                          ...
                          assessmentId: event.target.checked
                            ? draft.assessmentId
                            : "",
                        })
                      }
                    ...
                    <span>
                      ...
                      <span className="mt-1 block text-xs leading-5 text-zinc-500">
                        Новая версия регламента останется с обязательным
                        ознакомлением, а сотрудник увидит связанный тест или
                        аттестацию как следующий контроль.
                      </span>
                    </span>
                  </label>
                  ...
                  <label className="space-y-1">
                    ...
                    <select
                      ...
                      disabled={
                        !draft.requiresAssessmentRetake ||
              ...
              <div className="mt-5 rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                      Прикрепите ссылки на файл регламента, фото-инструкцию,
                      видео или внешний документ. При публикации материалы
                      попадут в снимок версии.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {draft.attachments.length === 0 ? (
                  <p className="mt-3 rounded-md border border-dashed border-zinc-300 p-3 text-sm text-zinc-500 dark:border-zinc-800">
                    Материалов пока нет. Можно оставить пустым или добавить
                    ссылку на актуальный документ, видеоразбор или чек-лист
                    стандарта.
                  </p>
                ) : (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 leading-6">
                    Откройте его только когда нужно изменить структуру
                    регламента, разделы, пункты, материалы или параметры
                    публикации.
                  </p>
                </div>
                <button
