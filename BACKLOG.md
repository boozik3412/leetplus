...
- Done: guest foundation sync now requests `all_operations_log/list` in Langame's `dd.mm.yyyy` date format and additionally probes `operation_type=Списание/Пополнение`, so dashboard revenue can use balance spend facts when Langame returns them.
