...
CURRENT187-J5-R8 реализован локально как actual J4 mTLS fixture candidate.
...
zero-secret receipt. Shell syntax, Node syntax, scoped formatting и diff check
green; actual Ubuntu/PostgreSQL execution и exact-SHA CI ещё не приняты.
Это промежуточный J4 gate, а не co-located J1–J4 topology; внешний доступ
остаётся `NO-GO`.
...
| `CURRENT187-J5-R8`                   |        P0 | `IMPLEMENTED LOCALLY / ACTUAL J4 MTLS CI PENDING / NO PROD EFFECT`                | Disposable fixture creates an ephemeral CA plus separate server/client certificates, enables PostgreSQL server TLS and PgBouncer client/server verify-full, then calls the public actual J4 collector with the PKCS#8 client key. Integration requires strict production-origin brand and proves PEM/raw hashes/passwords are absent from receipt; shell/Node syntax, formatting and diff checks are green                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Execute and accept Ubuntu PostgreSQL/PgBouncer exact-SHA CI; then build one co-located public J1–J4 runner topology; protected production signer/key/root; canonical ledger/runtime roles; restored-copy rehearsal and independent review                  |
