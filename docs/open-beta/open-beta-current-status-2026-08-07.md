...
> aggregate CURRENT187 `78/78`, typecheck green. Exact SHA `ceed7239…`, CI
> `31586755130` — `3/3 SUCCESS`, actual HBA step `SUCCESS`, artifact
> `sha256:faf8c3e2…aa283`. Actual disposable PgBouncer process остаётся следующим
> candidate. Evidence:
...
| CURRENT187-J4 PgBouncer          | `EXACT-SHA CI ACCEPTED / DENY-ONLY`       | simple-query admin `SHOW` collector; global/database/user/runtime transaction mode, backend/TLS mapping, `force_user`/stale mapping checks; `7/7`; collector CI accepted, actual disposable PgBouncer fixture and signer pending                                                               |
...
Ближайший этап — принять actual PgBouncer fixture и connection negative-probe matrix, затем
