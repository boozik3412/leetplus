...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  `sha256:f2cca9b5…d1e41`. Но live production backup/target, rehearsal и SMTP ещё
  обязательны, поэтому release decision
  остаётся `NO-GO`
...
| `FOUNDER-RESTORED-COPY-PREFLIGHT`       |        P0 | `EXACT-SHA CI ACCEPTED / UNIT 6/6 + SYNTHETIC PG READY / LIVE INPUTS REQUIRED`    | Read-only fail-closed CLI проверяет фактические SHA-256 artifact/backup, разные file identities, bounded exact manifest, только `127.0.0.1`/non-5432 и отдельное restored DB name, live PostgreSQL system/database/owner identity, zero other target sessions, exact source migration manifest, отсутствие failed rows и founder activation role. Synthetic PG16.14 (`183` migrations) вернул READY; residue 0. SHA `9caa3e49…`, push/PR CI оба `3/3 SUCCESS`, artifact `9295786786`, digest `sha256:e8cf5a0e…26704`. Output PII/secret/path-free; production, четыре клуба и tester не затрагивались                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Получить immutable production backup + independent checksum, скачать exact CI artifact и isolated target; принять live `READY` receipt; затем выполнить exact role controller и migration apply/replay/rollback/cleanup                                    |
