...
| Gate 1MT tenant/store isolation              | PostgreSQL A/B `35/35 PASS`; report/SSE/OWNER attachment, knowledge, shift и training STORES browser приняты; два staff parents и background остаток открыты |
...
6. Закрыть STORES adoption двух оставшихся network-only staff parent
   families (onboarding и checklists/templates),
   ...
   attachment lifecycle, knowledge, shift-regulations и training STORES
