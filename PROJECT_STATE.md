...
- Added reaction SLA for returned knowledge-base materials: returned articles store `returnedAt`/`revisionDueAt`, show SLA badges in `/staff/knowledge-base`, surface due dates in author notifications, and auto-created revision tasks from approval workflow are filterable through `/staff/tasks?view=approval`.
