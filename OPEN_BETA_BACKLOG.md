...
`27/27`, runner `14/14`. PostgreSQL append-only/RLS/RPC ledger и hostile race
matrix ещё не реализованы, поэтому I остаётся `FOUNDATION / NO-GO`. Production и
...
| `CURRENT187-I`                       |        P0 | `FOUNDATION IMPLEMENTED / POLICY GATE ENFORCED / DB LEDGER PENDING / DENY-ONLY`   | authority receipt now carries pinned-production vs synthetic-loopback provenance; H exposes signed operation/nonce, authority/document timelines and exact root/document/evaluation bindings; canonical consumption and scoped revocation bundles are secret-free and digest-bound; F accepts only branded persisted I receipt and rejects raw/cloned H; focused `31/31`, release acceptance `133/133`, standalone materializer `24/24`, journal `24/24`, runtime `27/27`, runner `14/14`; official sequential rehearsal script green; release/refreeze/assembly chain re-pinned; all launch/effect flags false                                                                                                                                                                                                                                        | Implement append-only PostgreSQL consumption/revocation tables, FORCE RLS, execute-only duty roles, fresh-after-lock RPCs, byte-exact replay and hostile race/expiry/revoke matrix; independent review and exact-head CI                                   |
...
1. Завершить PostgreSQL persisted one-time consumption/revocation/expiry/replay
   ledger для уже реализованного CURRENT187-I foundation; затем закрыть
   CURRENT187 infrastructure admission/provider recovery, exact runtime
