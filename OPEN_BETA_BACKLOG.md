...
`sha256:1db3c8e7…afc7`. Локальный CURRENT194 lifecycle gate теперь `32/32`:
zero-inflight drain, owner-only revoke, exact lost-response replay, terminal
close и disposable credential rotation готовы к exact-SHA CI. Production roots,
route/UI, текущая сеть из четырёх клубов и первый внешний tenant не изменяются.
...
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `BOOTSTRAP EXACT-SHA CI ACCEPTED + LIFECYCLE LOCAL 32/32 / DORMANT`                  | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Separate Prisma adapter и composed signed bootstrap приняты exact SHA `e829f303…`, CI `31694281914` `3/3 SUCCESS`, artifact `sha256:1db3c8e7…afc7`. Новый lifecycle сначала запрещает новые RPC и ждёт zero in-flight, затем выполняет только owner-side exact revoke с lost-response replay и всегда закрывает оба Prisma client; runtime role revoke не получает. Локальный общий gate `32/32`; disposable PG fixture дополнительно ротирует обе credentials, отрицает старые пароли, открывает новый signed bootstrap и проверяет два terminal `REVOKED`/шесть ledger events. Exact-SHA CI этого слоя pending. Production entry всегда deny; production credentials/roots/roles/grants не создаются | Принять lifecycle/rotation CI; затем pinned production TLS, durable double-lost revoke reconciliation и canonical restored-copy rehearsal |
