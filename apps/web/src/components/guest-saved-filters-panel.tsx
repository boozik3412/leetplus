...
import type { GuestListFilters, GuestSavedFilter } from "@/lib/guests";

type GuestSavedFiltersPanelProps = {
  currentFilters: GuestListFilters;
  savedFilters: GuestSavedFilter[];
};

export function GuestSavedFiltersPanel({
  currentFilters,
  savedFilters,
}: GuestSavedFiltersPanelProps) {
  ...
  async function saveFilter() {
    ...
    try {
      const response = await fetch("/api/guests/saved-filters", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        setError(payload?.message ?? "Не удалось сохранить фильтр");
        return;
      }
  ...
  async function deleteFilter(id: string) {
    ...
    try {
      const response = await fetch(`/api/guests/saved-filters/${id}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        ...
        setError(payload?.message ?? "Не удалось удалить фильтр");
        return;
      }
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold">
            Быстрые сегменты отчета
          </h2>
        </div>
        <div className="grid gap-2 sm:grid-cols-[minmax(12rem,1fr)_auto] lg:w-[32rem]">
          <input
            value={name}
            onChange={(event) => setName(event.target.value)}
            maxLength={80}
            placeholder="Например: VIP без визита 14 дней"
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-zinc-500">
                  {filterSummary(filter.filters)}
                </p>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                <Link
                  href={reportHref(filter.filters)}
                ...
                >
                  {deletingId === filter.id ? "Удаляю..." : "Удалить"}
                </button>
              </div>
            </article>
          ))}
