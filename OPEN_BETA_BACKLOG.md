...
| `CURRENT194-LANGAME-ATTESTATION-LEDGER` |     P0 | `LIFECYCLE EXACT-SHA CI ACCEPTED / DORMANT`                                          | Checksum-bound successor SQL `67a03177…` хранит owner-only register, one-time runtime consume, terminal revoke/expiry и append-only events. Separate Prisma adapter и composed signed bootstrap приняты exact SHA `e829f303…`. Новый lifecycle сначала запрещает новые RPC и ждёт zero in-flight, затем выполняет только owner-side exact revoke с lost-response replay и всегда закрывает оба Prisma client; runtime role revoke не получает. Общий gate `32/32`; disposable PG fixture ротирует обе credentials, отрицает старые пароли, открывает новый signed bootstrap и проверяет два terminal `REVOKED`/шесть ledger events. Exact SHA `01cd9e45…`, CI `31697571300` `3/3 SUCCESS`, artifact `sha256:2062ba99…b271`. Production entry всегда deny; production credentials/roots/roles/grants не создаются | Pinned production TLS, durable double-lost revoke reconciliation и canonical restored-copy rehearsal |
