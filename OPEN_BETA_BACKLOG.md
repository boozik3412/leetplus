...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `29/29`, включая staff attachments и
  ...
  Production roles/SMTP canary, остальные file parents, job/SSE/Telegram
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `29/29`: assortment `15/15`, chat/CRM/users и `PENDING/BOUND` staff attachments; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Exact `94db1fdd…` добавил atomic RecommendationState concurrency и bounded SSR fan-out; два restored-copy run дали `15/15 + 15/15`, hardened report BFF acceptance `9/9`, production-build report/download/mutation browser journey, whole-schema `156`-table postflight и zero database residue. До готовности остаются file parents, job/SSE/Telegram negative matrix пяти product modules и supporting `INTEGRATIONS`, controlled outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 29/29 + REPORT BROWSER PASS`                                    | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На CURRENT185 production-backup restored-copy clones выполнены assortment HTTP `15/15`, team-chat `3/3`, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`, attachments `3/3`: суммарно `29/29 PASS`; fixture/core/database residue нулевой. Exact `94db1fdd…` дважды прошёл `15/15`, hardened BFF acceptance `9/9`; production-build browser принял report reads, CSV/XLSX downloads, recommendation `PATCH`, OOS `POST→DELETE`, console `0/0` и whole-schema `156`-table postflight без unexpected deltas. Database residue `0`. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`, `gate-1mt-report-browser-evidence-2026-08-18.md`                                                                                                                                  | Store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram/remaining files/SSE, controlled outbound email/digest                                                                                                   |
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `29/29`, report browser,
   trusted TLS SMTP и protected enrollment/SENT/accept/disable уже приняты.
   Остались file parents, jobs/SSE/Telegram/public guest binding и controlled
