...
- Protected initial OWNER invite lifecycle начат: PII-free status и атомарный
  revoke реализованы под Platform Admin guard с tenant lock, fresh DB authority,
  exact invite CAS, outbox cancellation/evidence preservation, claim release и
  idempotent audit replay. Локально `2 suites / 14 tests`, typecheck и scoped
  lint зелёные; PostgreSQL trigger acceptance встроен в existing activation
  fixture и ожидает exact-SHA CI. Reissue создаёт только новый immutable token
  и остаётся следующим P0; blind resend запрещён
...
| `FOUNDER-OWNER-INVITE-LIFECYCLE`        |        P0 | `STATUS + REVOKE IMPLEMENTED / EXACT-SHA CI PENDING / PROD NO-GO`                 | Platform Admin получает PII-free status и exact-confirmed revoke. Common tenant lock, fresh DB authority, invite CAS, pre-provider outbox cancellation/ciphertext clearing, post-provider evidence preservation, IdentityEmailClaim release и request-digest audit replay выполняются атомарно. Local `2 suites / 14 tests`, typecheck и scoped lint PASS; PostgreSQL acceptance добавлен в founder activation fixture                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Принять exact-SHA CI; реализовать новый immutable reissue/outbox без blind resend; затем SMTP SENT/reissue/accept canary                                                                                               |
