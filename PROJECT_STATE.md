...
- Inside-team communication: `/staff/team-chat` is the first tenant-scoped operational feed for staff channels, announcements, incidents, pinned messages, read receipts, and quick creation of staff tasks from chat messages without realtime infrastructure yet.
...
- Started the inside-team operational feed: `StaffChatChannel`, `StaffChatMessage`, `StaffChatReadReceipt`, API `/staff/team-chat`, web proxies, sidebar entry, and `/staff/team-chat` workspace for channels, messages, pins, read state, club/role context, and creating staff tasks from messages.
