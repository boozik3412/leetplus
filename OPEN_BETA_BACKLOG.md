...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `26/26`, включая staff attachments и
  расширенный assortment `12/12`;
  ...
  Итоговая матрица `12/12` доказывает NETWORK-only category/supplier mutations,
  tenant-bound product/inventory/sales/movement CSV imports, store-scoped
  все local report variants, CSV/XLSX exports без foreign rows, OOS/recommendation
  state, cross-tenant и stale-scope deny. Два последовательных прогона exact
  ...
  core counts совпали `3/4/30/1483/51257`, database residue `0`. Production
  roles/SMTP canary, HTTP/BFF/browser report/export boundary, остальные file parents,
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `26/26`: assortment `12/12`, chat/CRM/users и `PENDING/BOUND` staff attachments; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Latest assortment suite на `f59c32fc…` покрывает category/supplier NETWORK-only mutations, tenant-bound product/inventory/sales/movement CSV imports, все local report variants, CSV/XLSX exports и OOS/recommendation state, cross-tenant и stale-scope deny; exact bytes дважды прошли на clean current-head PG и restored-copy clone с zero residue и exact core-count equality. До готовности остаётся полная HTTP/BFF/browser mutation/export/file matrix, file parents, job/SSE/Telegram negative matrix пяти product modules и supporting `INTEGRATIONS`, outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 26/26 PASS / HTTP+BROWSER IN PROGRESS`                          | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На CURRENT185 production-backup restored-copy clones выполнены assortment `12/12`, team-chat `3/3`, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`, attachments `3/3`: суммарно `26/26 PASS`; fixture/core/database residue нулевой. Latest assortment exact bytes `f59c32fc…` дважды прошли на clean current-head PG и restored-copy clone; category/supplier mutations, product/fact CSV imports, все local report variants, CSV/XLSX exports и OOS/recommendation state имеют residue `0/0/0/0/0/0/0` и exact source/target core-count equality. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`                                                                                                                                                                | Расширить HTTP/BFF/browser report/export matrix; store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram/files/SSE, outbound email/digest                                                                       |
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `26/26`, trusted TLS SMTP
