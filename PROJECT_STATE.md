...
- Communications: sidebar has a separate `Коммуникации` block and `/communications` overview hub for internal team chat, critical notifications, and CRM contact tasks. Existing operational routes remain stable: `/staff/team-chat` is the tenant-scoped feed for default information/support/general channels, per-club channels, custom employee channels, announcements, incidents, pinned messages, read receipts, quick task creation from chat messages, and automatic incident posts from failed checklist submissions; `/staff/notifications` remains the internal notification center; `/guests/crm/tasks` remains the guest CRM contact task workspace. A future backlog step tracks a separate `view_communications` capability.
- Navigation UX: `/communications` and `/staff` are overview hubs, sidebar product groups stay at major business areas, legacy guest, assortment, management, administration, sync/import/directory/utility pages use the same breadcrumbs pattern from the main dashboard to the current working screen, and communication CTAs/pages respect the existing `view_staff`/`view_guests` split until a dedicated `view_communications` permission is introduced.
