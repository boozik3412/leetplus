...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `31/31`, включая real HTTP team-chat SSE,
  ...
  unit `26/26`, restored-copy `4/4 + 4/4`, текущий Gate 1MT — `31/31`,
  `156` table counts zero-diff и database residue `0`.
  Production roles/SMTP canary, native binding writers пяти parent kinds,
  file browser/race, job/Telegram
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `31/31`: assortment `15/15`, team chat `4/4` с real HTTP SSE pre-header deny, CRM/users и staff attachments `4/4` со всеми семью reader parent kinds; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Exact `94db1fdd…` добавил atomic RecommendationState concurrency и bounded SSR fan-out; два restored-copy run дали `15/15 + 15/15`, report BFF `9/9`, production-build report/download/mutation browser journey, whole-schema `156`-table postflight и zero database residue. Exact `ccf81a28…`/`dfe5e0f8…` закрыл team-chat SSE BFF/API boundary. Exact `abb8a667…` закрыл fresh NETWORK/capability/exact-parent read для checklist/knowledge/regulation/training/onboarding attachments: unit `26/26`, PG `4/4 + 4/4`, zero table diff/residue. До готовности остаются native binding writers пяти parent kinds, file browser/race, job/Telegram/public-guest negative matrix пяти product modules и supporting `INTEGRATIONS`, controlled outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
18. Реализовать `BETA-MT-007..009`: полную shared PostgreSQL/API/BFF/browser/
    ...
    PostgreSQL foundation `31/31`, включая все семь attachment reader parent
    kinds и real HTTP SSE, и browser read/admission slice на `771bbd5f…` уже
    приняты; следующий срез — native attachment writers/STORES policies и file
    browser/races, затем background jobs/Telegram/public guest binding.
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `31/31`, report и SSE browser,
   trusted TLS SMTP и protected enrollment/SENT/accept/disable уже приняты.
   Остались native attachment writers/browser/races, jobs/Telegram/public guest
