...
- Guest module: first production read-only layer is live, including data foundation sync, guest analytics dashboard with retention v1, visits heatmap v1, LTV v1, full guest report with RFM v1 and churn risk v1, guest card, mini CRM fields, first staff-control report, and PC-count based load calculation.
...
- Reinforced the standalone dashboard sidebar state: entering `/dashboard` clears previously opened hover groups, so the home icon remains the only highlighted navigation entry after old assortment links, filter changes, or cross-section navigation.
