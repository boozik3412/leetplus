...
fail closed; standalone `10/10`, общий CURRENT187 gate `42/42`. J не имеет
production root/signer и не выполняет actual TCP/TLS/HBA/PgBouncer I/O, поэтому
решение по доступу не изменилось.
...
| `CURRENT187-J`                       |        P0 | `IMPLEMENTED LOCALLY / SYNTHETIC-CI-ONLY / DENY-ONLY`                            | exact `APPLICATION/COORDINATOR/MIGRATION/WORKER` probe matrix; separate branded network and host-control receipts; deterministic endpoint/TLS/HBA/pooler/service-account digests; `VERIFY_FULL`, safe HBA auth, SESSION lock/migration paths, positive+negative probes and pairwise distinct service mappings enforced; clone/proxy/accessor/sparse/cross-release/trust/wildcard/user-collapse hostile matrix `10/10`, aggregate CURRENT187 `42/42`; all access/effect flags false                                                                                                                                                                                                                                                                                                                                                                                                                 | Actual capability-bearing TCP/TLS/HBA/PgBouncer collectors; independent host signer/root; freshness/reload epoch and persisted consumption/revocation; hostile real topology matrix; exact binding into F/deploy authority; independent review and exact-SHA CI |
