...
- Protected initial OWNER invite status/revoke принят на exact SHA
  ...
  Reissue engineering реализован следующим P0: новая canonical migration 184,
  immutable predecessor-chain, новый encrypted invite/outbox/token, exact
  replay и запрет blind resend. Локально API/typecheck/lint, `3 suites / 20`
  и static migration `4/4` зелёные; exact-SHA PostgreSQL CI reissue ещё ожидает
...
| `FOUNDER-OWNER-INVITE-LIFECYCLE`        |        P0 | `STATUS + REVOKE EXACT-SHA ACCEPTED / REISSUE LOCAL PASS / REISSUE CI PENDING / PROD NO-GO` | Status/revoke принят на `0bc178bc…`, PR CI `32084166159` — `4/4 SUCCESS`. Reissue создаёт только новый encrypted OWNER invite/outbox/token, фиксирует immutable predecessor-chain, выполняет exact replay и запрещает blind resend. Common tenant lock, fresh DB authority, invite CAS, pre-provider cancellation/evidence preservation и IdentityEmailClaim replacement выполняются атомарно. Local typecheck/lint, focused `3 suites / 20`, envelope boundary `1/50`, static migration `4/4` и full API `157 suites / 3142 tests` PASS                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Принять migration 184 и reissue PG fixture exact-SHA CI; затем restored-copy SMTP `SENT` и reissue/accept canary                                                                                                                                           |
