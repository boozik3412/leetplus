...
| Покупка/аренда из каталога | LIVE `PRODUCT_EXPENSE` | Нужны guest binding и mapping; scheduler резервирует purchase-слоты и дренирует pending facts. Ledger остаётся SHADOW до reconciliation отмен/возвратов |
...
### Инварианты повторного выполнения и накопления

- `maxPendingRewards` задаётся отдельно для миссии/лутбокса и ограничивает число pending ordinary rewards либо живых `AVAILABLE|OPENING` entitlement от одного источника. Значение по умолчанию для новых и мигрированных существующих элементов — `1`; явно сохранённая операторская настройка имеет приоритет.
- Завершённая DAILY/WEEKLY/MONTHLY миссия начинает новый прогресс только на следующей club-local границе соответствующего периода. До границы summary остаётся в completed-state. Незавершённая задача не сбрасывается только из-за смены дня.
- Повторяемая миссия без периодичности начинает новый цикл после завершения, но события, уже использованные завершённым циклом, исключаются по physical progress identity и времени completion. Новая награда требует нового физического факта.
- Значение прогресса в summary и UI ограничивается целью. Сверхцелевые факты могут оставаться в аудите, но пользователь видит `цель/цель`.
- Модалка задания и `/game/rewards` не создают разные награды: оба интерфейса передают ID одного wallet item в exact claim/open endpoint. Повторный exact claim идемпотентен, а обновлённый summary убирает действие из обоих мест.
...
- Ручное открытие entitlement выполняется exact-маршрутом `POST /guest-portal/session/reward-wallet/items/:walletItemId/open`. Он принимает идентификатор wallet item, повторно проверяет tenant, профиль, сохранённый исходный клуб, 30-дневный срок и точный `AVAILABLE` entitlement, после чего открывает связанный lootbox один раз. Совместимый маршрут `/session/loot-boxes/:lootBoxId/open` использует ту же entitlement-authoritative проверку: legacy unlock event сам по себе открыть контейнер не может. Для события внутри текущего 30-дневного окна разрешён bounded backfill, который сначала создаёт entitlement и wallet item; событие на границе day 30 или старше завершается fail-closed.
- После материализации живой попытки entitlement является доказательством уже пройденной квалификации. При exact open wallet item и entitlement атомарно резервируются состоянием `OPENING`; оно является authoritative lock для параллельных запросов и опасных административных изменений. При открытии повторно не проверяются текущие статус и область клубов шаблона, аудитория, временное окно, session/tariff-условия и issuance limits: иначе несколько честно накопленных прав блокировали бы друг друга. Сохраняются проверки точного владельца, tenant/profile/source store, live entitlement/wallet, существующего исходного rule, единственного random roll и идемпотентного связывания reward с consumed entitlement. Незавершённый `OPENING` восстанавливается контролируемым stale-recovery, а не вторым roll.
- Кошелёк профиля общий внутри tenant, а wallet item сохраняет `storeId`. Exact open по `walletItemId` сам использует сохранённый исходный клуб entitlement, включая ставший неактивным клуб, и не меняет выбранный клуб или session cookie гостя. Уже выданное право не переквалифицируется по текущему статусу, области клубов, расписанию и лимитам шаблона; шаблон должен существовать, а wallet item и entitlement должны быть точными, неистёкшими, неиспользованными и принадлежать профилю. Автоматическое открытие при переключении, загрузке страницы или `claim-all` запрещено.

- Событие ручного открытия по сохранённому entitlement получает внутреннюю квалификацию `PREQUALIFIED_LOOT_BOX_ENTITLEMENT`. Даже если исходное право хранит `HOURLY`/`regular_session`, защитный контур generic-session remediation не должен принимать открытие кейса за новый session fact и блокировать случайный приз как `SOURCE_FACT_MISSING`. Эта квалификация формируется только серверным exact-open контуром; обычные LIVE/Ledger session-события продолжают проходить полную проверку классификации.
...
- При ненулевом `pendingCount` основное состояние кошелька показывается в карточке профиля и `/game/rewards`; дублирующий индикатор над `Профиль активен` не используется. В карточках источников отражается локальный статус.
...
- История наград является вторичным действием кошелька и показывает компактный timeline `WAITING_CLAIM`, `DELIVERY_PROCESSING`, завершённых и истёкших результатов. Пользователь может фильтровать по клубу, источнику, редкости и поиску; внутренний игровой журнал и claim code обычному гостю не показываются.
- Правая панель держит компактный список заданий, а «Открыть все» загружает полную доску через paginated endpoint. Лимит компактной панели не должен скрывать остальные активные задания.
...
- `/guests/sessions` не отдаёт историю тарифных сегментов: у физической сессии есть только текущий/итоговый `packet` и `expand`, без времени и направления смены. Поэтому `packet=null` и `expand=true` с итоговым почасовым marker нормализуются как неизвестный тип и подходят только `ANY`. Продление незавершённого пакета/абонемента не создаёт новый почасовой старт и не открывает `HOURLY`-кейс; typed-минуты такой сессии не делятся без отдельного структурированного segment event. Replay gate `hourly-session-expand-v1` заставляет каждый Ledger-источник один раз перечитать своё историческое окно и сохраняет прежнюю parser identity v4, чтобы не менять fact ID и origin receipt для неизменившихся строк.
- Перед materialization старое typed-событие повторно сверяется с remediation marker, точным Ledger-фактом и, для LIVE/snapshot, заблокированной строкой `GuestSession`. Ложная классификация без применённых эффектов нейтрализуется; неоднозначное событие с уже созданными эффектами переводится в `RECONCILIATION_REQUIRED`, при этом `APPLIED` reward/effect не отзываются автоматически. При rolling rollout сначала останавливают новые claims или ждут окончания прежних lease, затем дают первому replay/remediation drain завершиться и только после этого оценивают новые выдачи.
...
- Purchase condition принимает `ANY_PRODUCT`, выбранные товары или категории. В пользовательском тексте перечисляются конкретные категории, а служебный `windowDays` не выводится. Аренда/услуга учитывается, если Langame отдаёт её как положительный guest-bound `PRODUCT_EXPENSE` с подходящим product/category mapping.
- Scheduler резервирует до трети unscoped bounded batch под `PRODUCT_EXPENSE`, добирает pending покупки за пределами самого свежего окна и поднимает совпадающие с активными category-миссиями факты. Это остаётся одним последовательным LIVE handler, а не вторым параллельным purchase processor.
- Для миссий и лутбоксов редакторы показывают `maxPendingRewards`; API повторяет проверку атомарно перед выдачей.
- `REWARD_TEMPLATE` скрыт из витрины, `STANDALONE` не допускается как награда mission/Battle Pass, `BOTH` разрешён в обоих сценариях. Backend отклоняет сохранение Battle Pass с неподходящим case rule.
- Mission cover загружается в tenant media store, поддерживает crop/scale и сохраняется при последующем редактировании. Wizard preview и production card используют один presentation contract.
- Prize visual лутбокса поддерживает preset/custom image, frame/text/background colours и увеличенный visual с 4 px inset. Редактор roulette preview и игровая roulette используют один payload; клиент preloads images и применяет fallback, а открытая награда хранит visual snapshot.
...
12. Для повторной миссии выполнить один цикл, забрать награду из одной поверхности и убедиться, что вторая поверхность обновилась без второй выдачи. Затем проверить новый period boundary и доказать, что старый matched fact не участвует в следующем цикле.
13. Для purchase mission создать burst других snapshot-фактов и подтвердить, что guest-bound `PRODUCT_EXPENSE` всё равно входит в reserved batch, а category-matched pending purchase дренируется без generic backfill flags.
14. Для `maxPendingRewards=1` оставить одну награду незабранной и подтвердить блок новой квалификации; после claim/open новый физический факт должен создать следующий цикл, старый факт — нет.
