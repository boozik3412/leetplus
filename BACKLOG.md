...
- Planned: add preview/test mode directly inside employee-template builders: when creating shift regulations, checklist templates, task templates, training/onboarding steps, knowledge-base articles, and other employee-facing materials, managers should be able to open a "view as administrator/employee" sandbox preview, run through the flow with test answers/evidence, and validate wording, required fields, scoring, evidence, and employee-facing layout without logging in under an administrator account or creating real operational facts.
