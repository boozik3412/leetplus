...
- Gate 1MT staff STORES successor: knowledge (`085f8bbd…`), shift regulations
  (`6ce36a41…`) и training (`40a8e828…`) приняты на restored-copy и
  production-build B1↔B2 browser. Training включает unit `54/54`, Web BFF
  `14/14`, pilot HTTP `160/160`, PostgreSQL `11/11`, hidden foreign mutation
  `404` и forbidden foreign Store filter `403`. Остались две network-only
  parent families: onboarding и checklists/templates; общий release decision
  остаётся `NO-GO`.
