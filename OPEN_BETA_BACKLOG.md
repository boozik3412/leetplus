...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  прошли `1/1 + 1/1` с zero residue. Gate 1MT PostgreSQL A/B matrix на отдельном
  клоне той же clean copy прошла `14/14`, fixture/database residue `0`.
  Production roles/SMTP canary, HTTP/BFF/browser/background остаток Gate 1MT,
  Gate 2 и cutover ещё обязательны, поэтому release decision остаётся `NO-GO`
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 14/14 PASS / HTTP+BROWSER IN PROGRESS`                          | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На отдельном CURRENT185 production-backup restored-copy clone выполнены assortment `3/3`, team-chat `3/3`, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`: суммарно `14/14 PASS`; fixture tenants/users `0`, core row counts совпали с source, disposable database residue `0`. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`                                                                                                                                                                                                                                                                                                                                                                                                                                    | Расширить HTTP/BFF/browser matrix; store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram/files/SSE                                                                                                            |
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `14/14`, trusted TLS SMTP
