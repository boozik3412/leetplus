...
test("pins terminal CURRENT_190 and exactly five CURRENT_176 worker RPCs", () => {
  assert.equal(
    IDENTITY_MAIL_WORKER_ENROLLMENT_MIGRATION,
    "20260908090000_initial_owner_invite_link_mode",
  );
  assert.equal(IDENTITY_MAIL_WORKER_ENROLLMENT_MIGRATION_COUNT, 190);
  assert.deepEqual(
...
test("requires an exact apply confirmation bound to role OID and head", () => {
  ...
  assert.match(confirmation, /initial_owner_invite_link_mode 190$/u);
});
