...
one-file control `1ea4f39a…` установлен и проверен, но первый plan остановлен
E147 на штатной dangling systemd dependency-link до plan record/effect. Узкая
source-коррекция ожидает новую root fixture/CI, admission, install и GO. Schema
effect отсутствует.
...
| Release acceleration        | five-phase rollout terminal; V3 и trusted lane metrics merged; independent production-control gates выполняются bounded batch с отдельными logs/exit codes. Historical capacity retirement terminal; exact one-file control `1ea4f39a…` installed, E147 plan fail-closed before record/effect, narrow successor pending. Public/corporate/worker контуры нельзя объединять или понижать ради скорости                                                                                           |
...
Systemd dependency symlink проверяется по normalized target. Existing target
может вести только внутрь compiled unit-load roots или в `/dev/null`. Dangling
`.wants/.requires`-подобная ссылка допустима не по имени каталога, а только когда
её полная normalization остаётся внутри тех же roots: systemd затем вправе
разрешить unit basename через load path. Escaping, malformed, меняющаяся ссылка
или ссылка на удаляемый archive остаётся fail-closed. E147 выявил ровно одну
штатную dangling generated-ссылку; production systemd вручную не меняется.
