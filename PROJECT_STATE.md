...
- Hardened the standalone main dashboard navigation: `/dashboard`, root, query-string, trailing-slash, nested dashboard and mobile menu variants resolve to `Главная`, the home icon is always shown to authenticated users, while `Ассортимент` remains active only for explicit assortment routes; legacy `skuGrouping` dashboard links now redirect to a clean main dashboard URL.
