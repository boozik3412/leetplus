...
| BETA-IAM-004K | P0        | В работе      | `PROTECTED_MAIL_WORKER_TENANT_ENROLLMENT_V1`              | Slice 1 отделяет tenant-independent `providerAuthorityDigest` от полного `runtimeConfigDigest`; Tenant B не ротирует authority Tenant A. Slice 2 добавляет только неавторизующий `PROTECTED_MAIL_WORKER_TENANT_ENROLLMENT_PREFLIGHT_V1`: bounded canonical proposal загружается до Prisma, evidence читается одной `READ ONLY REPEATABLE READ` транзакцией, проверяются exact database/role name+OID, `CURRENT_179/179`, пять worker RPC и hostile ACL, tenant-scoped enrollment/drain, текущий release marker, независимо настроенные provider authority/policy; отчёт всегда `authorization=false`, `canMutate=false`, а signature, runtime-config digest, state/event migration, persisted replay и apply/rollback явно deferred. До `Готово` остаются additive CURRENT_180 state/command/event migration, signed admission, persisted resume/finalize, apply/rollback/zero-diff, PostgreSQL race/drain matrix и production-like rehearsal. Production registry остаётся пустым; deploy, enrollment, SMTP и invite не выполняются | BETA-IAM-004J, BETA-OPS-002, BETA-OPS-004, BETA-SEC-008..010                |
