...
Сводка launch readiness на 12.08.2026: Gate 0 закрыт exact synchronized SHA,
...
CURRENT187-J5-R9 реализован локально как кандидат единого public-collector
runner. J2 и negative-probe runner теперь принимают exact bounded client
certificate + PKCS#8 key только в production path, проверяют отдельные SHA-256
bindings и не возвращают credential bytes/hashes. Disposable fixture собирает
strict J1/J2 для четырёх service purpose, strict J3/J4 и передаёт их в
production runner для `4 + 20 + 12` проверок в одном процессе; временный HBA,
roles и hostname mapping очищаются fail closed. Локально: runner `11/11`, wire
`2/2`, J2 `10/10`, J2 TLS `1/1`, aggregate CURRENT187 `126/126`, typecheck,
refreeze `17/17`, assembler `21/21` green. До exact-SHA GitHub CI `3/3 SUCCESS`
и SHA-bound artifact это `CANDIDATE`, не acceptance evidence; production и
внешний доступ остаются `NO-GO`.
...
| `CURRENT187-J5-R9`                   |        P0 | `IMPLEMENTED LOCALLY / CO-LOCATED PUBLIC CHAIN / CI REQUIRED / NO PROD EFFECT`    | J2 and production negative runner accept exact bounded client certificate + PKCS#8 key with separate SHA-256 bindings; synthetic paths require exact nulls. One disposable fixture gathers strict public J1/J2 for four purposes plus strict J3/J4 and invokes the production runner for `4 positive + 20 network negative + 12 control negative`; HBA/roles/host mapping have scoped cleanup. Local runner `11/11`, wire `2/2`, J2 `10/10`, J2 TLS `1/1`, aggregate CURRENT187 `126/126`, typecheck, refreeze `17/17`, assembler `21/21` green                                                                                                                                                                                                                                                                                                                                          | Accept exact-SHA Linux CI `3/3` and artifact; then protected production signer/key/root, canonical ledger/runtime roles, restored-copy rehearsal and independent review                                                                                    |
...
| `CURRENT188-LANGAME`                 |        P0 | `IMPLEMENTED NONCANONICAL / ACTIVATION + WEB BFF DORMANT`                         | foundation `6/6`; API `180/180`; disposable PG stage/replay/cross-tenant/stale/expiry/lost-response smoke `PASS`; exact HMAC-bound activation adapter default-off и без автозапуска sync; exhaustive-unimported Web BFF candidate `12/12` принимает только исходный same-origin Request, exact preview/activate POST без query/hash, cookie-only JWT и bounded 8 KiB stream; требует exact Nest `201 application/json`, связывает ACTIVATE response с request `receiptId` и возвращает safe projection; отсутствующие status/reconcile/initial read-only sync объявлены blockers, generic sync не подставляется                                                                                                                                                                                                                                                                          | Canonical consolidation, execute-only runtime role/grants, expirer, exact status/reconcile API и отдельно одобренный initial read-only sync; atomic Route/UI cutover и browser A/B                                                                         |
...
| `GATE-1MT-STORE-SCOPE`               |        P0 | `IN PROGRESS — ASSORTMENT + B2B GAME + STAFF + COMM`                              | fresh DB resolver; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`, включая 60 exact NETWORK-guarded routes; communications `18/18`: прежние 10 re-attest NETWORK/STORES, новые 8 CRM contact-task routes fresh NETWORK-only; manifest `15 suites/154 tests`; exact CURRENT179 PG: assortment A/A1/A2↔B/B1 `1 suite/3 tests`, team-chat `1 suite/3 tests`, CRM communications `1 suite/4 tests`; residue нулевой                                                                                                                                                                                                                                                                                                                                                                                                                                              | Расширить PG/browser matrix; store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, users/roles, jobs/Telegram                                                                                                               |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   CI с hostile PostgreSQL fixture уже принят. CURRENT187-J1 actual backend
   session collector и CURRENT187-J2 endpoint/TLS-peer collector приняты
   exact-SHA CI. CURRENT187-J3 HBA/reload и J4 PgBouncer collectors приняты
   exact SHA `ceed7239…`, CI `31586755130`; actual disposable PgBouncer принят
   SHA `b9296430…`, CI `31591848857`. J5 signed matrix contract принят exact-SHA
   CI `31594459396`. Capability-bearing runner foundation принят exact-SHA CI
   `31597872402` на `e4789e29…`:
   runner `10/10`, J5 `20/20`, actual wire/TLS integration `1/1`, aggregate
   CURRENT187 `99/99`. J5-R2 signer foundation локально принят `6/6`, combined
   J5 `26/26`, aggregate CURRENT187 `105/105`. J5-R3 ledger contract локально
   принят `6/6`; noncanonical PostgreSQL candidate статически принят `7/7`,
   combined J5 `39/39`, aggregate CURRENT187 `118/118`. Следующим шагом принять
   exact-SHA hostile replay/race/lost-response PostgreSQL fixture и независимую
   latest-byte проверку; параллельный обязательный путь
   — branded J1–J4 production-like 36-outcome run, затем external key
   ceremony/ACL-HSM и reviewed root enrollment, после чего закрыть CURRENT187
   infrastructure admission/provider recovery,
   exact runtime roles/grants/attestation,
   canonical promotion и head-pinned application readiness.
