...
- На 17.08.2026 v2 atomic activation реализован локально: current clean
  ...
  `sha256:f2cca9b5…d1e41`. Но live production backup/target, downloaded artifact
  child process, rehearsal и SMTP ещё обязательны, поэтому release decision
  остаётся `NO-GO`
- Dedicated activation Prisma pool admission реализован локально: перед каждым
  ...
  закрыт; полный child process из скачанного artifact остаётся следующим
  барьером
...
- Downloaded artifact child-process fixture реализован: новый зависимый CI job
  ...
  residue. Exact-SHA CI этого следующего gate ещё pending
...
| `FOUNDER-ACTIVATION-POOL-API`           |        P0 | `EXACT-SHA CI ACCEPTED / UNIT 60/60 + POSTGRESQL HTTP PASS / PROD NO-GO`          | Dedicated activation Prisma pool fresh-attest'ит exact session role/database/TLS в каждой транзакции; production ACTIVE требует `sslmode=verify-full`. Production database service и настоящий HTTP `AdminController` дали `ACTIVATED→REPLAYED` и zero residue. SHA `51995635…` принят push/PR CI `32068262701`/`32068266758` attempt 2 как `3/3 SUCCESS`; artifact `9301062934`, digest `sha256:ed1db27f…16e7`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Полный child process из downloaded artifact, затем restored-copy production-like acceptance и controlled revoke/drain                                                                                              |
| `FOUNDER-RUNNABLE-RELEASE-ARTIFACT`     |        P0 | `EXACT-SHA CI ACCEPTED / HYDRATION PASS / PROD NO-GO`                             | SHA-bound tar содержит API/Web runtime, exact package manifests/lock, 183 migrations и шесть founder operational files; исключены cache/dev/symlink/node_modules и test/key fixtures. CI до upload проверил outer/inner SHA-256, offline frozen production install, Prisma generate, CLI/Nest/Prisma resolution и web BUILD_ID. SHA `90a94f1b…` принят push/PR CI `32075030815`/`32075035388` как `3/3 SUCCESS`; artifact `9303394475`, `28 419 842` bytes, digest `sha256:b73c932f…d5fd`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Скачать этот artifact в отдельном job/host и принять полный API child process; затем использовать тот же механизм на immutable restored copy                                                                         |
| `FOUNDER-ARTIFACT-API-CHILD`            |        P0 | `ENGINEERING IMPLEMENTED / EXACT-SHA CI PENDING / PROD NO-GO`                     | Зависимый CI job скачивает artifact, повторно проверяет SHA-256, гидратирует prod dependencies, создаёт disposable PG database и activation role, запускает artifact `dist/main.js`, проверяет version/readiness, JWT/PlatformAdmin guards и HTTP `provision→GO→ACTIVATED→REPLAYED`, затем требует zero database/role residue и отсутствие PII/secrets в response/child output                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Принять exact SHA/CI; повторить на immutable production-backup restored copy с production HBA/TLS/PgBouncer и session drain                                                                                           |
