...
| BETA-IAM-004A | P0        | В работе      | Реализовать initial OWNER identity outbox                 | Migrations 167..171 дают case-insensitive `IdentityEmailClaim`, persisted provenance/revocation, immutable privacy-safe activation locator, dormant atomic `NETWORK OWNER` issue и encrypted `HOLD` outbox при zero runtime sealed-table privileges; runtime allowlist остаётся seven-RPC, issue RPC — `EXCLUDED_PENDING`. Legacy reconciliation выделен в `BETA-IAM-004B`, writer isolation — в `004D`, transport — в `004E`, locator — в `004F`, dormant HOLD — в `004G`, signed admission — в `004H`, atomic activation/release — в `004I`; до готовности initial OWNER остаются persisted GO, controlled `HOLD→PENDING`, verified delivery, activation/trial и protected reissue/resend; до deploy настраиваются отдельные production HMAC/encryption/signing secrets; raw email/token/URL/ciphertext отсутствуют в responses/logs/audit | BETA-SEC-008, BETA-SEC-009, BETA-TEN-001, BETA-TEN-002, BETA-IAM-004E..004I |
...
| BETA-IAM-004H | P0        | В работе      | `SIGNED_ADMISSION_PROVENANCE_ASSERT_V1`                   | Migration 172 должна создать sealed Ed25519-bound gate attestations, tenant admission decision и exact three-gate links. Подпись связывает заявленные release SHA/environment/schema/artifact/policy/database identity, tenant, locator, исходную reservation claim/execution/profile revisions, claimed `shellEvidenceDigest` и deterministic six-module profile digest; assert принимает current identity только как exact `RESERVATION` либо доказанный immutable command + live `OWNER/NETWORK` invite + encrypted `HOLD` outbox (`ISSUED_HOLD`). Сам checkpoint не доказывает, что заявленная database/release identity является фактическим текущим контекстом процесса, а `shellEvidenceDigest` — фактическим Store/OWNER override/provisioning-audit состоянием. Решение допускает только create/assert/revoke; consume запрещён до activation. Production root registry остаётся пустым и fail-closed; application/PUBLIC не получают table/column/function privileges, runtime allowlist остаётся `7`. Отдельный PG16 integration выполняет `seal → ровно один issue RPC в короткой transaction → persisted outbox → open` и negative AEAD binding/tamper checks. `HOLD→PENDING`, independently acquired actual-context/actual-shell binding, SMTP/worker, tenant/trial mutation, route, production deploy и tester account не входят | BETA-IAM-004G, BETA-TEN-008, BETA-SEC-008..010 |
| BETA-IAM-004I | P0        | Запланировано | `ACTIVATE_AND_RELEASE_OWNER_INVITE_V1`                    | Один sealed activation writer под persisted GO и фиксированным lock order обязан независимо получить фактический текущий DB/release/environment/schema/artifact/policy context и фактический shell: `Tenant`, ровно один inactive `Store`, exact OWNER override/capability digest, provisioning audit/receipt и ровно шесть entitlement rows. Он domain-separate пересчитывает actual-context и actual-shell digests и сопоставляет их с подписанными claims до issue и повторно до consume. Только после этого writer атомарно вызывает dormant issue ровно один раз, запускает trial, переводит exact shell в `ACTIVE/OWNER_INVITED`, CAS-consume admission decision и меняет ровно связанный outbox `HOLD→PENDING`; replay опирается на immutable activation command и не выпускает новый secret. Универсальный standalone release RPC запрещён. Delivery worker/SMTP и protected resend/reissue остаются отдельными gates | BETA-IAM-004H, BETA-TEN-008, BETA-OPS-002 |
...
только на `CURRENT_171` — exact prefix
...
Текущий implementation candidate `CURRENT_171`:
