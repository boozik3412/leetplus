...
- Navigation UX: `/communications` and `/staff` are overview hubs, sidebar product groups stay at major business areas, and legacy guest, assortment, management, administration, sync/import/directory/utility pages now use the same breadcrumbs pattern from the main dashboard to the current working screen.
