...
| BETA-IAM-004K | P0        | В работе      | `PROTECTED_MAIL_WORKER_TENANT_ENROLLMENT_V1`              | Отдельная operator-only ceremony по exact release SHA создаёт или изменяет ровно один tenant-scoped worker enrollment с ожидаемыми database/role name+OID, provider authority digest, bounded delivery policy и optimistic revision; enable, rotate и disable требуют независимого production-like admission, PII/secret-free receipt и audit. Slice 1 отделяет tenant-independent `providerAuthorityDigest` от полного `runtimeConfigDigest`: добавление Tenant B и изменение allowlist/poll/batch/lease/attempt/retry policy не ротирует authority Tenant A, а SMTP timeouts/crypto/release/database/role drift её ротирует; process и SMTP provider владеют canonical frozen config snapshot. Claim/reap/provider marker передают в DB authority; readiness сверяет tenant receipt в приложении. Исторический `complete_v1` authority не принимает и завершает уже выданную lease, поэтому rotate требует `DRAINING + zero CLAIMED`. Добавлен contract-only proposal parser без DB/apply authority; для crash recovery выбран exact persisted-request replay: command identity `(tenantId, action, requestId, contentDigest)` сохраняется до `DRAINING`, новый proposal из `DRAINING` запрещён. До `Готово` остаются additive migration/event ledger, persisted resume/finalize, signed check/apply/rollback/zero-diff, two-tenant PostgreSQL race/drain matrix, hostile ACL, production-like rehearsal и exact-SHA CI. Production registry остаётся пустым; mutation выполняется только после отдельного подтверждения | BETA-IAM-004J, BETA-OPS-002, BETA-OPS-004, BETA-SEC-008..010                |
...
1. завершить `BETA-IAM-004K`: state/event migration, signed tenant
   enrollment check/apply/rollback/zero-diff и two-tenant drain/race evidence;
