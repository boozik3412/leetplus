...
- На 17.08.2026 v2 atomic activation реализован локально: current clean
  ...
  apply/check/reconcile/rollback. Но live production backup/target,
  HBA/TLS/SCRAM/pool/API acceptance, rehearsal и SMTP ещё обязательны, поэтому
...
| `FOUNDER-ACTIVATION-ROLE-DEPLOYMENT`    |        P0 | `ENGINEERING IMPLEMENTED / UNIT 6/6 + SYNTHETIC PG LIFECYCLE PASS / PROD NO-GO`   | Restored-copy-only controller: live-branded preflight перед первым effect, SERIALIZABLE + advisory lock, client-side salted SCRAM verifier вместо raw password SQL, exact LOGIN/NOINHERIT attributes, connection limit/expiry, zero membership/ownership/cross-DB dependencies, ровно CONNECT+USAGE+один wrapper EXECUTE, persisted role marker и secret-free receipt. Synthetic PG16.14: PLAN→APPLIED→ATTESTED→APPLY_RECONCILED→ROLLED_BACK→ROLLBACK_RECONCILED; исходные PUBLIC ACL восстановлены, role/DB/files residue 0. Fixture HBA=trust, production не менялся                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | На immutable production-backup restored copy принять downloaded CI artifact, HBA/TLS/SCRAM auth, dedicated pool/live API call, session drain и controlled emergency revoke; затем migration apply/replay/rollback/zero-diff                                |
...
1. Clean exact SHA и воспроизводимый CI artifact приняты. Исполнимый read-only
   ...
   выполнить уже реализованный exact role controller и принять HBA/TLS/SCRAM,
   dedicated pool/live API call и session drain. Synthetic ACL/lost-response/
   rollback matrix уже зелёная.
