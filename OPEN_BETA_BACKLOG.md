...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  Итоговая матрица `7/7` доказывает NETWORK-only category/supplier mutations,
  tenant-bound product/inventory/sales/movement CSV imports, cross-tenant и
  stale-scope deny. Два последовательных current-head PostgreSQL run дали
  `7/7 + 7/7`, fixture residue `0/0/0/0/0`. Эти четыре новые проверки ещё
  требуется повторить на restored-copy clone. Production roles/SMTP canary,
  reports/exports, остальные file parents,
