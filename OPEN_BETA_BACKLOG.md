...
закрыты. CURRENT187-F локально добавил stable signed policy binding foundation
(planner `16/16`, acquisition/binding `15/15`), но semantic allowlist и remote
exact-SHA CI ещё не приняты. Production и `Tenant A/A1..A4` не изменялись.
...
1. Закрыть CURRENT187 infrastructure admission/provider recovery, exact runtime
