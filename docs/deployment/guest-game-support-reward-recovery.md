...
ровно один связанный с ним `PENDING` wallet item типа
`LOOT_BOX_ENTITLEMENT`, отсутствие любых других wallet items и bonus-ledger,
отсутствие Battle Pass и только настроенный XP. Case wallet item обязан иметь
нулевой `claimXpDelta`, не ссылаться на reward/event и не является денежным
начислением. Немедленный повтор должен дать zero-diff.

Если material effects уже полностью появились, но запрос оборвался до итоговой
support audit-записи, новый preview распознаёт только exact immutable plan,
operator receipt, event, entitlement и case wallet item той же пары
`ticket + fact + rule`. Apply в этом режиме не запускает правило повторно и не
создаёт новый entitlement: он ещё раз проверяет полное постусловие и
идемпотентно завершает недостающий аудит. Обычное событие, несовпавший plan,
лишний wallet item или любая неполная/неоднозначная materialization остаются
stop condition.
...
старый факт до активации или существующий материальный эффект без exact
support-recovery plan и полного безопасного постусловия. Тикет можно закрывать
только после postcondition-запроса и проверки состояния в гостевом портале;
сам кейс остаётся неоткрытым, а возможный приз появляется только после действия
гостя по обычным правилам.
