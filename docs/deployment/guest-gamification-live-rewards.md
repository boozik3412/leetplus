...
- `GUEST_GAME_LEDGER_FALLBACK_GRACE_MS`, `...CLAIM_LEASE_MS`, `...INTERVAL_MS`, `...BATCH_SIZE`, `...TENANT_ID` и `...TENANT_SLUG` задают grace-window, lease для восстановления после рестарта, частоту, пакет и область rollout. Grace-window начинается при первом появлении origin receipt, поэтому исторический re-sync не может немедленно создать fallback-событие.
