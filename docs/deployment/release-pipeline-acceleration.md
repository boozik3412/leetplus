...
| Production repeat-canary `2026-08-27` на `96b28f44…` завершился success до первого двухсекундного poll | idempotent static oneshot был собран systemd раньше, чем authority успел увидеть fresh start; app-result был успешным, но receipt корректно не выпущен | только временный canary service drop-in получает `RemainAfterExit=yes`; authority проверяет fresh `active(exited)` metadata и zero PID/cgroup/jobs, явно останавливает service и лишь затем публикует receipt; timer profile остаётся неизменным |
| Stable timer apply на `85920b7b…` прошёл все worker checks, но финальный authorization verifier отклонил strict installed-control child из-за environment key `_` | очистка exported переменных в parent Bash не гарантирует exact environment следующему внешнему Node process | nested verifier запускается только через `/usr/bin/env -i` с canonical `PATH/LANG/LC_ALL/TZ`; exact-environment Node fixture и static invocation guard обязательны до deploy |
| Fast CI `33877270900` прошёл новый direct PID-1 gate, но двухсекундный successor fixture упал при создании `cutover.lock` | один вызов GNU `install` с `/dev/null` и двумя file destinations трактует предпоследний destination как второй source, потому что последний operand не является каталогом | каждый exact lock-файл создаётся отдельным `install`; до authority fixture требует оба parent как non-symlink `root:root:0700`, а оба lock как non-symlink regular `root:root:0600`, затем исполняет полный plan/apply/check |
| Fast CI `33878593674` дошёл до successor negative, но получил `start-fence directory is noncanonical` вместо ожидаемого отсутствующего fence-файла | verifier намеренно проверяет directory trust раньше file presence, а fixture пропустил создание `.d` для двух additive units и потому не мог достичь заявленного guard | pre-successor модель создаёт только exact non-symlink `root:root:0755` каталоги, доказывает отсутствие `90` fence и `91` authorization, а отдельный negative сохраняет отказ при отсутствующем ancestor; apply обязан создать exact `root:root:0644:1` fence bytes и пройти полный check |
| Fast CI `33879816438` впервые дошёл до successor `plan`, но exact pinned manifest отверг собственный controller как malformed | parser применял `NF != 2` к четырём canonical comment lines, хотя остальные manifest consumers пропускают blank/comment rows | controller сохраняет exact 31-line/SHA pin, отдельно требует 27 classified entries, пропускает только blank/comment rows и строго валидирует остальные; ранний parser fixture проводит actual-manifest positive и unknown-class/third-field/duplicate matrix, а root fixture проверяет controller end-to-end |
| Fast CI `33881525897` после comment fix всё ещё отверг все 27 unit rows | AWK program передаётся shell single-quoted, поэтому regex `\\.` искал literal backslash и любой следующий символ вместо literal dot; корректным AWK escape является `\.` | regex использует один AWK backslash; отдельный parser fixture до дорогих root/systemd gates связывает полную exact AWK grammar-строку controller с behavioral positive/negative matrix, затем root fixture проверяет тот же controller через plan/apply/check |
| Fast CI `33883726435` и Full `33883732716` впервые прошли parser и дошли до successor `apply`, но 60-second drain settling завершился fail-closed | disposable `psql` facade моделировал `rollback_direct_memberships=0` и `audit_direct_memberships=0`, тогда как canonical verifier требует ровно по одному безопасному direct membership; это была устаревшая fixture-строка, production state не проверялся и не менялся | facade теперь возвращает exact 22-field clean audit row с обоими direct-membership counters `1`; полный apply/check/recovery остаётся обязательной end-to-end regression, а причина и exact run IDs сохранены здесь |
