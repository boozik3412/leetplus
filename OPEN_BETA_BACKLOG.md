...
Сводка launch readiness на 10.08.2026: Gate 0 закрыт exact synchronized SHA,
...
CURRENT187-J1 теперь имеет local candidate capability-bearing read-only
...
role-policy facts проверяются fail closed; standalone `10/10`, общий CURRENT187
gate `52/52`. Actual loopback PostgreSQL smoke включён в обязательный CI, но
exact-SHA CI для J1 ещё не получен. Endpoint identity, matched HBA rule,
...
| `CURRENT187-J`                       |        P0 | `FOUNDATION EXACT-HEAD ACCEPTED / J1 LOCAL CANDIDATE / DENY-ONLY`                 | Foundation: exact `APPLICATION/COORDINATOR/MIGRATION/WORKER` synthetic matrix, separate network/host-control brands and five deployment digests, SHA `04ffff27…`, CI `31420665364` — `3/3 SUCCESS`. J1: capability-bearing actual Prisma/PostgreSQL backend-session collector checks exact DB/role OID, application, backend/network, read-only, TLS and role-policy facts; unit `10/10`, aggregate CURRENT187 `52/52`; actual loopback PG smoke is wired into mandatory CI but exact-SHA CI is pending. J1 keeps endpoint/HBA/PgBouncer/negative-probe and every access/effect flag false                                                                                                                                                                                                                                                                                               | Exact-SHA CI for J1; four production service-identity runs; independent endpoint/TLS-peer, HBA/reload and PgBouncer collectors; negative probes; host signer/root, freshness and persisted consumption/revocation; hostile real topology; binding into F/deploy authority; independent review |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   CURRENT187-J foundation на independently signed actual TCP/TLS/HBA/pooler
   collectors и закрыть CURRENT187 infrastructure admission/provider recovery,
