...
- Gate 1MT staff attachment reconciliation controller: добавлен защищённый
  ...
  plan/apply не выполнялись. Настоящий fresh-schema deploy дополнительно
  подтвердил существующий exact-history blocker:
  `20260731120000_identity_mail_delivery_release_head` ожидает preterminal
  digest `7f986797…`, тогда как текущие 178 завершённых migrations дают
  `ba9ca94c…`. До отдельного history fix, clean deploy, restored-copy rehearsal,
  exact production-role/grant review и owner approval production reconciliation
  остаётся `NO-GO`.
...
| BETA-MOD-STAFF-009 | P0        | В работе      | Защитить attachments и evidence                     | Reader всех семи parent kinds, native atomic binding/unbind/quarantine, bind/unbind/replacement races и persisted-user/custom-role/system-role revoke приняты. Knowledge/regulation/training/onboarding/checklist parents используют совпадающую fresh `NETWORK \| STORES` authority. Production read-only trigger inventory `7/7` принят на admitted SHA; aggregate scan выявил `5 446 UNRESOLVED`, `309` multiple-parent, `741` unreferenced и `243` URL-review cases. Digest-bound reconciliation controller прошёл unit `9/9` и disposable PostgreSQL plan/apply/replay/check/rollback/replay/zero-residue, но production plan/apply не выполнялись. До effect нужны exact-history CURRENT179 digest fix, clean deploy, restored-copy rehearsal, least-privilege production-role/grant attestation и owner review; затем audited production reconciliation + zero-diff, archive/delete/orphan/retention production-build browser matrix и общий file journey                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | BETA-MOD-STAFF-001, BETA-SEC-006 |
