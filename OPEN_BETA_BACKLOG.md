...
- Текущий инженерный successor owner onboarding реализован поверх нового
  ...
  `157 suites / 3144 passed / 2 todo`. Это локальное engineering evidence:
  exact SHA/CI artifact этого successor ещё принимается, production SMTP,
  migration, roles, tenant и tester не изменялись. Этот канонический
...
| `FOUNDER-OPERATOR-BETA-GO`              |        P0 | `V2 + CURRENT185 DELIVERY/ACCEPT LOCAL PASS / PRODUCTION NO-GO`                   | Ручное создание tester account запрещено. GO/revoke, atomic v2 activation и dedicated database pool/live least-privilege assertion не требуют USB/offline key; default mode `DISABLED`. V2 одной SERIALIZABLE transaction создаёт OWNER/NETWORK invite, запускает 30-day trial, consume GO и выполняет единственный HOLD→PENDING. На canonical CURRENT185 clean PG16 deployment, restricted-role activation/reissue, worker `SENT`, preview и production `AuthService.acceptInvite` приняты локально: один OWNER/NETWORK, tenant ONBOARDING, claim USER, ciphertext очищен. Reissue exact-SHA baseline — `f33e598a…`; новый CURRENT185 successor ещё ожидает exact-SHA CI. Production и текущие четыре клуба не изменены; PUBLIC/обычные roles не имеют RPC EXECUTE, production activation/worker roles, secrets и grants не создавались                                                                                                                                                                                                                                                                                            | Принять exact SHA/artifact; restored-copy apply/replay/rollback + worker/SENT/accept; production trusted-SMTP canary; Gate 1MT/2; затем PREPARE→one-tenant ACTIVE canary и mailbox-bound OWNER invite                                                      |
...
| `FOUNDER-OWNER-INVITE-LIFECYCLE`        |        P0 | `REISSUE EXACT-SHA ACCEPTED + CURRENT185 SENT/ACCEPT LOCAL PASS / PROD NO-GO`     | Status/revoke принят на `0bc178bc…`; reissue exact-SHA `f33e598a…`, push/PR CI `32098804217`/`32098806708`, artifact `9311012974`. Новый canonical CURRENT185 fail-closed перепривязывает active worker к `185` migrations без расширения delivery RPC. Strict-TLS worker `1/1` и полный disposable PG `activate→revoke→reissue→SENT→preview→accept` `1/1`; full API `157 suites / 3144 passed / 2 todo`. Accept создаёт одного OWNER/NETWORK, переводит tenant в ONBOARDING и claim в USER, очищает ciphertext. CURRENT185 successor пока только local engineering evidence; production SMTP/roles/migration не менялись                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Exact-SHA CI artifact; restored-copy worker enrollment/SENT/accept/restore; production trusted-SMTP canary, Gate 1MT/2 и controlled owner invite                                                                                                           |
