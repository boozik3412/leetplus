...
- Готово: SLA возврата материалов базы знаний получил настройку `revisionSlaDays`; если срок не задан вручную, backend рассчитывает пресет по роли статьи и обязательным rich-материалам, а `/staff/knowledge-base` показывает/редактирует срок в конструкторе.
- Следующая полировка: вынести SLA-пресеты базы знаний в tenant-level настройки, если сетям понадобится единая политика по ролям.
