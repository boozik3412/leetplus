...
| BETA-IAM-004K | P0        | В работе      | `PROTECTED_MAIL_WORKER_TENANT_ENROLLMENT_V1`              | Slice 1 отделяет `providerAuthorityDigest` от `runtimeConfigDigest`; Tenant B не ротирует authority Tenant A. Slice 2 добавляет неавторизующий read-only preflight на exact `CURRENT_179/179`. Slice 3 создаёт вне canonical chain candidate `20260801010000_identity_mail_tenant_enrollment_control_plane`, SQL SHA-256 `e84ba3c4e9e61d1d759b82a33fc22c853471fb0ef908546e755699d0d264f683`: `DORMANT_SCHEMA_ONLY / NOT_DEPLOYABLE`, static `81/81`, self-test `21` probes, `COMPLIANT`; два независимых PostgreSQL 16.13 smoke — `PASS`, source `CURRENT_179/179` zero-diff, disposable clone/temp cleanup подтверждён, review P0/P1/P2=`0/0/0`. Canonical target остаётся `CURRENT_179/179`; historical 004J evidence не меняется. Promotion запрещён до worker v2/runtime attestation, producer/worker tenant advisory lock, `DRAINING` zero-secret barrier, independently signed apply/rollback/zero-diff, production-like rehearsal и `SHARED BETA GO`. Production registry остаётся пустым; deploy, enrollment, SMTP, Tenant B/Store B1, account и invite не выполняются | BETA-IAM-004J, BETA-OPS-002, BETA-OPS-004, BETA-SEC-008..010                |
...
4. independent signature и signed apply/rollback/zero-diff;
5. production-like rehearsal exact release;
