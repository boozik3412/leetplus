...
export const EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_CONTRACT =
  "EXTERNAL_LANGAME_PRODUCTION_190_TO_191_V1";
export const EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_CONFIRMATION =
  "I_ACCEPT_EXACT_EXTERNAL_LANGAME_190_TO_191_V1";
export const EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_PLAN_READY =
...
const TARGET_COUNT = 191;
const TARGET_HEAD = "20260908180000_external_langame_simple_onboarding";
...
export class ExternalLangameCurrent191ProductionUpgradeError extends Error {
  constructor(reasonCode) {
    super(reasonCode);
    this.name = "ExternalLangameCurrent191ProductionUpgradeError";
...
function fail(reasonCode) {
  throw new ExternalLangameCurrent191ProductionUpgradeError(reasonCode);
}
...
function digest(domain, value) {
  return sha256(
    `${EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_CONTRACT}\0${domain}\0${stableJson(value)}`,
  );
}
...
function currentDate(now) {
  ...
  if (!(value instanceof Date) || !Number.isFinite(value.valueOf())) {
    fail("CURRENT191_UPGRADE_CLOCK_INVALID");
  }
  return value;
}
...
function canonicalPublicKey(pem, expectedDigest) {
  ...
  try {
    ...
    fail("CURRENT191_UPGRADE_APPROVAL_KEY_INVALID");
  }
  if (
...
export function normalizeExternalLangameCurrent191ProductionUpgradeManifest(value) {
  ...
  const release = exactRecord(
    ...
    "CURRENT191_UPGRADE_RELEASE_INVALID",
  );
...
function summarizeRawInventory(raw) {
  ...
  const ticketTable = normalizeCriticalObject(
    value.ticketTable,
    "CURRENT191_UPGRADE_TICKET_TABLE_INVALID",
  );
...
async function inspectArtifactAuthority(manifest, verifier = verifyCurrentReleaseArtifact) {
  ...
  if (
    ...
    fail("CURRENT191_UPGRADE_ARTIFACT_MIGRATION_MISMATCH");
  }
  return Object.freeze({
...
export async function inspectExternalLangameCurrent191ProductionUpgradeInventory({
  adapter,
  ...
  return Object.freeze({
    ...
    decision: "CURRENT191_UPGRADE_INVENTORY_READY_NOT_AUTHORIZATION",
...
export async function buildExternalLangameCurrent191ProductionUpgradePlan({
  adapter,
  artifactInspector = inspectArtifactAuthority,
  manifest: rawManifest,
  now = () => new Date(),
...
function approvalPayload(plan) {
  return Buffer.from(
    `${EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_CONTRACT}\0approval\0${plan.planDigest}`,
  );
}

export function signExternalLangameCurrent191ProductionUpgradePlan({
  manifest: rawManifest,
  plan: rawPlan,
  privateKeyPem,
}) {
...
function verifyApproval({ approval, manifest, pinnedApprovalKeySpkiSha256, plan }) {
  const receipt = exactRecord(
    ...
    "CURRENT191_UPGRADE_APPROVAL_INVALID",
  );
  if (
    ...
    fail("CURRENT191_UPGRADE_APPROVAL_INVALID");
  }
...
function assertPlanWindow(plan, manifest, now) {
  ...
  if (
    ...
    fail("CURRENT191_UPGRADE_PLAN_EXPIRED");
  }
}
...
function bridgeInvariantExact(source, target) {
  return (
    stableJson(externalLangameCurrent191BridgeAttestationInvariant(source)) ===
    stableJson(externalLangameCurrent191BridgeAttestationInvariant(target))
  );
}

export async function applyExternalLangameCurrent191ProductionUpgradePlan({
  adapter,
  approval,
  artifactInspector = inspectArtifactAuthority,
  confirmPlanDigest,
  ...
  for (const method of ["quiesce", "restore"]) {
    if (typeof workerSafetyAdapter?.[method] !== "function") {
      fail("CURRENT191_UPGRADE_WORKER_SAFETY_ADAPTER_INVALID");
    }
  }
  ...
  try {
    ...
    if (exactFinalState(current, plan.sourceDatabaseEvidence, manifest)) {
      ...
      const recoveredBridgeDigest =
        externalLangameCurrent191BridgeAttestationDigest(recoveredBridge, {
    ...
    await emitPhase(onPhase, plan, "SOURCE_190_AND_DUAL_SLOT_BRIDGE_VERIFIED");
    workerState = await workerSafetyAdapter.quiesce();
    await emitPhase(onPhase, plan, "LANGAME_DAILY_WORKER_QUIESCED", {
    ...
    if (!["AMBIGUOUS", "SUCCEEDED"].includes(result?.status)) {
      fail("CURRENT191_UPGRADE_MIGRATION_FAILED");
    }
...
export async function verifyExternalLangameCurrent191ProductionUpgradeFinal({
  adapter,
  manifest: rawManifest,
  plan: rawPlan,
  runtimeAdapter,
...
export async function rehearseExternalLangameCurrent191ProductionUpgrade({
  ...
  if (
    ...
    fail("CURRENT191_UPGRADE_REHEARSAL_TARGET_REQUIRED");
  }
  if (
    typeof adapter?.inspect !== "function" ||
    typeof artifactInspector !== "function" ||
  ...
  return Object.freeze({
    ...
    contractVersion: EXTERNAL_LANGAME_CURRENT191_PRODUCTION_UPGRADE_CONTRACT,
    databaseEvidenceDigest: databaseEvidenceDigest(final),
    decision: "CURRENT191_RESTORED_COPY_REHEARSAL_PASS",
    migrationCount: final.migrationCount,
    migrationHead: final.migrationHead,
    recoveredFromLostResponse: result.status === "AMBIGUOUS",
  });
...
DO $lock$ BEGIN PERFORM pg_catalog.pg_advisory_xact_lock(13577189, 190191); END $lock$;
WITH applied AS (
  ...
  WHERE namespace.nspname = 'public' AND relation.relname = 'Store'
), migration_table AS (
  SELECT relation.oid, owner.rolname AS owner, relation.relacl
  FROM pg_catalog.pg_class relation
  JOIN pg_catalog.pg_namespace namespace ON namespace.oid = relation.relnamespace
  ...
  SELECT
    COALESCE(pg_catalog.max(candidate.indexrelid)::text, '0') AS oid,
    CASE
      WHEN pg_catalog.count(*) = 0 THEN 'ABSENT'
      WHEN pg_catalog.count(*) = 1
       AND pg_catalog.bool_and(candidate.indisunique)
       AND pg_catalog.bool_and(candidate.indisvalid)
       AND pg_catalog.bool_and(candidate.indisready)
       AND pg_catalog.bool_and(candidate.indislive)
       AND pg_catalog.bool_and(candidate.indnkeyatts = 3)
       AND pg_catalog.bool_and(candidate.indnatts = 3)
       AND pg_catalog.bool_and(
         candidate.index_owner_oid = (
           SELECT relation.relowner
           FROM pg_catalog.pg_class relation
           WHERE relation.oid = 'public."Store"'::regclass
         )
       )
       AND pg_catalog.bool_and(candidate.indpred IS NULL)
       AND pg_catalog.bool_and(candidate.indexprs IS NULL)
       AND pg_catalog.bool_and(
         pg_catalog.pg_get_indexdef(candidate.indexrelid, 1, true) = '"externalProvider"'
       )
       AND pg_catalog.bool_and(
         pg_catalog.pg_get_indexdef(candidate.indexrelid, 2, true) = '"externalDomain"'
       )
       AND pg_catalog.bool_and(
         pg_catalog.pg_get_indexdef(candidate.indexrelid, 3, true) = '"externalClubId"'
       )
      THEN 'EXACT'
      ELSE 'INVALID'
    END AS definition
  FROM (
    SELECT index_row.*, index_relation.relowner AS index_owner_oid
    ...
    WHERE index_namespace.nspname = 'public'
      AND index_relation.relname = 'store_external_identity_global_uidx'
      AND index_relation.relkind = 'i'
      AND index_row.indrelid = 'public."Store"'::regclass
  ) candidate
  ...
  FROM (
    ...
    WHERE namespace.nspname='public'
      AND relation.relname <> 'store_external_identity_global_uidx'
    UNION ALL
  ...
  'descriptionsBelow20', (
    ...
    FROM (
      ...
      WHERE "externalProvider" IS NOT NULL
        AND "externalDomain" IS NOT NULL
        AND "externalClubId" IS NOT NULL
      GROUP BY "externalProvider", "externalDomain", "externalClubId"
...
async function trustedExecutable(filePath) {
  ...
  if (
    ...
    fail("CURRENT191_UPGRADE_TRUSTED_EXECUTABLE_INVALID");
  }
  return canonical;
}
...
async function runPsql(target, sql, timeoutSeconds) {
  if (
    ...
    fail("CURRENT191_UPGRADE_PRIVILEGED_AUTHORITY_REQUIRED");
  }
  ...
  const unit = `leetplus-current191-db-${randomBytes(8).toString("hex")}.service`;
  const args = [
    "--quiet",
    "--wait",
    "--collect",
  ...
  if (outputLimitExceeded) fail("CURRENT191_UPGRADE_PSQL_OUTPUT_LIMIT");
  return Object.freeze({
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'CURRENT191_UPGRADE_CLUSTER_GUARD_FAILED';
  END IF;
END $guard$;
`;
}
...
async function systemctlAction(systemctl, action, unit) {
  ...
  if (result.code !== 0 || result.signal !== null || stderr !== "") {
    fail("CURRENT191_UPGRADE_WORKER_CONTROL_FAILED");
  }
}

export function createExternalLangameCurrent191WorkerSafetyAdapter() {
  if (
    process.platform !== "linux" ||
    typeof process.geteuid !== "function" ||
    process.geteuid() !== 0
  ...
  return Object.freeze({
    quiesce: async () => {
      if (quiesced) fail("CURRENT191_UPGRADE_WORKER_STATE_INVALID");
    ...
    restore: async (state) => {
      ...
      if (
        ...
        fail("CURRENT191_UPGRADE_WORKER_RESTORE_FAILED");
      }
      quiesced = false;
    },
  });
