...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `17/17`, включая staff attachments;
  ...
  `9/9 + 9/9`, fixture residue `0/0/0/0/0`. Эти шесть добавленных проверок ещё
  требуется повторить на restored-copy clone. Production roles/SMTP canary,
  остальные report/export variants, остальные file parents,
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `17/17`: ассортимент/chat/CRM/users и `PENDING/BOUND` staff attachments; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Latest assortment suite на `58410b37…` расширена до `9/9`: category/supplier NETWORK-only mutations, tenant-bound product/inventory/sales/movement CSV imports и store-scoped report/CSV export, cross-tenant и stale-scope deny дважды прошли на clean current-head PG с zero residue; restored-copy replay этих шести checks обязателен. До готовности остаётся полная list/detail/aggregate/write/all-export/file/BFF/browser/job/SSE/Telegram negative matrix пяти product modules и supporting `INTEGRATIONS`, включая остальные report/export variants и file parents, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 17/17 PASS / HTTP+BROWSER IN PROGRESS`                          | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На CURRENT185 production-backup restored-copy clones выполнены assortment `3/3`, team-chat `3/3`, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`, attachments `3/3`: суммарно `17/17 PASS`; fixture/core/database residue нулевой. Latest current-head assortment suite на `58410b37…` расширена до `9/9` реальными category/supplier mutations, product/fact CSV imports и report/CSV export scope, дважды прошла с residue `0/0/0/0/0`; restored-copy replay шести добавленных checks остаётся обязательным. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`                                                                                                                                                                                                     | Расширить HTTP/BFF/browser matrix и остальные report/export variants; store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram/files/SSE                                                                         |
