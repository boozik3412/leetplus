...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `28/28`, включая staff attachments и
  расширенный assortment HTTP `14/14`;
  ...
  Итоговая матрица `14/14` доказывает NETWORK-only category/supplier mutations,
  ...
  restored-copy clone
  через реальный Nest HTTP/RolesGuard; report BFF routes переведены на
  hardened cookie-backed file/JSON proxies, BFF acceptance `8/8`, семь
  table counts и core counts совпали с source, database residue `0`. Production
  roles/SMTP canary, browser report/export journey, остальные file parents,
  job/SSE/Telegram matrix Gate 1MT, Gate 2 и cutover ещё обязательны, поэтому
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `28/28`: assortment `14/14`, chat/CRM/users и `PENDING/BOUND` staff attachments; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Latest assortment suite на `3e0389b4…` покрывает category/supplier NETWORK-only mutations, tenant-bound product/inventory/sales/movement CSV imports, все local report variants, CSV/XLSX exports, OOS/recommendation state и реальный Nest HTTP/RolesGuard, cross-tenant и stale-scope deny; hardened report BFF acceptance `8/8`. Два restored-copy run дали `14/14 + 14/14`, exact seven-table/core-count equality и zero database residue. До готовности остаётся browser report/export journey, file parents, job/SSE/Telegram negative matrix пяти product modules и supporting `INTEGRATIONS`, outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
| `GATE-1MT-STORE-SCOPE`                  |        P0 | `RESTORED-COPY PG 28/28 PASS / BROWSER IN PROGRESS`                               | fresh DB resolver re-attests tenant/store plus role/custom-role/effective capabilities; assortment `57/66`; in-app game `77/117` fresh NETWORK-only; staff `83/84`; communications `18/18`; all 8 executable users/roles service paths are fresh (`73/73` focused), direct create remains fail-closed and 4 invite routes remain blocked. На CURRENT185 production-backup restored-copy clones выполнены assortment HTTP `14/14`, team-chat `3/3`, CRM communications `4/4`, users/roles A/A1/A2↔B/B1 `4/4`, attachments `3/3`: суммарно `28/28 PASS`; fixture/core/database residue нулевой. Latest exact bytes `3e0389b4…` дважды прошли `14/14` на restored-copy clone; все report reads, selected CSV/XLSX streams, OOS/recommendation mutations прошли Nest HTTP/RolesGuard, hardened BFF acceptance `8/8`, seven-table/core counts совпали с source. Evidence: `gate-1mt-restored-copy-pg-evidence-2026-08-18.md`                                                                                                                                                                                                             | Пройти production-build browser report/download/mutation journey; store-create provisioning/quota; STORES game/staff/salary/CRM, public guest binding, jobs/Telegram/files/SSE, outbound email/digest                                                      |
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `28/28`, trusted TLS SMTP
   и protected enrollment/SENT/accept/disable уже приняты. CURRENT198–202 и USB
   не входят в critical path и остаются deny-only post-beta hardening.
