...
function normalizeTiming(
  ...
  return {
    ...
    timeOfDay:
      timing.mode === "TIME_OF_DAY" ? (timing.timeOfDay ?? "09:00") : null,
  ...
  return [
    {
      ...
      description:
        "Проверка готовности администратора, зоны и рабочих сервисов.",
      items: [
        {
          id: createId("item"),
...
export function StaffChecklistTemplateBuilder({
  ...
  const withDefaultStore = (value: DraftTemplate): DraftTemplate =>
    value.id || value.storeId ? value : { ...value, storeId: defaultStoreId };
  const emptyDraft = withDefaultStore(toDraft(null));
  const initialDraft = initialSourceRegulation
    ? withDefaultStore(draftFromRegulation(initialSourceRegulation))
  ...
  const [sourceRegulationId, setSourceRegulationId] = useState(initialSourceId);
  ...
  const [dragOverItem, setDragOverItem] = useState<DraggedChecklistItem | null>(
    null,
  );
  const selectedStoreName =
    report.stores.find((store) => store.id === draft.storeId)?.name ??
    "Вся сеть";
  const selectedTemplate = draft.id
    ? (report.rows.find((template) => template.id === draft.id) ?? null)
    : null;
  const isReadOnly = draft.id
    ? selectedTemplate?.canManage === false
    : !report.canManageTemplates;
  const summary = useMemo(() => {
    ...
    return {
      ...
      timed: items.filter(
        (item) => normalizeTiming(item.timing).mode !== "NONE",
      ).length,
  ...
  const hasUnsavedChanges =
    !isReadOnly && !isPending && currentDraftSnapshot !== savedDraftSnapshot;

  function loadTemplate(template: StaffChecklistTemplate | null) {
    const nextDraft = withDefaultStore(toDraft(template));
    setDraft(nextDraft);
  ...
  function loadTemplatePack(pack: StaffChecklistTemplatePack) {
    ...
    setMessage(
      "Пак загружен в конструктор. Проверьте клуб, пункты и сохраните как чек-лист.",
    );
  }
  ...
  async function saveTemplate(status?: StaffChecklistTemplateStatus) {
    if (isReadOnly) {
      setMessage("Этот сетевой шаблон доступен только для чтения.");
      return false;
    }
    ...
    setIsConstructorOpen(true);
    setMessage(
      status === "ACTIVE" ? "Чек-лист активирован." : "Чек-лист сохранен.",
    );
    router.refresh();
    return true;
  }
  ...
  function createFromRegulation() {
    ...
    setDraft(withDefaultStore(draftFromRegulation(regulation)));
    setIsConstructorOpen(true);
              ...
              >
                ...
                <label className="mt-3 block text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setSourceRegulationId(event.target.value)
                    }
                ...
                <button
                  ...
                  disabled={
                    isPending || report.publishedRegulations.length === 0
                  }
              ...
              <div className="mt-3 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                    Возьмите основу под типовой контроль и отредактируйте под
                    клуб.
                  </p>
                </div>
            ...
            {report.rows.map((template) => (
              ...
              >
                ...
                {template.canManage ? (
                  ...
                  <div className="border-t border-zinc-200 px-3 py-2 text-xs font-semibold text-zinc-500 dark:border-zinc-800">
                    Сетевой шаблон · только чтение
                  </div>
                )}
              </div>
            ))}
        ...
        <section className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          {isReadOnly ? (
            <div className="mt-4 space-y-3">
              <p className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-sky-800 dark:border-sky-500/30 dark:bg-sky-500/10 dark:text-sky-100">
                Сетевой шаблон доступен как общая основа. Изменять или удалять
                его из клубного доступа нельзя.
              </p>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-2">
                ...
                <label className="block text-sm">
                  ...
                  <select
                    value={draft.storeId}
                    onChange={(event) =>
                      patchDraft({ storeId: event.target.value })
                    }
                    className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
                  >
                    ...
                    onChange={(event) =>
                      patchDraft({
                        shiftKind: event.target
                          .value as StaffChecklistShiftKind,
                      })
                    }
                    className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
                  >
                    ...
                    onChange={(event) =>
                      patchDraft({
                        roleScope: event.target
                          .value as StaffChecklistTemplateRoleScope,
                      })
                    }
                    className="mt-1 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"
                  >
                    ...
                    <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                      {section.items.map((item, itemIndex) => (
                        <div
                          ...
                          onDrop={(event) =>
                            handleItemDrop(event, section.id, item.id)
                          }
                          className={[
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <div className="flex flex-col gap-1">
                              <button
                                type="button"
                                onClick={() =>
                                  moveItemByStep(section.id, item.id, -1)
                                }
                          ...
                          <div className="grid gap-2 lg:col-start-2 lg:col-span-5 sm:grid-cols-[minmax(10rem,1fr)_8rem_8rem_8rem_auto]">
                            ...
                            <label className="text-xs font-semibold uppercase text-zinc-500">
                              ...
                              <input
                                type="number"
                                value={
                                  normalizeTiming(item.timing).offsetMinutes ??
                                  0
                                }
                                disabled={
                                  normalizeTiming(item.timing).mode ===
                                    "NONE" ||
                                  normalizeTiming(item.timing).mode ===
                                    "TIME_OF_DAY"
                                }
                            ...
                            <label className="text-xs font-semibold uppercase text-zinc-500">
                              ...
                              <input
                                type="time"
                                value={
                                  normalizeTiming(item.timing).timeOfDay ??
                                  "09:00"
                                }
                                disabled={
                                  normalizeTiming(item.timing).mode !==
                            ...
                            <label className="text-xs font-semibold uppercase text-zinc-500">
                              ...
                              <input
                                ...
                                max={240}
                                value={
                                  normalizeTiming(item.timing).toleranceMinutes
                                }
                                disabled={
                                  normalizeTiming(item.timing).mode === "NONE"
                                }
                                onChange={(event) =>
                                  patchItem(section.id, item.id, {
                                    timing: {
                            ...
                            <div className="flex items-end">
                              <Toggle
                                checked={
                                  normalizeTiming(item.timing).affectsDiscipline
                                }
                                onChange={(checked) =>
                                  patchItem(section.id, item.id, {
                                    timing: {
