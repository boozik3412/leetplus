...
CURRENT187-J2 теперь имеет local candidate фактического endpoint/TLS-peer
...
`1/1`; exact-SHA CI pending. J2 выставляет только observed, но не attested;
HBA/PgBouncer/negative-probe, production root/runtime и все launch/effect flags
остаются false, внешний доступ остаётся `NO-GO`.
...
| `CURRENT187-J`                       |        P0 | `FOUNDATION + J1 EXACT-HEAD / J2 LOCAL CANDIDATE / DENY-ONLY`                     | Foundation/J1 приняты exact-SHA CI. J2: actual DNS address-set → selected IP/family/port → TCP → PostgreSQL SSLRequest → TLS 1.2/1.3 collector с verify-full hostname/CA, leaf DER/SPKI и validity; unit `10/10`, aggregate CURRENT187 `63/63`, protocol/TLS integration `1/1`, exact-SHA CI pending. J2 связывается с J1 receipt и выставляет endpoint/TLS observed, но не attested; HBA/PgBouncer/negative-probe, production root/runtime и все access/effect flags false                                                                                                                                                                                                                                                                                                                                                                                                              | Exact-SHA CI J2; four production J1+J2 service-identity runs; independent HBA/reload and PgBouncer collectors; negative probes; host signer/root, freshness and persisted consumption/revocation; hostile real topology; binding into F/deploy authority; independent review |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   session collector также принят exact-SHA CI. CURRENT187-J2 actual
   endpoint/TLS-peer collector реализован локально и требует exact-SHA CI.
   Следом реализовать independently signed HBA/PgBouncer collectors и закрыть CURRENT187
