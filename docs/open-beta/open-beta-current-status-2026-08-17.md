...
| Release acceleration | five-phase rollout завершён; V3 и trusted lane metrics merged; independent production-control gates собираются bounded batch с отдельными logs/exit codes. Historical three-dump retirement завершён; exact control `1ea4f39a…` установлен, но первый def5174 plan остановлен E147 до plan record/effect; узкая dangling-link коррекция ожидает новую CI/admission/install и GO                                                   |
...
| External Langame     | обе rollout operations terminal; active green `a05d2a50f4d0382b40bd61cf296c29ea0798fcdf`, rollback blue `fa21bbe99be78313a883893b2dd6dc1d7c892777`. DB остаётся CURRENT190/190, migration pending. Fresh backup + off-host verification завершены; restored-copy ещё не запускалась из-за capacity. Control `1ea4f39a…` установлен; E147 оставил one-file backup и пустой operation root без plan/effect                          |
...
для внешнего onboarding; на момент того historical rollout production был
`def5174f…/CURRENT190`. Текущий source-контракт запрещает ручные
...
добавлен `supersede-after-cutover-intent-bind-rollback`. Он не переключает
nginx и не делает schema effect: safe previous blue продолжает обслуживать
traffic. Legacy `GUEST_SUPPORT_SCHEMA_FORWARD_BRIDGE` и successor
`EXTERNAL_LANGAME_SIMPLE_ONBOARDING_SCHEMA_FORWARD_BRIDGE` совместимы только
в exact `CURRENT_190/190 → CURRENT_191/191` readiness assertion; все остальные
release/build/head/count/profile/flag/migration checks остаются strict.

Это same-contract re-pin: previous и target contracts recovery оба
`CURRENT191/191 + ALLOW_CURRENT_190/OFF`; binder rollback возвращает target link
к `PRIOR_RELEASE=fa21bbe99be78313a883893b2dd6dc1d7c892777`, а не к historical
first-bridge `CURRENT190 OFF/LIVE`.

До terminalizer допускается только receipt-bound порядок: `fence target →
canonical binder rollback → canonical
restore-slot-environment-after-cutover-intent-bind-rollback → unmask/reset-failed
while stopped/dead/PID=0/process-free → terminalize`. Restore mode получает
exact bind/rollback digests, создаёт immutable `04` restore intent/receipt и
crash-safe idempotently atomically возвращает только target env к byte-exact
pre-BIND backup, пока units masked/stopped. Ручной env copy запрещён; restore
не меняет DB, nginx, link или units, а unmask без start возможен лишь после его
receipt. Terminalizer требует exact restore-receipt digest и пишет лишь
...
publication. Shared `.intent`, `.intent.accepting.new` или `.intent.recovering.new`
...
Следующий admitted rollout `72b1b053…` переключил production на active blue и
сохранил green `466ca90d…` hot rollback. Повторный canary `2026-09-04` прошёл
`4/4 SUCCESS` без дублей. Stable timer дошёл до финального authorization check,
но штатный systemd 255 вернул два exact `DropInPaths` через один пробел, тогда
как избыточный предварительный pattern ожидал двойной разделитель. Fail-closed
cleanup выключил timer, удалил permit/drop-in и восстановил 90-fence. Source
repair оставляет exact sorted-set проверку обоих обязательных путей и запрет
третьего, устраняя только зависимость от whitespace. До нового Fast+Full,
control install, canary и terminal timer check внешний beta остаётся `NO-GO`.
...
Dangling systemd dependency-link допустима только при normalized target внутри
compiled unit roots; escaping, malformed или changing ссылка отклоняется.
...
off-host copy verified. После установки control `1ea4f39a…` read-only inventory
показал current available `10,356,858,880` и projected `12,378,058,752`, если
future exact effect успешно завершится, при прежнем requirement
`12,103,890,199`; live capacity будет пересчитана controller. Первый plan
остановлен E147 на единственной dangling generated systemd dependency-link;
operation root пуст, plan/intent/receipt отсутствуют, архив сохранён. Узкая
source-коррекция ещё требует root fixture/CI, exact-main admission/install/GO;
restored-copy и schema effect остаются pending, manual systemd edit/delete
запрещены.
