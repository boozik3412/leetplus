...
`b64abfe5…`. CURRENT187-G локально реализовал secret-free semantic risk facts
по 12 role/ACL surfaces и включил их digest в `clusterCatalogDigest`; signed
semantic allowlist ещё не реализован. Production и
...
| `CURRENT187-G`                       |        P0 | `IMPLEMENTED LOCALLY / DENY-ONLY / NOT AN ALLOWLIST`                              | pure extractor принимает exact canonical rows 12 role/ACL surfaces; secret-free receipt считает privileged role attributes, LOGIN, direct/elevated memberships, settings, ownership, current/default ACL, PUBLIC/grantable grants и effective privileges; malformed/proxy/accessor/PUBLIC mismatch fail closed; per-DB digest агрегируется в уже signed-bound `clusterCatalogDigest`; focused `7/7`, planner `16/16`, acquisition/policy `15/15`; все launch/effect flags false                                                                                                                                                                                                                                                                                                                                                                        | Independently approved signed semantic allowlist и fail-closed facts+allowlist evaluator; persisted consumption/revocation, production roots/GO и actual host/network/runtime attestation                                                                  |
...
1. Поверх CURRENT187-G risk facts реализовать independently approved signed
   semantic allowlist/evaluator; затем закрыть CURRENT187 infrastructure
   admission/provider recovery, exact runtime roles/grants/attestation и
   head-pinned application readiness.
