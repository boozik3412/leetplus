...
- На 18.08.2026 v2 atomic activation реализован и принят exact-SHA CI: current
  ...
  клонах той же clean copy прошла `33/33`, включая real HTTP team-chat SSE,
  ...
  Production roles/SMTP canary, file browser/scope-revoke race, job/Telegram
...
| BETA-MT-007 | P0        | В работе      | Доказать двухtenantную изоляцию на общем PostgreSQL/runtime         | Restored-copy PostgreSQL A/B foundation расширена до `33/33`: assortment `15/15`, team chat `4/4` с real HTTP SSE pre-header deny, CRM/users и staff attachments `6/6` со всеми семью reader parent kinds и native writer/lifecycle/race flow пяти content parents; production-build browser read/admission slice для OWNER/STORES принят на `771bbd5f…`. Exact `94db1fdd…` добавил atomic RecommendationState concurrency и bounded SSR fan-out; два restored-copy run дали `15/15 + 15/15`, report BFF `9/9`, production-build report/download/mutation browser journey, whole-schema `156`-table postflight и zero database residue. Exact `ccf81a28…`/`dfe5e0f8…` закрыл team-chat SSE BFF/API boundary. Exact `abb8a667…` закрыл fresh NETWORK/capability/exact-parent attachment reads; exact `fc07e959…` закрыл atomic native writers, same-parent replay и foreign-tenant rollback; exact `f2e9e6ca…` закрыл status-only retention, unbind, last-reference quarantine и shift-regulation delete lifecycle; exact `7928b7f8…` закрыл row-lock bind/unbind/replacement races: final attachment-focused unit `48/48`, PG `4/4 + 4/4` и `6/6 + 6/6`, zero table diff/residue. До готовности остаются file browser/scope-revoke race, job/Telegram/public-guest negative matrix пяти product modules и supporting `INTEGRATIONS`, controlled outbound digest, stale token, hidden UUID, scope change и owner delegation без test-only bypass | BETA-MT-003..006, BETA-OPS-002, BETA-OPS-003                                     |
...
18. Реализовать `BETA-MT-007..009`: полную shared PostgreSQL/API/BFF/browser/
    ...
    PostgreSQL foundation `33/33`, включая все семь attachment reader parent
    ...
    attachment browser/scope-revoke race и STORES policies, затем background
...
1. Завершить Gate 1MT HTTP/BFF/browser/background adoption по всему
   согласованному scope. Restored-copy PostgreSQL A/B `33/33`, report и SSE browser,
   trusted TLS SMTP и protected enrollment/SENT/accept/disable уже приняты.
   Остались attachment browser/scope-revoke race, jobs/Telegram/public guest
