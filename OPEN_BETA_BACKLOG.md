...
и stale server. J3 `8/8`, J4 `7/7`, общий CURRENT187 gate `78/78`, database
typecheck green. Exact SHA `ceed7239…`, CI `31586755130` — `3/3 SUCCESS`, actual
HBA step `SUCCESS`, artifact `sha256:faf8c3e2…aa283`. Actual disposable
PgBouncer process остаётся отдельным следующим candidate; production
...
| `CURRENT187-J`                       |        P0 | `FOUNDATION + J1/J2/J3/J4 EXACT-HEAD / DENY-ONLY`                                 | Foundation/J1/J2 приняты exact-SHA CI; J2 SHA `d386dfa2…`, CI `31584476362` `3/3 SUCCESS`, artifact `sha256:722f77c2…f495`. J3 actual read-only `pg_hba_file_rules` + reload-clock collector: trust/plaintext/wildcard/regex/group/map deny, `8/8`; current file observed, effective loaded HBA/rule not attested. J4 PgBouncer simple-query SHOW collector: global/database/user/runtime transaction mode, backend/TLS mapping, `force_user`/stale mapping deny, `7/7`. Aggregate CURRENT187 `78/78`, typecheck green. J3/J4 exact SHA `ceed7239…`, CI `31586755130` `3/3 SUCCESS`, actual HBA step `SUCCESS`, artifact `sha256:faf8c3e2…aa283`; production root/runtime, actual PgBouncer process, negative probes и все access/effect flags false                                                                                                                                     | Actual PgBouncer fixture; four production J1/J2 service runs + J3/J4 control runs; positive/negative connection probes; signer/root, freshness and persisted consumption/revocation; hostile topology; binding into F/deploy authority; independent review |
...
1. Принять CURRENT187-I candidate независимой latest-byte проверкой; exact-head
   ...
   exact-SHA CI. CURRENT187-J3 HBA/reload и J4 PgBouncer collectors также
   приняты exact SHA `ceed7239…`, CI `31586755130`; остаётся actual PgBouncer
   fixture. Следом добавить
   independently signed connection-probe evidence и закрыть CURRENT187
